...
test("experience metafile exclusions inspect retained primitive inputs, not deleted factory modules", () => {
  expect(
    prohibitedExperienceInputs("effect-only", [
      "packages/aigpu-api/dist/effect.js",
      "packages/aigpu-api/dist/scene/geometry-src/mesh-box.js",
      "packages/aigpu-api/dist/timer.js",
      "packages/aigpu-api/dist/storage.js",
      "packages/aigpu-api/dist/scene/geometry-descriptor.js",
      "packages/aigpu-api/dist/query-ring.js",
    ]),
  ).toEqual([
    { category: "scene primitive mesh", input: "packages/aigpu-api/dist/scene/geometry-src/mesh-box.js" },
    { category: "timer", input: "packages/aigpu-api/dist/timer.js" },
    { category: "storage", input: "packages/aigpu-api/dist/storage.js" },
    { category: "geometry descriptor", input: "packages/aigpu-api/dist/scene/geometry-descriptor.js" },
    { category: "query ring", input: "packages/aigpu-api/dist/query-ring.js" },
  ]);
  expect(prohibitedExperienceInputs("triangle-low-level", ["packages/aigpu-api/dist/draw.js"])).toEqual([]);
  expect(prohibitedExperienceInputs("triangle-low-level", ["packages\\aigpu-api\\dist\\scene\\geometry-src\\mesh-box.js"])).toEqual([
    { category: "scene primitive mesh", input: "packages/aigpu-api/dist/scene/geometry-src/mesh-box.js" },
  ]);
  expect(prohibitedExperienceInputs("draw-recipe-box", [
    "packages/aigpu-api/dist/scene/geometry-src/mesh-box.js",
    "packages/aigpu-api/dist/scene/geometry-src/mesh-cache.js",
    "packages/aigpu-api/dist/scene/geometry-src/mesh-torus.js",
  ])).toEqual([
    { category: "non-box scene primitive mesh", input: "packages/aigpu-api/dist/scene/geometry-src/mesh-torus.js" },
  ]);
});
...
test("the growth threshold is configurable per package and by flag", () => {
  ...
  expect(resolveThreshold({ name: "x", aigpuBundleBudgetGrowthThreshold: 0.2 })).toBe(0.2);
  expect(resolveThreshold({ name: "x", aigpuBundleBudgetGrowthThreshold: 0.2 }, 0.01)).toBe(0.01);
  expect(() => resolveThreshold({ name: "x", aigpuBundleBudgetGrowthThreshold: "5%" })).toThrow(/non-negative number/);
});
...
test("a missing artifact never passes a budget", () => {
  ...
  expect(formatVerdictLine("@aigpu/gone", verdict)).toContain("missing artifact");
});

test("unclassified entries default to the hard client gate", () => {
  expect(resolvePackageAudience({ name: "@aigpu/core" })).toBe("client");
  expect(resolveExportAudience({ name: "@aigpu/wgsl" }, "./runtime")).toBe("client");
  expect(resolveExportAudience({ name: "@aigpu/wgsl", aigpuBundleAudience: "tooling" }, "./runtime")).toBe("tooling");
  expect(resolveExportAudience({ name: "@aigpu/wgsl", aigpuBundleAudience: "tooling", aigpuExportBundleAudiences: { ".": "client" } }, ".")).toBe("client");
  expect(() => resolvePackageAudience({ name: "@aigpu/core", aigpuBundleAudience: "server" })).toThrow(/unknown audience/);
});
...
test("failures name the budget field, the entry, both sizes and the update command", () => {
  ...
  expect(message).toContain("@aigpu/wgsl");
  expect(message).toContain('packages/wgsl/package.json -> aigpuExportBundleBudgetsGzipBytes["."]');
...
test("tarball measurement ignores the budget metadata it rewrites", () => {
  const manifest = { name: "@aigpu/core", version: "1.0.0", aigpuBundleAudience: "tooling", aigpuBundleBudgetGzipBytes: 32768, aigpuExportBundleBudgetNote: BUDGET_NOTE };
  const stripped = JSON.parse(stripBudgetMetadata("package/package.json", Buffer.from(JSON.stringify(manifest))).toString());
  expect(stripped).toEqual({ name: "@aigpu/core", version: "1.0.0" });
...
test("the tar reader applies PAX path overrides, so filtering and stripping still see real paths", () => {
  const manifest = { name: "@aigpu/core", version: "1.0.0", aigpuBundleAudience: "tooling", aigpuBundleBudgetGzipBytes: 32768 };
  const buffer = tar([
  ...
  const payload = measuredTarballPayload(entries).toString();
  expect(payload).not.toContain("docs");
  expect(payload).not.toContain("aigpuBundleBudgetGzipBytes");
  expect(JSON.parse(payload)).toEqual({ name: "@aigpu/core", version: "1.0.0" });
});
...
test("the tar reader accepts what pnpm pack actually produces", () => {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "pack-sample", version: "0.0.0", files: ["dist", "src/**/*.docs.md"], aigpuBundleAudience: "tooling", aigpuBundleBudgetGzipBytes: 1024 }, null, 2)}\n`,
  );
...
test("bundle-check gates, warns and re-baselines a workspace", () => {
  ...
  patch(manifest, { aigpuExportBundleBudgetsGzipBytes: { ".": measured - 1 }, aigpuExportBundleAudiences: { ".": "tooling" } });
  ...
  patch(manifest, { aigpuExportBundleBudgetsGzipBytes: { ".": Math.floor(measured / 1.2) } });
  ...
  expect(toolingFail.stderr).toContain('aigpuExportBundleBudgetsGzipBytes["."]');
  ...
  patch(manifest, { aigpuExportBundleBudgetsGzipBytes: { ".": measured - 1 }, aigpuExportBundleAudiences: undefined });
  ...
  expect(updated.aigpuExportBundleBudgetsGzipBytes["."]).toBe(nextBudgetBytes(measured));
  expect(updated.aigpuExportBundleBudgetNote).toBe(BUDGET_NOTE);
...
function writeFixture() {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "demo", version: "0.0.0", exports: { ".": { types: "./dist/index.d.ts", import: "./dist/index.js" } }, aigpuExportBundleBudgetsGzipBytes: { ".": 1_000_000 } }, null, 2)}\n`,
  );
  return root;
}
