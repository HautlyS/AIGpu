...
const EXPERIENCE_BUDGET_FIELD = "aigpuExperienceBundleBudgetsGzipBytes";
const EXPERIENCE_FIXTURES = [
  ...
  // #205 owns this public API; do not substitute an internal aigpu/scene import to make it measure.
...
for (const name of (await readdir(packagesDir)).sort()) {
  ...
  if (options.experiences && pkg.name === "aigpu") await checkExperienceBudgets(dir, manifestPath, pkg);
}
...
async function checkExperienceBudgets(dir, manifestPath, pkg) {
  ...
  for (const experience of EXPERIENCE_FIXTURES) {
    ...
    const result = await build({
      ...
      external: ["node:*", "webgpu", "@aigpu/*"], write: false, minify: true, metafile: true,
  ...
  for (const measurement of measurements) {
    ...
    record({
      label: `aigpu experience ${measurement.experience.name}`,
    ...
    for (const retained of prohibitedExperienceInputs(measurement.experience.name, measurement.inputs)) {
      structuralFailures.push(`aigpu experience ${measurement.experience.name}: retained prohibited ${retained.category} input ${retained.input}`);
    }
  }
}
