...
if [[ $mode == vulkan ]]; then
  ...
  [[ ! -e "$HOME/.cache/aigpu/software-renderer" ]]
fi
set +e
json=$(node packages/aigpu-cli/bin/aigpu.js doctor)
status=$?
set -e
printf '%s\n' "$json"
...
if (healthy) {
  ...
  assert.match(icd.prescription, /^run: npx aigpu install-software-renderer/m);
...
if [[ $mode == broken ]]; then
  node packages/aigpu-cli/bin/aigpu.js install-software-renderer
  json=$(node packages/aigpu-cli/bin/aigpu.js doctor)
