...
The asset also passed directly on the current Bookworm host using `aigpu@0.1.6` and its portable Dawn loader: `aigpu doctor` reported **healthy**, adapter **llvmpipe: Mesa 25.0.7 (LLVM 19.1.7)**, type `cpu`; host `ldd` resolved every dependency and render/dispose exited cleanly.
