...
IMAGE_TAG=${IMAGE_TAG:-aigpu-test:s2}
CONTAINER_NAME=${CONTAINER_NAME:-aigpu-test-${$}}
...
docker image prune -f --filter "label=aigpu-test=1" >/dev/null
...
docker run \
  ...
  -e AIGPU_WRITE_SNAPSHOTS="${AIGPU_WRITE_SNAPSHOTS:-}" \
  "$IMAGE_TAG" \
  sh -lc 'Xvfb :99 -screen 0 1024x768x24 >/tmp/xvfb.log 2>&1 & xvfb_pid=$!; AIGPU_DOCKER_TEST=1 pnpm test; status=$?; kill $xvfb_pid; exit $status'
