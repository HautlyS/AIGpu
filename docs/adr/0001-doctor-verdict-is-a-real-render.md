...
`aigpu doctor`'s verdict is produced by actually rendering (init → draw → readback → dispose), not by static checks. Static probes exist to explain failures, not to declare health.
