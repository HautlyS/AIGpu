...
\```ts
import { <symbol> } from "<exact entrypoint>";   ← aigpu | aigpu/node | aigpu/mock | aigpu/scene | aigpu/core | @aigpu/render/<subpath>
\```
...
**Throws:** <code AIGPU-*> when <condition> — <how to fix it>. Omit only if the symbol cannot throw.
...
2. **Do not use ring jargon in public docs — refer to entrypoints and layers instead.** Prefer "the main API (`aigpu`)", "the core layer (`aigpu/core`)", and "scene helpers (`aigpu/scene`)".
...
5. **Returns and Throws are required** for every callable. Cite `AIGPU-*` codes with their condition and fix.
...
9. Files under `skills/aigpu/` are generated: never edit them directly. Edit the co-located source `.docs.md` and regenerate.

## Docs change process

1. Edit the co-located `.docs.md` next to the source.
2. Run `pnpm -F @aigpu/cli generate:docs`.
