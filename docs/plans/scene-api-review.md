...
- **Custom materials can't reach scene lights** — since `@group(0)` is renderer-owned,
  `@aigpu/wgsl-std/scene` must export the lights struct + eval helpers, or every lit
...
- **`set()` silently ignores unknown keys** (medium): diverges from draw.set strictness; a
  `AIGPU-SCENE-KEY-UNKNOWN` error listing accepted keys is what makes mistakes
  LLM-recoverable.
...
`@aigpu/wgsl-std/scene`, `node.find`/`findAll`, `visible` subtree semantics, pipeline cache
