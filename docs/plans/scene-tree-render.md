# Plan: scene tree + renderer for `aigpu/scene`
...
`aigpu/scene` today is a helpers library: pure geometry descriptors (`box()`, `sphere()`, …),
...
- `examples/examples/fft-ocean/camera.ts` hand-rolls `lookAt`/`perspective`/mat4 multiply.
- `examples/examples/raymarched-fractal` and `black-hole` each carry their own
  `installOrbitInput` / `installDragOrbit` pointer-orbit implementation (three copies, two shapes).
- `packages/aigpu-api/tests/scene/primitives/primitive-camera.ts` wraps cameras a fourth way.
- Frozen cameras cannot be resized or animated in place, so every consumer rebuilds them
  per frame — the opposite of aigpu's stable-identity model.

Goal: one retained scene tree (groups/meshes/cameras/lights), a small material contract that
user WGSL can plug into, and a renderer that maps the tree onto aigpu's existing primitives
...
- **Local runtime math.** `geometry-src/camera-math.ts` deliberately avoids loading the
  `wgpu-matrix` runtime to protect the `aigpu/scene` bundle budget; the tree follows the same
...
DOM-touching, browser-only; ships from `aigpu/scene` but is tree-shakeable and inert in Node.
...
`lambertMaterial({ color })` (uses the existing `@aigpu/wgsl-std/light` `lambert()`).
...
const glow = shaderMaterial(/* wgsl */ `
  import { SceneVarying } from "@aigpu/wgsl-std/scene";
...
New `@aigpu/wgsl-std/scene` module (pure — struct/const/fn exports only, per resolver rules):
...
- **Bundle-friendly**: transform/material-param updates are in-place buffer writes; bundles
  ...
  (then the view re-records and reports, mirroring `AIGPU-R3-BUNDLE-STALE` semantics).
...
1. **`mesh(geometry, material)` factory vs the old `Mesh` class — settled by renaming the
   ...
   (`aigpu/scene` re-exports the `Geometry` type instead). `SceneGeometry` (the pure CPU
   recipe) keeps its name; the `AIGPU-MESH-*` error codes keep theirs (scope-bound), while
...
2. **Materials + view** — `@aigpu/wgsl-std/scene`, built-in materials, `shaderMaterial`
...
4. **Topic 2: PBR + glTF** (separate plan) — `@aigpu/wgsl-std/pbr` (BRDF, normal mapping,
   ...
   ```ts
   import { loadGltf } from "aigpu/scene";
   ...
   loaded geometry, or import `@aigpu/wgsl-std/pbr` pieces into their own `shaderMaterial`.
