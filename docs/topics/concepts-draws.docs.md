...
A [`Draw`](/reference/aigpu/draw#draw) renders geometry with custom vertex buffers: you write both the vertex and the fragment stage, and a geometry supplies the buffers. If you want to render a full-screen shader instead, use an [Effect](/concepts/effects).

## Draw a geometry

`geometry(gpu, geometry)` turns geometry from `aigpu/scene` into vertex and index buffers. Your vertex shader declares the attributes it consumes — `@location(0) position`, `@location(1) normal` — and the geometry feeds them.

```ts
import { init, draw, geometry, target } from "aigpu";
...
- `GeometryLike` is an open interface: `geometry(gpu)` builds one from `aigpu/scene` geometry, but you can also pass your own `GPUBuffer`s and vertex layouts. See the [reference](/reference/aigpu/draw#geometrylike).
...
```ts
import { init, draw, surface } from "aigpu";
