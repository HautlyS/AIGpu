...
Validate the complete import graph with `npx aigpu check ./examples/transmission/glass.wgsl`.
...
const glass = readFileSync(
  "./examples/transmission/glass.wgsl",
  "utf8"
);
...
const helpers = [
  readFileSync("./examples/transmission/env-common.wgsl", "utf8"),
...
`effect(gpu)` reflects one raw WGSL string and rejects remaining imports with `AIGPU-WGSL-REFLECT-SOURCE-IMPORT`. The extraction therefore uses the exact shipped math while excluding bindings and entry points.
...
```typescript
import { init, effect, sampler, target } from "aigpu/node";
...
If `npx aigpu doctor` still fails after applying its own fixes, run the harness inside a container with a software GPU. In the aigpu repository, that image is the one CI uses:

```bash
docker build -t aigpu-test-dev:ci -f infra/test-docker/Dockerfile.dev .
docker run --rm -v "$PWD:/workspace" -w /workspace -e AIGPU_DOCKER_TEST=1 aigpu-test-dev:ci \
  sh -lc 'Xvfb :99 -screen 0 1024x768x24 & DISPLAY=:99 node snippet.mjs'
```

Mount an output directory (`-v "$OUT:/out"`) and have the harness write its JSON and PNGs there so the evidence survives the container. This is the aigpu repository's own infrastructure: results are deterministic and identical to CI. In other projects, any Linux container with a software Vulkan stack (Mesa lavapipe or SwiftShader) plus Xvfb works the same way.

## See also

- [The default workflow for developing shaders with aigpu](shader-workflow.docs.md) — the eight steps this methodology escalates from.
- [Shader diagnostics and fix-its](shader-fix-its.docs.md) — error codes such as `AIGPU-RESOLVE-MODULE-BINDING`.
