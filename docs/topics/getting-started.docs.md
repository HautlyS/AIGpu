...
Start with the public `aigpu` package. A program has one `Gpu` context, explicit WGSL bindings, and explicit frames. There are no global uniforms: time comes from the frame clock (`clock(gpu).time`, `.deltaTime`, `.frameCount`) and resolution comes from targets (`target.size`, `target.texelSize`).

```ts
import { clock, init, effect, frameLoop, surface } from "aigpu";
...
[Using aigpu without a bundler](no-bundler.docs.md).

The full step-by-step playbook — from `aigpu docs` to browser validation — is
[The default workflow for developing shaders with aigpu](shader-workflow.docs.md).
...
- Use `set()` for every binding declared in WGSL; missing bindings fail with `AIGPU-R1-BINDING-NEVER-SET`.
- Keep plain JS values plain from their first `set()`; if you need user-owned lifetime, pass a resource from the first `set()`.
- Request optional device capabilities at startup with `init({ requiredFeatures: [...] })` — for example `"timestamp-query"` for `timer(gpu)`; a name the adapter lacks fails init with `AIGPU-FEATURE-UNSUPPORTED`.
...
```sh
aigpu docs cat concepts-context.md         # the Gpu context, surfaces, targets
aigpu docs cat concepts-wgsl-modules.md    # split shaders with import and export
aigpu docs cat concepts-draws.md           # draw(), meshes, instancing
aigpu docs cat concepts-compilation.md     # compile() and pipeline warmup
aigpu docs cat concepts-effects.md         # fragment effects and set()
aigpu docs cat concepts-passes.md          # frame.pass and multi-pass work
aigpu docs cat concepts-frames.md          # frame batching and animation loops
aigpu docs cat concepts-render-bundles.md  # record draws once, replay cheap
```
...
```sh
aigpu docs cat bundlers.md   # Vite (other bundlers or webpack), Vite, .wgsl types, canvas component
```
...
```sh
aigpu docs cat no-bundler.md   # resolveShader(), headless Node rendering, ESM + CommonJS usage
```
...
```sh
aigpu docs cat two-pass-rendering.md   # offscreen depth target -> composite to canvas
```

For performance work and testing:

```sh
aigpu docs cat /guides/performance-model.docs.md
aigpu docs cat /guides/performance-patterns.docs.md
aigpu docs cat browser-testing
```
