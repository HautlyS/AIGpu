...
- Keep WGSL imports pure: modules export helpers only; bindings live in the entry shader. If a module declares a binding, fix `AIGPU-RESOLVE-MODULE-BINDING`.
- For headless tests use `aigpu/mock` for deterministic unit tests and `aigpu/node` only when Dawn/WebGPU behavior is under test.
