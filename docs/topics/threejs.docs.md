---
title: Using aigpu WGSL modules with three.js
summary: Copy the three-tsl reference helper to turn aigpu-resolved pure WGSL functions into callable three.js TSL nodes for node materials.
...
# Using aigpu WGSL modules with three.js

aigpu's WGSL loader can flatten a reusable WGSL module graph into ordinary WGSL, while three.js TSL can call a WGSL function from a node material. The [`three-tsl` example](/examples/three-tsl) connects those two pieces with a small reference helper.

> `wgsl-tsl.ts` is example code, not an export from `@aigpu/wgsl`. Copy it into your project and keep it covered by your own tests if you adapt it or upgrade three.js.
...
```sh
npm install @aigpu/wgsl @aigpu/wgsl-std three@^0.180.0
npm install --save-dev @types/three@^0.180.0 vite typescript
```

The reference implementation is tested against three.js r180. The sample below uses Vite. For webpack, other bundlers, and the ambient TypeScript declaration in more detail, see [Using aigpu with Vite and other bundlers](bundlers.docs.md).

## Copy the reference helper

Copy [`examples/three-tsl/src/wgsl-tsl.ts`](https://github.com/hautlys/AIGpu/blob/main/examples/three-tsl/src/wgsl-tsl.ts) into your source tree, for example as `src/wgsl-tsl.ts`. The same file is visible in the source panel of the [live example](/examples/three-tsl).
...
```wgsl
// surface.wgsl
import { perlin3d } from "@aigpu/wgsl-std/noise/perlin";
...
A `.wgsl` file with no imports takes the loader's leaf fast path. In a leaf file, write an ordinary `fn` without aigpu's `export` marker; an exported leaf is not resolved by the current loader path.
...
```sh
pnpm --filter @aigpu/example-three-tsl test
pnpm --filter @aigpu/example-three-tsl dev
```
...
- [Using aigpu with Vite and other bundlers](bundlers.docs.md) — webpack, other bundlers, Vite, and `.wgsl` TypeScript setup.
- [Using aigpu without a bundler](no-bundler.docs.md) — resolve a WGSL graph directly from Node.js or custom tooling.
