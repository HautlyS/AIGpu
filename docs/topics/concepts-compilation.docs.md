...
```ts
import { init, draw, effect, surface } from "aigpu";
...
```ts
import { init, draw, geometry } from "aigpu";
...
```ts
import { init, draw, target } from "aigpu";
...
The lazy path is different: since `draw()` returns immediately, a pipeline that fails to compile on first use reports through [`gpu.onError`](/reference/aigpu/gpu#onerror), and `gpu.settled()` lets tests wait for those deliveries. Pre-warmed or not, the failure never lands twice.
