...
[`bundle(gpu)`](/reference/aigpu/bundle#bundle) records draws against a target and returns a [`Bundle`](/reference/aigpu/bundle#bundle). Replay it inside a pass with `pass.bundles()`:

```ts
import { init, bundle, clock, effect, frameLoop, surface } from "aigpu";
