...
An [`Effect`](/reference/aigpu/effect#effect) is a full-screen fragment shader created with `effect(gpu, source)`. Its pipeline compiles lazily on first use; call `await effect.compile(target)` during load if you want to pre-warm it. See [Compilation](/concepts/compilation) for the full pre-warm flow. Every draw fills the whole target — you only write the fragment.

Effects chain through targets: render one effect into an offscreen [`Target`](/reference/aigpu/target#target), then bind that target as a texture input of the next effect with `set()`.
...
```ts
import { init, effect, sampler, surface, target } from "aigpu";
...
```ts
import { clock, init, effect, surface } from "aigpu";
