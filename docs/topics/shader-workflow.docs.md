# The default workflow for developing shaders with aigpu
...
```sh
npx aigpu docs ls          # top-level paths: /guides plus one entry per package
npx aigpu docs ls /guides  # every guide, by file name
```

The corpus ships inside the package, so it works offline. Subcommands: `ls`, `cat`, `grep`, `find`, `path`, `symbols`. Full reference: `npx aigpu docs cat cli.docs.md`.

## 2. Read getting started

```sh
npx aigpu docs cat getting-started.md
```
...
```sh
npx aigpu docs cat concepts-wgsl-modules.md # import/export, pure modules, resolution
npx aigpu docs cat concepts-effects.md   # fullscreen fragment work and set()
npx aigpu docs cat concepts-passes.md    # frame.pass, multi-pass work
npx aigpu docs cat concepts-frames.md    # frame batching and animation loops
```
...
```sh
npx aigpu check ./shaders/glass.wgsl
```
...
`doctor` acquires a real adapter and renders a real frame, then prints a JSON verdict. When it is unhealthy, apply the fix it prescribes literally — usually `npx aigpu install-software-renderer`, sometimes a `VK_ICD_FILENAMES` export. Re-run until it reports healthy. If it still fails after its own fixes, use the Docker fallback in [Debugging shaders by extracting internal values](shader-debugging.docs.md).
...
Port the same code to `aigpu` (not `aigpu/node`) with `surface(gpu, canvas, ...)` and drive it from `frameLoop(gpu, ...)`. Test it the way users run it, with the public API and deterministic frame submission: [Browser testing with Playwright WebGPU](browser-testing.docs.md).
