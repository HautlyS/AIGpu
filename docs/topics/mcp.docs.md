---
title: MCP
summary: Set up AIGPU MCP in coding agents to search documentation, inspect verified examples, and opt into scoped local downloads.
websitePath: /mcp
relatedSymbols:
---
# MCP

Connect coding agents directly to AIGPU documentation and verified examples through the [Model Context Protocol](https://modelcontextprotocol.io). Start with the hosted HTTP server for a read-only setup that requires no authentication or installation. Use local stdio when an agent needs package-versioned docs, an offline examples cache, or explicitly scoped example downloads.

## Quick setup

Use `add-mcp` to detect installed MCP clients and add the hosted AIGPU server globally:

```terminal
npx -y add-mcp https://github.com/hautlys/AIGpu/api/mcp -g
```

Remove `-g` to configure clients for the current project instead. The installer lets you review its detected clients before writing their configuration. No AIGPU account or authorization flow is required.

## What is AIGPU MCP?

AIGPU MCP is the official agent interface for the same documentation and verified example source exposed by the AIGPU website and CLI. It gives an agent typed tools to:
...
AIGPU offers two transports. Hosted HTTP is public, stateless, and read-only. Local stdio runs from the `aigpu` npm package and can additionally use the verified local cache or enable confined filesystem writes.
...
| Discovery | `https://github.com/hautlys/AIGpu/.well-known/mcp.json` |
...
```terminal
claude mcp add --transport http aigpu https://github.com/hautlys/AIGpu/api/mcp
```

Start a new Claude Code session or run `/mcp` to confirm that the `aigpu` server and its two tools are available.

### Codex CLI

```terminal
codex mcp add aigpu --url https://github.com/hautlys/AIGpu/api/mcp
codex mcp list
```
...
```json
{
  "mcpServers": {
    "aigpu": {
      "url": "https://github.com/hautlys/AIGpu/api/mcp"
    }
  }
}
```
...
- “Search the AIGPU docs for render pipelines and summarize the setup.”
- “Resolve the API reference for the texture type used by AIGPU.”
...
Search and navigate the canonical AIGPU documentation corpus.
...
Tool failures return bounded structured errors with stable AIGPU error codes. Example manifests and files retain the same compatibility and SHA-256 integrity verification used by the human CLI.

## Local stdio

Run the read-only local server from the public `aigpu` package:
...
Bare stdio does not advertise `download`. It serves documentation bundled with that installed AIGPU package and, unlike hosted HTTP, can read from the verified examples cache with `offline: true`.
...
npx -y aigpu mcp --project-from-cwd

# A fixed project
npx -y aigpu mcp --output-dir /absolute/path/to/project

# A host-managed environment
AIGPU_MCP_OUTPUT_DIR=/absolute/path/to/project npx -y aigpu mcp
```

`--output-dir` and `AIGPU_MCP_OUTPUT_DIR` must name an existing absolute directory. AIGPU resolves symlinks and canonicalizes that boundary before starting. An explicit CLI selector overrides the environment variable, and `--output-dir` cannot be combined with `--project-from-cwd`.
...
AIGPU returns the canonical absolute destination after publication. It rejects absolute or encoded agent paths, dot segments, backslashes, control characters, symlink ancestors, the boundary itself, and existing destinations before repository access. MCP never exposes the human CLI's `--force` behavior.

Use `--project-from-cwd` only when the MCP host launches the command from the active project directory. For a global configuration, prefer a fixed `--output-dir` or `AIGPU_MCP_OUTPUT_DIR`. Codex configurations should omit `cwd` when they are meant to inherit the active workspace. Conductor inherits the selected host's Claude Code, Codex, or Cursor MCP configuration; it does not define another MCP format. MCP does not provide a portable workspace-root authorization boundary, so AIGPU never infers one.
...
- Verify that remote configurations use the official `https://github.com/hautlys/AIGpu/api/mcp` endpoint. No token or authentication header is required.
...
| `download` is missing | Use local stdio on Linux or macOS and configure `--project-from-cwd`, `--output-dir`, or `AIGPU_MCP_OUTPUT_DIR`. Hosted HTTP, bare stdio, and Windows are read-only. |
...
| --- | --- |
| Search and read docs or examples | Hosted HTTP at `https://github.com/hautlys/AIGpu/api/mcp` |
| Use AIGPU docs matching an installed package | Bare local stdio |
