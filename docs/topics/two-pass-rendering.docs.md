...
```ts
import { draw, effect, frame, geometry, init, sampler, surface, target } from "aigpu";
import { box, orbit, perspectiveCamera, sphere } from "aigpu/scene";
...
- **Binding the target itself.** `set({ scene })` passes the `Target` where the WGSL declares a `texture_2d<f32>`; aigpu binds its color texture. Pair it with a `sampler(gpu, ...)` for the `sampler` binding.
...
```ts
import { draw, effect, frame, geometry, init, sampler, target } from "aigpu/node";
...
To load the two shaders from `.wgsl` files instead of inline strings in this setup, resolve them first: [Using aigpu without a bundler](no-bundler.docs.md).
...
- [Using aigpu without a bundler](no-bundler.docs.md) — loading the shaders above from `.wgsl` files.
