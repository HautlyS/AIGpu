...
- **Probe** — one read-only environment check run by `aigpu doctor` (e.g. "Vulkan ICD present").
...
- **Healthy** — the machine can acquire a WebGPU adapter and render headless with `aigpu/node`, by any adapter (hardware or software).
