...
Imports WGSL modules through the `@aigpu/wgsl` Vite loader and connects their
...
(`scripts/generate-previews.ts`): `aigpu/node` creates the Dawn-backed WebGPU
...
involved.
The environment needs a Vulkan ICD for Dawn (see `@aigpu/adapter-node`'s
system requirements: `VK_ICD_FILENAMES`, `XDG_RUNTIME_DIR`,
`AIGPU_DAWN_FLAGS=backend=vulkan`).
...
src/lava.wgsl          heat, crust, sink, and blackbody fields; uses
                       @aigpu/wgsl-std voronoi3d + noise.wgsl
...
scripts/generate-previews.ts  headless preview renders on aigpu/node (Dawn)
scripts/field-viz.ts          renders lava.wgsl fields to PNGs with pure aigpu
```
...
- `lavaGlow` — the full glow composition as `vec2f(heat, meltMask)`:
  ...
  boundaries (`f2 - f1` from `@aigpu/wgsl-std/noise`); melt washes flanking
...
`@aigpu/wgsl-std`) are the sweet spot.

## Tests

`pnpm --filter @aigpu/example-three-tsl test` covers the header parser and
wrapper generation, and resolves `src/lava.wgsl` through
`@aigpu/wgsl/runtime` to check the bridge against real loader output.
