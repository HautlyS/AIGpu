...
// in Node on aigpu's Dawn-backed device — no browser involved.
//
//   pnpm --filter @aigpu/example-three-tsl previews
//
// aigpu/node creates the WebGPU device (see @aigpu/adapter-node's system
...
async function main(): Promise<void> {
  ...
  const { init } = await import("aigpu/node");
  ...
  // aigpu owns the device; three renders on it.
