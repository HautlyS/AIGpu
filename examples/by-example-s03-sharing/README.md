...
This example demonstrates mixed ownership in one bind group. A user-owned `Uniform` named `camera` is shared by two draws, while each draw keeps its own lib-owned `params` color set from plain JS values. Updating or replacing the shared resource is explicit; ordinary per-draw values remain managed by aigpu.
...
docker run --rm --platform linux/arm64 -v "$PWD:/workspace" -w /workspace -e AIGPU_DOCKER_TEST=1 aigpu-test-dev:spike \
