...
test("parses allowlist entries and maps virtual paths", () => {
  const entries = parseAllowlist("@aigpu/core Buffer packages/core/src/buffer.docs.md\n");

  expect(entries).toEqual([{ package: "@aigpu/core", symbol: "Buffer", repoPath: "packages/core/src/buffer.docs.md" }]);
  expect(virtualPathFor(entries[0])).toBe("/@aigpu/core/buffer.docs.md");
});
...
test("fails on missing allowlisted docs", () => {
  expect(() => createManifest("@aigpu/core Missing packages/core/src/Missing.docs.md", {
    exists: () => false,
    read: () => "",
...
test("includes guide docs as a first-class kind", () => {
  const manifest = createManifest("@aigpu/core Buffer packages/core/src/buffer.docs.md", {
    exists: () => true,
    read: (path) => `# ${path}\n\nSummary for ${path}.`,
...
test("extracts schema v3 topic metadata from symbol docs", () => {
  const manifest = createManifest("aigpu Effect packages/aigpu-api/src/effect.docs.md", {
    exists: () => true,
...
test("parses declared search keywords for guides", () => {
  const manifest = createManifest("", {
    exists: () => true,
    read: () => "---\ntitle: Using aigpu with Vite\nkeywords: bundlers, Vite, wgsl loader, declare module, , bundlers\n---\n\n# Using aigpu with Vite\n\nBody.\n",
    guides: ["docs/topics/bundlers.docs.md"],
  });

  expect(manifest.records[0]).toMatchObject({
    symbol: "bundlers",
    topicTitle: "Using aigpu with Vite",
    keywords: ["bundlers", "vite", "wgsl loader", "declare module"],
  });
});
...
test("the shipped bundlers guide declares the queries agents type", () => {
  const record = docsManifest.records.find((item) => item.symbol === "bundlers");

  expect(record).toMatchObject({ package: "guides", kind: "guide", repoPath: "docs/topics/bundlers.docs.md" });
  expect(record?.keywords).toEqual(expect.arrayContaining(["bundlers", "vite", "webpack", "wgsl loader", "declare module"]));
});
...
test("getting-started cat references resolve against the docs index", () => {
  ...
  const refs = [...gettingStartedSource.matchAll(/aigpu docs cat\s+([^\s`|]+)/gu)]
    .map((match) => match[1])
...
test("concept guides preserve canonical title and numeric website order", () => {
  ...
  expect(concepts).toContain("**Context** — Everything in aigpu starts from one call.");
