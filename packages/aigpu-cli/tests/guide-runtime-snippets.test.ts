...
import { init, clock, effect, frame, sampler, surface, target } from "aigpu/mock";

const root = resolve(import.meta.dirname, "../../..");
const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor;

test("getting-started TypeScript fences execute against aigpu/mock", async () => {
  ...
  for (const block of blocks) {
    const executable = block
      .replace(/^import \{[^}]*\} from ["']aigpu(?:\/node)?["'];?\n/mu, "")
...
test("corrected playbook and post-processing patterns run against aigpu/mock", async () => {
