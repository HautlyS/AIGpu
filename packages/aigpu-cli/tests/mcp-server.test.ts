...
import {
  ...
  MCP_MAX_TEXT_BYTES,
  createAIGpuMcpServer,
} from "../lib/mcp/server.js";

const cleanups: Array<() => Promise<void>> = [];
const clientInfo = { name: "aigpu-test", version: "1.0.0" };
type ServerOptions = Parameters<typeof createAIGpuMcpServer>[0];
...
async function connectHttp(options: ServerOptions) {
  const handler = createMcpHandler(() => createAIGpuMcpServer(options));
  const transport = new StreamableHTTPClientTransport(new URL("http://aigpu.test/mcp"), {
...
async function connectInMemory(options: ServerOptions) {
  const server = createAIGpuMcpServer(options);
  ...
  const docs = {
    execute: () => {
      const error = Object.assign(new Error("Ambiguous symbol: Uniform"), {
        code: "AIGPU-DOCS-AMBIGUOUS",
        candidates: [
          { symbol: "Uniform", package: "@aigpu/core", path: "api/core/Uniform.md" },
          { symbol: "Uniform", package: "@aigpu/shader", path: "api/shader/Uniform.md" },
        ],
      });
      throw error;
    },
...
test("non-read results and errors stay within a whole-response budget without duplicating large payloads", async () => {
  ...
  expect(oversized).toMatchObject({
    isError: true,
    structuredContent: { error: { code: "AIGPU-MCP-RESULT-TOO-LARGE" } },
  });
...
test("the real docs service exposes typed misses and ambiguity through MCP", async () => {
  ...
  const missing = await client.callTool({
    name: "docs",
    arguments: { operation: "search", query: "definitely-no-such-aigpu-document" },
  });
  expect(missing).toMatchObject({
    isError: true,
    structuredContent: {
      error: {
        code: "AIGPU-DOCS-NOT-FOUND",
        query: "definitely-no-such-aigpu-document",
      },
    },
  });
    ...
    structuredContent: {
      error: {
        ...
        candidates: expect.arrayContaining([
          expect.objectContaining({ symbol: "Uniform", virtualPath: "/aigpu/core/uniform.docs.md" }),
          expect.objectContaining({ symbol: "Uniform", virtualPath: "/aigpu/uniform.docs.md" }),
        ]),
      },
    },
  });
