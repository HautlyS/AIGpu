...
describe("examples grammar", () => {
  test("documents safe canonical commands", async () => {
    ...
    expect(result.stdout).toContain("npx aigpu examples");
  ...
  test.each([
    ...
    expect(JSON.parse(result.stderr!).error.code).toBe("AIGPU-EXAMPLES-USAGE");
  });
});
...
test("pull publishes atomically, refuses existing destination, and force replaces instead of merging", async () => {
  ...
  await expect(pullExample(client, manifest, destination)).rejects.toMatchObject({ exitCode: 6, code: "AIGPU-EXAMPLES-DESTINATION-EXISTS" });
...
test("Windows pull remains unsupported without downloading files", async () => {
  ...
  await expect(pullExample(client, manifest, "unused", { platform: "win32" } as any)).rejects.toMatchObject({
    code: "AIGPU-EXAMPLES-FILESYSTEM",
