...
import { resolveVersion, runCli } from "../bin/aigpu.js";
import { mcpHelp, runMcpStdio } from "../lib/mcp/stdio.js";

// Under vitest the CLI always runs in-repo, where `bin/aigpu.js` resolves its version from the
// public `aigpu` package (packages/aigpu-api) rather than from `@aigpu/cli`'s own private, drifting
// version -- mirror that resolution here instead of special-casing the test.
const packageVersion = JSON.parse(readFileSync(new URL("../../aigpu-api/package.json", import.meta.url), "utf8")).version;
...
const routerHelp = `aigpu ${packageVersion}
...
test("documents the MCP stdio command without starting a server", async () => {
  await expect(Promise.resolve(runCli(["mcp", "--help"]))).resolves.toMatchObject({
    ...
    stdout: expect.stringContaining(
      "Usage: aigpu mcp [--output-dir <absolute-directory> | --project-from-cwd]",
    ),
  });
});

test("rejects invalid or ambiguous MCP output-directory configuration", async () => {
  const root = mkdtempSync(join(tmpdir(), "aigpu-mcp-cli-options-"));
  ...
  try {
    ...
    const stdioCases = [
      [[], { env: { AIGPU_MCP_OUTPUT_DIR: "relative" } }, "MCP output directory must be absolute: relative\n"],
...
// aigpu-api/package.json happens to exist at the exact path the branch would read. This test is the
// executable guard for that hard constraint: if it ever fails, `npx aigpu --version` is wrong.
test("resolveVersion ignores the sibling aigpu-api package for the published tarball shape", () => {
  const tmp = mkdtempSync(join(tmpdir(), "aigpu-cli-version-"));
  try {
    ...
    // exact path the branch would consult from this `here` (resolve(here, "../../aigpu-api") ==
    // <tmp>/dist/aigpu-api), the other one level further out, so any near-miss of that lookup still
    // finds a mismatching version instead of falling back to the stamp.
    for (const dir of [resolve(here, "../../aigpu-api"), join(tmp, "aigpu-api")]) {
      mkdirSync(dir, { recursive: true });
      writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "aigpu", version: "1.2.3" }));
    }
...
test("resolveVersion resolves the in-repo @aigpu/cli version from the real aigpu-api sibling", () => {
  const binDir = dirname(fileURLToPath(new URL("../bin/aigpu.js", import.meta.url)));
  expect(resolveVersion(binDir, { name: "@aigpu/cli", version: "0.0.0-test" })).toBe(packageVersion);
...
test("exposes the manual native installers", async () => {
  await expect(Promise.resolve(runCli(["install-dawn", "--help"]))).resolves.toMatchObject({
    code: 0,
    stdout: expect.stringContaining("AIGPU_CACHE_DIR"),
  });
...
test("puts the getting-started guide first in CLI help", () => {
  ...
  expect(rootHelp.indexOf("npx aigpu docs cat getting-started.md")).toBeLessThan(rootHelp.indexOf("## Validate shader code"));

  const help = success(["docs", "help"]);
  expect(help).toContain("Usage: aigpu docs <command>");
  expect(help.split("\n")[2]).toBe("Start here: aigpu docs cat getting-started.md   (the guide for using the latest API correctly)");
  expect(help.indexOf("aigpu docs cat getting-started.md", help.indexOf("Examples:"))).toBeLessThan(help.indexOf("aigpu docs ls /guides"));
});

test("curates root and guide listings for onboarding", () => {
  const root = success(["docs", "ls"]).trimEnd().split("\n");
  expect(root.slice(0, 4)).toEqual(["/guides", "/aigpu", "/aigpu/scene", "/aigpu/core"]);
  expect(root.at(-1)).toBe("Tip: start with \"aigpu docs cat getting-started.md\"; /guides holds concept guides; @aigpu/render/* is low-level tooling.");
  expect(root.indexOf("/@aigpu/wgsl")).toBeLessThan(root.indexOf("/@aigpu/render/edit"));
  ...
  expect(success(["docs", "ls", "/aigpu/core"])).toContain("buffer.docs.md");
});

test("cats docs by path and unique symbol", () => {
  expect(success(["docs", "cat", "/aigpu/core/buffer.docs.md"])).toContain("# Buffer");
...
test("cats getting-started guide from forgiving guide names", () => {
  ...
  for (const form of acceptedForms) {
    ...
    expect(output).toContain("aigpu docs cat browser-testing");
  }
});
...
test("greps content with case and package options", () => {
  ...
  const filtered = success(["docs", "grep", "-i", "--package", "@aigpu/wgsl", "MINIFY"]);
  expect(filtered).toContain("/@aigpu/wgsl/");
});

test("finds symbols and resolves paths", () => {
  expect(success(["docs", "find", "Buffer"])).toContain("Buffer\taigpu/core");
  expect(success(["docs", "path", "Buffer"])).toBe("/aigpu/core/buffer.docs.md\n");
  expect(success(["docs", "path", "/aigpu/core/buffer.docs.md"])).toBe("/aigpu/core/buffer.docs.md\n");
...
// The queries below are the ones a base agent actually typed while building a Vite app against
// aigpu (see the dogfood friction log). Every one of them used to print "No docs found"; each must
// now land on the page that answers it.
test("routes the dogfood queries to the page that answers them", () => {
  const cases: [string, string][] = [
    ["vite", "/guides/bundlers.docs.md"],
    ["bundlers", "/guides/bundlers.docs.md"],
    ["bundler", "/guides/bundlers.docs.md"],
    ["wgsl loader", "/guides/bundlers.docs.md"],
    ["wgsl import", "/guides/bundlers.docs.md"],
    [".wgsl", "/guides/bundlers.docs.md"],
    ["typescript wgsl import", "/guides/bundlers.docs.md"],
    ["declare module", "/guides/bundlers.docs.md"],
    ["d.ts", "/guides/bundlers.docs.md"],
    ["webpack", "/@aigpu/wgsl/loader-webpack/index.docs.md"],
    ["noise", "/@aigpu/wgsl-std/noise/index.docs.md"],
    // Cloud/plasma looks: Perlin and simplex are dedicated subpaths with their own fBM helpers.
    ["fbm", "/@aigpu/wgsl-std/noise/perlin/index.docs.md"],
    ["perlin", "/@aigpu/wgsl-std/noise/perlin/index.docs.md"],
    ["simplex", "/@aigpu/wgsl-std/noise/simplex/index.docs.md"],
    // The router advertises `docs find "<AIGPU-error-code>"`, so codes must resolve too.
    ["AIGPU-WGSL-PKG-NOTFOUND", "/@aigpu/wgsl/runtime/resolve-shader.docs.md"],
    ...
    // require() from CJS works for @aigpu/wgsl/runtime; the guide documents both module systems.
    ...
    ["resolveShader", "/@aigpu/wgsl/runtime/resolve-shader.docs.md"],
...
test("find requires every word of the query to match and still reports honest misses", () => {
  ...
  expect(success(["docs", "find", "wgsl webpack"])).toContain("/@aigpu/wgsl/loader-webpack/index.docs.md");
...
// "gpu" is a substring of "aigpu", so route text (titles like "@aigpu/render/edit", every
// /aigpu/... path) used to match 134 records and print them alphabetically, burying the exact `Gpu`
...
test("caps and ranks the gpu route-hits tier", () => {
  ...
  expect(lines[0]).toBe("Gpu\taigpu\t/aigpu/gpu.docs.md");
...
test("does not add a notice when the route-hit count is under the cap", () => {
  ...
  expect(out).toContain("Buffer\taigpu/core");
...
test("caps the content-tier fallback with the same notice format", () => {
  // "shader" matches no route-specific symbol, so it exercises the content-search tier, which
  // exceeds the 20-result cap in the current 34-guide catalog.
  const out = success(["docs", "find", "shader"]);
  const lines = out.trimEnd().split("\n");
  expect(lines.length).toBeLessThanOrEqual(21);
  expect(lines.at(-1)).toMatch(/showing the 20 best\. Add another word to narrow\.$/);
});
...
test("ranking never filters route hits into the body-search fallback", () => {
  const out = success(["docs", "find", "gpu"]);
  expect(out).toContain("/aigpu/gpu.docs.md");
  expect(out).toContain("Gpu\taigpu\t/aigpu/gpu.docs.md");
});

test("the bundlers guide is reachable by cat and from getting-started", () => {
  for (const form of ["bundlers", "bundlers.md", "/guides/bundlers.docs.md"]) {
    expect(success(["docs", "cat", form])).toContain("# Using AIGpu with Vite and other bundlers");
  }
  expect(success(["docs", "cat", "getting-started.md"])).toContain("aigpu docs cat bundlers.md");
});
...
test("the no-bundler and two-pass guides are reachable by cat and from getting-started", () => {
  for (const form of ["no-bundler", "no-bundler.md", "/guides/no-bundler.docs.md"]) {
    expect(success(["docs", "cat", form]), `docs cat ${form}`).toContain("# Using aigpu without a bundler");
  }
  ...
  expect(gettingStarted).toContain("aigpu docs cat no-bundler.md");
  expect(gettingStarted).toContain("aigpu docs cat two-pass-rendering.md");
  expect(gettingStarted).toContain("[Using aigpu without a bundler](no-bundler.docs.md)");
...
test("the new guides do not steal the symbol routes of the API pages they describe", () => {
  expect(success(["docs", "find", "Target"])).toContain("/aigpu/target.docs.md");
  expect(success(["docs", "find", "TargetOptions"])).toContain("/aigpu/target.docs.md");
  expect(success(["docs", "find", "Effect"])).toContain("/aigpu/effect.docs.md");
  // The body-fallback step still runs for queries no route text claims.
  expect(success(["docs", "find", "AIGPU-WGSL-PKG-NOTFOUND"])).toContain(
    "/@aigpu/wgsl/runtime/resolve-shader.docs.md",
  );
  ...
  expect(success(["docs", "find", "fbm"])).not.toContain("/@aigpu/wgsl-std/noise/index.docs.md");
});
...
test.skipIf(process.env.AIGPU_DOCKER_TEST === "1")("snapshot command requires the Docker GPU harness", async () => {
  await expect(Promise.resolve(runCli(["snapshot", "--ci"]))).resolves.toMatchObject({
    code: 1,
    stderr: expect.stringContaining("AIGPU_DOCKER_TEST=1"),
  });
});
