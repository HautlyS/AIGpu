...
test("docs service returns ranked structured search results", () => {
  ...
  expect(result.results[0]).toEqual({
    ...
    symbol: "Buffer",
    package: "aigpu/core",
    virtualPath: "/aigpu/core/buffer.docs.md",
  });
});
      ...
      candidates: [
        {
          symbol: "Uniform",
          package: "aigpu/core",
          virtualPath: "/aigpu/core/uniform.docs.md",
          repoPath: "packages/aigpu-api/src/core/uniform.docs.md",
          kind: "api",
        },
        {
          symbol: "Uniform",
          package: "aigpu",
          virtualPath: "/aigpu/uniform.docs.md",
          repoPath: "packages/aigpu-api/src/core/uniform.docs.md",
          kind: "api",
        },
      ],
...
test("docs service lists a documentation directory without CLI-only hints", () => {
  ...
  expect(docs.execute({ operation: "list", path: "/" })).toMatchObject({
    operation: "list",
    path: "/",
    entries: expect.arrayContaining(["/guides", "/aigpu", "/aigpu/core"]),
  });
...
test("the bundled CLI reference documents both MCP transports and their write boundary", () => {
  ...
  expect(result.document.content).toContain("https://github.com/hautlys/AIGpu/api/mcp");
  expect(result.document.content).toContain("npx aigpu mcp --project-from-cwd");
...
test("the project-scoped Codex MCP example follows the active workspace cwd", () => {
  ...
  expect(block?.[1]).toContain('args = ["-y", "aigpu", "mcp", "--project-from-cwd"]');
  expect(block?.[1]).not.toContain("cwd =");
  expect(result.document.content).toContain("npx aigpu mcp --output-dir /absolute/path/to/project");
