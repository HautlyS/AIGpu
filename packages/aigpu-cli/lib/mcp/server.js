...
export function createAIGpuMcpServer({
  version,
  docs,
  examples,
  allowDownload = false,
  ...
  const server = new McpServer(
    ...
    {
      instructions: [
        "Use docs to discover and read AIGPU documentation. Use examples to find and inspect working examples.",
        ...(allowDownload
  ...
  server.registerTool(
    "docs",
    {
      title: "AIGPU documentation",
      description: "Search, resolve, list, grep, and read the canonical AIGPU documentation corpus.",
  ...
  server.registerTool(
    "examples",
    {
      title: "AIGPU examples",
      description: allowDownload
        ? "Search, inspect, read, or download a verified AIGPU example beneath the configured MCP output directory."
        : "Search, inspect, and read verified AIGPU examples.",
...
function normalizeToolError(error) {
  ...
  if (/not found/iu.test(message)) return { code: "AIGPU-DOCS-NOT-FOUND", message };
  if (/^Ambiguous symbol:/u.test(message)) return { code: "AIGPU-DOCS-AMBIGUOUS", message };
  return { code: "AIGPU-TOOL-ERROR", message };
}
...
function resultTooLarge(message) {
  return Object.assign(new Error(message), { code: "AIGPU-MCP-RESULT-TOO-LARGE" });
}
