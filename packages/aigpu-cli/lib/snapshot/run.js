...
const DEFAULT_BASELINE = "packages/aigpu-api/tests/__snapshots__/representative-gradient.png";

export async function runSnapshotCommand(options = {}) {
  if (process.env.AIGPU_DOCKER_TEST !== "1") {
    return { code: 1, stderr: "aigpu snapshot must run inside the Docker GPU harness with AIGPU_DOCKER_TEST=1.\n" };
  }
...
function snapshotUsage() {
  return "Usage: aigpu snapshot [--ci] [--update] [--baseline <path>]\n";
}

async function loadNodeInit() {
  try {
    // The whole namespace: the render needs `init` plus the free functions it calls.
    const api = await import("aigpu/node");
    return { api };
  } catch (error) {
    if (error?.code !== "ERR_MODULE_NOT_FOUND" && error?.code !== "ERR_PACKAGE_PATH_NOT_EXPORTED") throw error;
    return { error: "aigpu snapshot requires the peer package `aigpu` with its `aigpu/node` entry installed. Install it with `pnpm add aigpu` or run this command from the AIGPU workspace.\n" };
  }
}
