...
```ts
import { perspectiveCamera, type PerspectiveCameraOptions } from "aigpu/scene";
```
...
declare function perspectiveCamera(options: PerspectiveCameraOptions): import("aigpu/scene").PerspectiveCamera;
```
...
**Throws:** `AIGPU-SCENE-VALUE-INVALID` for out-of-range `fov`, non-positive `near`, or `far <= near`.

## Examples

```ts
import { perspectiveCamera } from "aigpu/scene";
...
```ts
import type { PerspectiveCamera } from "aigpu/scene";
```
...
declare class PerspectiveCamera {
  set(values: import("aigpu/scene").PerspectiveCameraValues): this;
...
**Throws:** `AIGPU-SCENE-VALUE-INVALID` from `set()` on invalid projection parameters.

## Examples

```ts
import { perspectiveCamera, type PerspectiveCamera } from "aigpu/scene";
...
```ts
import type { PerspectiveCameraValues } from "aigpu/scene";
```
