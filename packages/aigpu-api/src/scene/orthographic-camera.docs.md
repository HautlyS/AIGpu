...
```ts
import { orthographicCamera, type OrthographicCameraOptions } from "aigpu/scene";
```
...
interface OrthographicCameraOptions {
  ...
  readonly target?: import("aigpu/scene").CameraVec3;
  readonly up?: import("aigpu/scene").CameraVec3;
  readonly label?: string;
}

declare function orthographicCamera(options: OrthographicCameraOptions): import("aigpu/scene").OrthographicCamera;
```
...
**Throws:** `AIGPU-SCENE-VALUE-INVALID` for non-positive `near` or `far <= near`. Avoid `left === right` and `bottom === top`; they produce unusable matrices rather than a validation error.

## Examples

```ts
import { orthographicCamera } from "aigpu/scene";
...
```ts
import type { OrthographicCamera } from "aigpu/scene";
```
...
declare class OrthographicCamera {
  set(values: import("aigpu/scene").OrthographicCameraValues): this;
...
**Throws:** `AIGPU-SCENE-VALUE-INVALID` from `set()` on invalid near/far planes.

## Examples

```ts
import { orthographicCamera, type OrthographicCamera } from "aigpu/scene";
...
```ts
import type { OrthographicCameraValues } from "aigpu/scene";
```
