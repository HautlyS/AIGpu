...
```ts
import { degToRad } from "aigpu/scene";
```
...
- Public camera helpers in `aigpu/scene` accept degrees for field-of-view values; do not convert `PerspectiveCameraOptions.fov` yourself.
