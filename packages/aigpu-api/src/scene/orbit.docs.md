...
```ts
import { orbit, type Mat4 } from "aigpu/scene";
...
- `orbit` does not read global time. Pass `clock(gpu).time` from the main API (`aigpu`) or your own clock explicitly.
