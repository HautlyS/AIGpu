...
import { claimedGroupNativeValidationError, type AIGpuError } from "./errors.ts";
...
export type ValidationErrorSink = (error: AIGpuError) => void | Promise<void>;
 ...
 * aigpu can validate their layout synchronously and the normal path should not
 ...
 * delivered to the supplied device-level error sink as `AIGPU-R4-GROUP-VALIDATION`;
...
function defaultErrorSink(error: AIGpuError): void {
  console.error(error);
}
