 ...
 */
import { Device, validateRequiredFeatures, type RequiredDeviceLimits, type AIGpuAdapter } from "@aigpu/core";
import type { GpuErrorListener } from "./api-types.ts";
import { unsupportedError, AIGpuError } from "./errors.ts";

/**
 * Options for the device aigpu creates and owns; it destroys that device on `dispose()`.
 *
...
export type AdapterFactory = () => AIGpuAdapter;
...
export interface Kernel {
  ...
  reportError(error: AIGpuError): Promise<void>;
...
export function kernelOf(gpu: Gpu): Kernel {
  ...
  if (!kernel) {
    throw new AIGpuError({
      code: "AIGPU-GPU-FOREIGN",
      message: "This object was not created by init(); it has no aigpu kernel.",
      fix: "Pass the gpu returned by init() from aigpu, aigpu/node or aigpu/mock.",
      where: "gpu",
    });
  }
  return kernel;
