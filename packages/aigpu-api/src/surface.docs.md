...
```ts
import type { Surface, SurfaceOptions, SurfaceResizeEvent } from "aigpu";
```
...
**Throws:** `AIGPU-SURFACE-CONTEXT` when `getContext("webgpu")` returns `null`; `AIGPU-SURFACE-DUPLICATE` when a live surface already owns the canvas; `AIGPU-SURFACE-AUTORESIZE-UNSUPPORTED` for explicit `autoResize: true` on buffer-only canvases; `AIGPU-SURFACE-DISPOSED` when using a disposed surface; `AIGPU-SURFACE-RESIZE-REENTRANT` when resizing the same surface from its own resize callback; `AIGPU-FRAME-REENTRANT` when `frame(gpu)` is called from any `onResize` callback. The immediate `onResize` fire on subscription also counts as being inside an `onResize` callback, so call `frame(gpu)` before subscribing or from code outside the callback.
...
```ts
import { init, effect, frame, surface, target } from "aigpu/mock";
...
```ts
import { init, surface, target } from "aigpu";
