...
Main API (`aigpu`) render bundle recorded by `bundle(gpu, { target }, cb)`. Bundles freeze commands, attachment formats, sample count, and bind-group identities for static work; `FramePass.bundles()` checks signature and resource staleness (`AIGPU-R3-BUNDLE-STALE`) when replaying.

## Import

```ts
import type { Bundle, BundleOptions, BundleRecorder } from "aigpu";
```

## Signature

```ts
import type { Draw, DrawCallOptions, Effect, Target, TargetSignature } from "aigpu";
...
**Throws:** `AIGPU-R3-BUNDLE-STALE` when replay target formats/depth/sample count differ from the recorded signature or when a recorded draw's bound resource identity / claimed group changed after recording; `AIGPU-R3-BUNDLE-INVALID` when replay receives an object not created by `bundle()`; `AIGPU-BUNDLE-BLEND-CONSTANT` when recording a draw with `blendConstant` (the blend constant is render-pass state that render bundle encoders cannot set; encode such draws in a frame pass instead); `AIGPU-BUNDLE-STENCIL-REF` when recording a draw whose `stencil` has `ref` (the stencil reference is likewise render-pass state; stencil state without `ref` records fine); `AIGPU-SURFACE-DISPOSED` when replaying against a disposed surface; draw binding errors such as `AIGPU-R1-BINDING-NEVER-SET` can throw during recording. Signature mismatch messages print both recorded and actual signature keys.

## Examples

```ts
import { init, bundle, draw, frame, target } from "aigpu/mock";
...
```ts
import { init, bundle, effect, frame, surface } from "aigpu/mock";
...
```ts
import { init, bundle, clock, effect, frame, pingPong } from "aigpu/mock";
...
- Re-record when the bundle samples a resized target; aigpu detects the changed texture identity and reports `AIGPU-R3-BUNDLE-STALE`.
...
- Draws with `blendConstant` cannot be recorded: render bundle encoders have no way to set the pass blend constant. Recording throws `AIGPU-BUNDLE-BLEND-CONSTANT`; use `FramePass.draw` for those draws.
- Draws whose `stencil` has `ref` cannot be recorded either: render bundle encoders have no way to set the pass stencil reference. Recording throws `AIGPU-BUNDLE-STENCIL-REF`; stencil pipeline state without `ref` records fine.
