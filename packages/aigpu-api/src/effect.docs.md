...
```ts
import type { Effect, EffectOptions } from "aigpu";
```

## Signature

```ts
import type { DrawCallOptions, Target, TargetSignature } from "aigpu";
...
|---|---|---:|---|---|
| effect.source | `string \| ShaderSource` | ✔ | — | WGSL string or `ShaderSource`. If no `@vertex` entry exists, aigpu injects a fullscreen triangle vertex stage and provides `@location(0) uv`. |
...
| opts.label | `string` | ✖ | `"effect"` | Used in shader reflection labels, GPU object labels, and `AIGPU-*` error `where` fields. |
...
**Throws:** `AIGPU-TARGET-REQUIRED` when `effect.draw()` or compile pre-warm is called without `target`; `AIGPU-BLEND-INVALID` for an unknown blend preset or malformed blend object; `AIGPU-WRITEMASK-INVALID` for a non-array or unknown write mask channel; `AIGPU-RING1-UNSUPPORTED` when `effect(gpu)` receives mesh/vertex data; `AIGPU-SHADER-SOURCE-INVALID` for malformed `ShaderSource`; `AIGPU-R1-BINDING-NEVER-SET` when a reflected binding has no value at draw time; `AIGPU-R1-OWNERSHIP-FLIP` when a binding switches between JS-value and resource ownership; `AIGPU-SET-TEXTURE-FILTERABILITY` when an ordinarily sampled facade texture is not filterable (structured detail names its format/binding and paired sampler; use a filterable format, request `float32-filterable`, or use `textureLoad` without a sampler). Asynchronous draw validation errors are delivered through `gpu.onError`; tests can `await gpu.settled()`.
...
```ts
import { init, effect, target } from "aigpu/mock";
