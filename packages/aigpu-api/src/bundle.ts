...
import { bundleBlendConstantError, bundleStencilReferenceError, surfaceNotInFrameError, AIGpuError } from "./errors.ts";
 ...
 * recorded state is re-checked at replay and a mismatch throws `AIGPU-R3-BUNDLE-STALE` instead of
...
function bundleStaleError(id: string, message: string): AIGpuError {
  return new AIGpuError({ code: "AIGPU-R3-BUNDLE-STALE", message, where: `bundle '${id}' replay` });
}
