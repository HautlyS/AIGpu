...
export const FRAME_DRAWABLE: unique symbol = Symbol("aigpu.frame.drawable");
...
export const FRAME_BUNDLE: unique symbol = Symbol("aigpu.frame.bundle");

export interface FrameBundleProtocol {
  readonly gpu: GPURenderBundle;
  /** Throws `AIGPU-R3-BUNDLE-STALE` when the recorded signature no longer matches `target`. */
...
export const FRAME_PASS_ATTACHMENT: unique symbol = Symbol("aigpu.frame.passAttachment");
