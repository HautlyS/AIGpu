/**
 * Adoption of a `GPUDevice` aigpu did not create.
 *
 ...
 * program that lets aigpu create its own device never bundles it. See the `init-only` budget in
...
/**
 * Wraps a device owned by another library (WebLLM, transformers.js, a host engine) so aigpu can
 * render from the same queue without copying through the CPU.
 *
 * aigpu never destroys what it did not create: `gpu.dispose()` drops this wrapper and leaves the
...
export async function initFromDevice(device: GPUDevice): Promise<Gpu> {
  if (!isGPUDeviceShape(device)) throw initError("AIGPU-INIT-DEVICE-INVALID", "Invalid external GPUDevice shape.");
...
function initError(code: string, message: string): AIGpuError { return new AIGpuError({ code, message, where: "initFromDevice" }); }
