...
```ts
import type { Geometry, GeometryOptions, GeometryBufferOptions, GeometrySliceOptions } from "aigpu";
...
```ts
interface Gpu {
  geometry(geometry: import("aigpu/scene").SceneGeometry): Geometry;
...
| Code | When | Fix |
|---|---|---|
| `AIGPU-MESH-LAYOUT-INVALID` | Invalid stride/offset/format, both `data` and `buffer`, or integer-like attribute key. | Use record keys that are names, align offsets/strides, and choose one data source. |
| `AIGPU-MESH-LIMIT-EXCEEDED` | More than 8 buffers or 16 attributes. | Split draws or reduce streams/attributes. |
| `AIGPU-MESH-LOCATION-CONFLICT` | Duplicate explicit `location` values. | Give each explicit shader location once. |
| `AIGPU-MESH-DATA-MISALIGNED` | Data byte length is not divisible by stride, or index bytes do not match format. | Repack data or pass an explicit `stride`. |
| `AIGPU-MESH-RANGE-INVALID` | Slice or draw-time range is negative, non-integer, outside parent counts, or uses index ranges on non-indexed geometries. | Clamp ranges and use indexed fields only with indexed geometries. |
| `AIGPU-MESH-WRITE-RANGE` | `write()` or `writeIndices()` would overflow the fixed buffer. | Create a larger geometry; writes do not resize buffers. |
| `AIGPU-MESH-ATTRIBUTE-UNMATCHED` | Named geometry attribute has no vertex-stage shader input. | Rename the attribute or specify `location`. |
| `AIGPU-MESH-INPUT-MISSING` | Shader declares an uncovered `@location` input. | Add the geometry attribute or remove the shader input. |
| `AIGPU-MESH-FORMAT-MISMATCH` | Vertex format base type does not match the WGSL input base type. | Use a compatible `GPUVertexFormat`; width differences are allowed by WebGPU. |

## Examples

```ts
import { init, geometry } from "aigpu/mock";
...
```ts
import { init, draw, geometry } from "aigpu/mock";
...
```ts
import { init, draw, geometry, target } from "aigpu/mock";
