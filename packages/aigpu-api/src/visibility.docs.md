...
```ts
import type { Visibility, VisibilityOptions, VisibilityQuery } from "aigpu";
```
...
- `AIGPU-VIS-CAPACITY-LIMIT` when `capacity` is not an integer in `[1, 4096]` (WebGPU `createQuerySet` caps `count` at 4096) — lower it, or create several visibility instances.
- `AIGPU-VIS-LABEL-DUPLICATE` when `vis.query()` receives a label that is already live — reuse the existing handle, or `dispose()` the old one first.
- `AIGPU-VIS-DISPOSED` for any use of a disposed handle or instance — create a fresh one with `vis.query(label)` / `visibility(gpu)`.
- `AIGPU-VIS-INVALID` for an empty or non-string label — label each queried object, e.g. `vis.query("statue")`.
- `AIGPU-VIS-INVALID` for mismatched plumbing: a non-`Visibility` pass option, a non-`VisibilityQuery` `occlusion()` argument, a handle from another instance, or an instance used with another gpu's frames — keep one instance per gpu and pass only its own handles.
- Pass-side errors (`AIGPU-VIS-NO-DEPTH`, `AIGPU-VIS-CAPACITY`, `AIGPU-QUERY-NO-VISIBILITY`, `AIGPU-QUERY-NESTED`, `AIGPU-QUERY-DUPLICATE`) are documented on `Frame`.

## Examples

```ts
import { init, draw, effect, frameLoop, target, visibility } from "aigpu/mock";
...
```ts
import { init, visibility } from "aigpu/mock";
...
- Results are **zero vs non-zero only**, mirroring WebGPU occlusion query semantics: a resolved value of `0` means no samples passed depth testing inside the scope; any non-zero value is unspecified. aigpu decodes that to `"hidden"` / `"visible"` and never exposes a sample count.
...
- The pass target needs a depth attachment: without depth testing every rasterized sample passes and each query would report `"visible"`, useless for culling (`AIGPU-VIS-NO-DEPTH`). MSAA targets and `depthReadOnly` passes both work — read-only depth still tests.
