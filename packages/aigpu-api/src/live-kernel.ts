...
/**
 * Kernel of `gpu`, or a thrown error when the gpu is disposed (`AIGPU-GPU-DISPOSED`) or was not
 * created by `init()` (`AIGPU-GPU-FOREIGN`, from `kernelOf`).
 *
 * @internal
 */
...
/** @internal */
export function gpuDisposedError(where: string): AIGpuError {
