...
export interface Visibility {
  /** Stable handle, created once outside the loop. Duplicate label → AIGPU-VIS-LABEL-DUPLICATE. */
...
export class InternalVisibility {
  ...
  constructor(device: Device, options: VisibilityOptions, frameCounter: () => number, host: QueryHostOptions = {}) {
    ...
    this.#ring = createQueryRing(device, { type: "occlusion", capacity, label: "aigpu.visibility", trackSettled: host.trackSettled, errorSink: host.errorSink });
  }
  ...
  beginQuery(query: VisibilityQuery, frame: unknown): number {
    ...
    // overwrites the earlier result in native WebGPU, so aigpu rejects it up front.
