...
**Throws:** `AIGPU-GPU-DISPOSED` when `clock(gpu)` runs after `gpu.dispose()` — read the clock before disposing, or `init()` a new gpu; `AIGPU-GPU-FOREIGN` when the argument was not created by `init()`; `AIGPU-CLOCK-DELTA-INVALID` when `advance()` receives a value that is not a finite, non-negative number — pass elapsed seconds, e.g. `advance(1 / 60)`.

## Examples

```ts
import { init, clock, effect, frameLoop, surface } from "aigpu";
...
- **See also:** `frame`, `frameLoop`, `Gpu`, and the guide *Driving aigpu with an external ticker*.
