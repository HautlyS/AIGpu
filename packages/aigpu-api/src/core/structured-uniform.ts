import { bind, Buffer, createBindGroup, createBindGroupLayout, ValidationError, type BindVisibility, type Device } from "@aigpu/core";
import {
  alignUniforms,
  isWgslUniformType,
  uniformValueCount,
...
function invalidUsage(where: string, message: string): ValidationError {
  return new ValidationError({ code: "AIGPU-CORE-INVALID-USAGE", message, where });
}
