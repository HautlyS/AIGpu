...
Low-level user-owned uniform buffer with a stable bind group at binding `0`. Prefer main API (`aigpu`) `set({ params: ... })` for ordinary values; use `Uniform` when you need byte-level writes or one buffer shared by many draws.

## Import

```ts
import { Uniform } from "aigpu/core";
import type { UniformOptions } from "aigpu/core";
```
...
declare class Uniform {
  ...
  readonly size: number;
  readonly buffer: import("aigpu/core").Buffer;
  readonly bindGroupLayout: GPUBindGroupLayout;
...
|---|---|---:|---|---|
| device | `Device` | ✔ | — | Core device, usually `gpu.device` from the main API (`aigpu`) `init()`. |
...
**Throws:** No main API (`aigpu`) `AIGPU-*` errors are thrown directly by `Uniform`; invalid sizes, incompatible reused layouts, out-of-range writes, or destroyed-buffer usage can surface as core/native WebGPU validation errors. Binding a `Uniform` through main API (`aigpu`) can still trigger `AIGPU-R1-OWNERSHIP-FLIP` if the same shader binding was first set with JS values.
...
- `Uniform` is user-owned from the first `draw.set({ name: uniform })`; aigpu binds its identity and never packs JS values into it.
