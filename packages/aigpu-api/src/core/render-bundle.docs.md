...
Low-level render bundle helper around `GPUDevice.createRenderBundleEncoder`. Prefer main API (`aigpu`) `bundle(gpu, { target }, cb)` when recording main API (`aigpu`) `Draw`/`Effect` commands because it derives formats from `Target` and performs stale-bundle checks (`AIGPU-R3-BUNDLE-STALE`).

## Import

```ts
import { createRenderBundle, RenderBundleRecorder } from "aigpu/core";
import type { RenderBundleOptions } from "aigpu/core";
```

## Signature

```ts
import type { Buffer } from "aigpu/core";
...
| opts.sampleCount | `number` | ✖ | WebGPU encoder default (`1`) | Must match replay pass sample count. main API (`aigpu`) passes `target.sampleCount`. |
...
**Throws:** No custom `AIGPU-*` errors are thrown here. Native WebGPU validation errors occur for incompatible formats, pipelines, bind groups, buffers, or draw parameters. main API (`aigpu`) stale errors (`AIGPU-R3-BUNDLE-STALE`, `AIGPU-R3-BUNDLE-INVALID`) are available only through `bundle(gpu)` / `FramePass.bundles()`.

## Examples

```ts
import { init } from "aigpu/mock";
import { createRenderBundle } from "aigpu/core";
...
```ts
import { init, target } from "aigpu/mock";
import { createRenderBundle } from "aigpu/core";
...
- This helper intentionally does not know about main API (`aigpu`) `Draw`, `Effect`, or `Target`; you must supply formats and native commands yourself.
