import { AIGpuError, attachBindGroupLayoutMetadata, attachBindGroupMetadata, type Device } from "@aigpu/core";
import {
  copyDstUsage,
...
export class UniformPool {
  ...
  constructor(readonly device: Device, opts: UniformPoolOptions = {}) {
    ...
    this.gpu = device.gpu.createBuffer({ label: "aigpu UniformPool", size: this.capacityBytes, usage: uniformUsage | copyDstUsage });
  }
  ...
  #assertCanPush(layoutSize: number, stride: number): void {
    ...
    throw new AIGpuError({
      code: "AIGPU-UNIFORM-POOL-OVERFLOW",
  ...
  #assertLayoutFits(layout: UniformLayout<unknown>): void {
    ...
    throw new AIGpuError({
      code: "AIGPU-UNIFORM-LAYOUT-OVERSIZED",
