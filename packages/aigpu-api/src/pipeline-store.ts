import { bindGroupLayoutMetadata, type Device } from "@aigpu/core";
import type { EntryPointInfo, OverrideInfo } from "@aigpu/wgsl/reflect-source";
import type { Target, CompileTarget, TargetSignature } from "./target.ts";
import { isTarget } from "./target-utils.ts";
import { compileDisposedError, compileFailedError, compileSignatureInvalidError, constantsInvalidError, entryInvalidError, pipelineLayoutGapError, type AIGpuError } from "./errors.ts";
...
export type ErrorSink = (error: AIGpuError) => void | Promise<void>;
