...
// The creation API: named exports of `aigpu`, `aigpu/node` and `aigpu/mock`, all gpu-first.
...
| geometry.input | `GeometryOptions \\| GeometryRecipe` | ✔ | — | A raw buffer descriptor, or a `aigpu/scene` recipe such as `box()` or `plane()`. |
...
| storage.bytes | `number` | ✔ | — | Byte size for a main API (`aigpu`) storage buffer. |
...
| onError.cb | `GpuErrorListener` | ✔ | — | Receives asynchronous aigpu errors; returns an unsubscribe function. |
...
**Throws:** `AIGPU-GPU-DISPOSED` when any factory (or `clock(gpu)`) runs after `gpu.dispose()` — the device and everything it owned are gone, so the handle it would return could only fail later; create resources before disposing, or `init()` a new gpu; `AIGPU-GPU-FOREIGN` when the first argument was not created by `init()` (a plain object, a `GPUDevice`, a gpu from another library): it carries no aigpu kernel, so pass the object returned by `init()` from `aigpu`, `aigpu/node` or `aigpu/mock`; `AIGPU-LIMIT-STORAGE-VERTEX` / `AIGPU-LIMIT-STORAGE-FRAGMENT` when a selected render entry exceeds its granted storage-buffer limit. The structured detail reports `stage`, `entryPoint`, `count`, `limit`, and each counted binding's `name`, `group`, and `binding`; request a supported limit or reduce/move the data; `AIGPU-SHADER-SOURCE-INVALID` for malformed `ShaderSource`; `AIGPU-SET-TEXTURE-FILTERABILITY` when a known facade texture format cannot satisfy an ordinarily sampled float binding (detail reports format, texture binding/name/label, and paired sampler identity); `AIGPU-RING1-UNSUPPORTED` for unsupported effect/compute/target cases; `AIGPU-TARGET-REQUIRED` when one-shot drawing needs an explicit target; `AIGPU-TARGET-SIZE-REQUIRED` for runtime JS calls to `target(gpu)` without `size`; `AIGPU-SURFACE-*` errors from `surface()`, surface resize, surface readback, or using disposed surfaces; plus method-specific `AIGPU-R1-*`, `AIGPU-R3-*`, and `AIGPU-R4-*` errors documented on `Effect`, `Draw`, `Compute`, `Frame`, `Bundle`, `Target`, and `SharedUniforms`.
...
`gpu.onError(cb)` subscribes to asynchronous aigpu errors and returns an unsubscribe function. Listeners run in subscription order; removing one stops future deliveries; a throwing listener is reported to `console.error` without stopping the rest. If no listener is registered, aigpu reports the error to `console.error` by default.
...
- Every factory rejects a disposed gpu with `AIGPU-GPU-DISPOSED`, and an object aigpu did not create with `AIGPU-GPU-FOREIGN`. Both are thrown synchronously, from the call that made the mistake.
...
aigpu infers sampled-texture layouts per selected WGSL entry point. A non-multisampled `texture_*<f32>` used by `textureSample*` or `textureGather*` with an ordinary sampler receives WebGPU `sampleType: "float"`; a texture used only by `textureLoad` remains `"unfilterable-float"`. Calls through helper functions are included.
