...
test("invalid depth options fail at draw construction", async () => {
  ...
  const expectInvalid = (label: string, depth: unknown): void => {
    expect(() => draw(gpu, { shader: DRAW_SHADER, label, depth: depth as never })).toThrowError(/AIGPU-DEPTH-INVALID|Invalid depth/);
  };
...
test("depth bias outside the i32 range fails at draw construction", async () => {
  ...
  const expectInvalid = (label: string, bias: number): void => {
    expect(() => draw(gpu, { shader: DRAW_SHADER, label, depth: { bias } })).toThrowError(/AIGPU-DEPTH-INVALID|Invalid depth/);
  };
...
test("nonzero depth bias is rejected for non-triangle topologies", async () => {
  ...
  const expectInvalid = (label: string, topology: GPUPrimitiveTopology, depth: unknown): void => {
    expect(() => draw(gpu, { shader: DRAW_SHADER, label, geometry: { topology }, depth: depth as never })).toThrowError(/AIGPU-DEPTH-INVALID|Invalid depth/);
  };
...
test("frame.pass validates clearDepth range and rejects it with clear false", async () => {
  const gpu = await init();
  const colorTarget = target(gpu, { size: [2, 2], depth: true });
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearDepth: 2 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARDEPTH-INVALID|clearDepth/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearDepth: -0.5 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARDEPTH-INVALID|clearDepth/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearDepth: Number.NaN }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARDEPTH-INVALID|clearDepth/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clear: false, clearDepth: 0 }, () => undefined))).toThrowError(/AIGPU-PASS-PRESERVE-CLEARDEPTH|preserves depth/);
  gpu.dispose();
});
...
test("stencil-only depth formats are rejected at target creation", async () => {
  const gpu = await init();
  expect(() => target(gpu, { size: [2, 2], depth: "stencil8" })).toThrowError(/AIGPU-TARGET-DEPTH-STENCIL-ONLY|stencil-only/);
  gpu.dispose();
});
