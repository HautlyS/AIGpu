...
test("immediate onResize callback also guards frame reentrancy", async () => {
  ...
  canvasSurface.onResize(() => {
    checked = true;
    expect(() => frame(gpu)).toThrowError(/AIGPU-FRAME-REENTRANT|Nested frame/);
  });
...
test("buffer-only surfaces default autoResize false, reject explicit autoResize true, and do not grow with dpr", async () => {
  ...
  expect(() => surface(gpu, canvasLike(1, 1, false), { autoResize: true })).toThrowError(/AIGPU-SURFACE-AUTORESIZE-UNSUPPORTED|autoResize needs/);
  gpu.dispose();
});
...
test("surface lifecycle rejects duplicates, disposes, unregisters, and allows re-creation", async () => {
  ...
  expect(() => surface(gpu, canvas)).toThrowError(/AIGPU-SURFACE-DUPLICATE|already has surface/);
  canvasSurface.dispose();
  expect(contextOf(canvas).unconfigure).toHaveBeenCalledTimes(1);
  expect(() => canvasSurface.resize([1, 1])).toThrowError(/AIGPU-SURFACE-DISPOSED|disposed/);
  expect(() => canvasSurface.onResize(() => undefined)).toThrowError(/AIGPU-SURFACE-DISPOSED|disposed/);
...
test("resize and frame reentrancy are guarded, but resizing another surface and creating resources is allowed", async () => {
  ...
  a.onResize(() => { expect(() => a.resize([20, 20])).toThrowError(/AIGPU-SURFACE-RESIZE-REENTRANT|Cannot resize/); });
  a.onResize(() => { expect(() => frame(gpu)).toThrowError(/AIGPU-FRAME-REENTRANT|Nested frame/); });
...
test("target is required for frame and one-shot draws, and target size is required at runtime", async () => {
  ...
  }).toThrowError(/AIGPU-TARGET-REQUIRED|Target required/);
  expect(() => shader1.draw()).toThrowError(/AIGPU-TARGET-REQUIRED|Target required/);
  expect(() => drawable.draw()).toThrowError(/AIGPU-TARGET-REQUIRED|Target required/);
  expect(() => {
    // @ts-expect-error target(gpu, opts) requires size; this asserts the runtime JS error.
    target(gpu, {});
  }).toThrowError(/AIGPU-TARGET-SIZE-REQUIRED|Target size required/);
...
test("surface bundle survives resize, and re-recording from onResize is usable in the same frame", async () => {
  ...
  canvasSurface.dispose();
  expect(() => frame(gpu, (f) => f.pass({ target: canvasSurface }, (p) => p.bundles(recorded)))).toThrowError(/AIGPU-SURFACE-DISPOSED|disposed/);
  gpu.dispose();
});
