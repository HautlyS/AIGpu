...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("rgba16float target readback keeps HDR values", async () => {
  ...
  try {
    ...
    // read() stays raw bytes and is now 8 bytes per texel instead of throwing AIGPU-CORE-UNSUPPORTED-FORMAT.
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("rgba32float target readback is exact", async () => {
  const gpu = await init();
  try {
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("r32float target reads back one component per texel across padded rows", async () => {
  const gpu = await init();
  try {
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("rgba8unorm readback is unchanged and readFloats normalizes it", async () => {
  const gpu = await init();
  try {
