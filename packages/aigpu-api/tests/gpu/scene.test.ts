...
describe.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("aigpu/scene Docker GPU acceptance", () => {
  test("lit cube resolves std light imports and Lambert responds to light direction", async () => {
    ...
    try {
      ...
      expect(shader.deps.some((dep) => dep.endsWith("node_modules/@aigpu/wgsl-std/src/light/index.wgsl"))).toBe(true);
...
async function mkdirTemp(): Promise<string> {
  ...
  return mkdtemp(join(tmpdir(), "aigpu-scene-"));
}
