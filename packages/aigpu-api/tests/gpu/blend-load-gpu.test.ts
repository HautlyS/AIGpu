...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("additive blend accumulates repeated fullscreen draws", async () => {
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("writeMask can preserve alpha while writing rgb", async () => {
  const gpu = await init();
  try {
    const colorTarget = target(gpu, { size: [2, 2], format: "rgba8unorm" });
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("MSAA target resolves additive blend", async () => {
  const gpu = await init();
  try {
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("clear false preserves offscreen target contents across passes and frames", async () => {
  const gpu = await init();
  try {
