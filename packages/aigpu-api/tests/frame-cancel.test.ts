...
test("cancel() after submit() throws AIGPU-FRAME-SUBMITTED", async () => {
  ...
  catch (error) { expect(error).toMatchObject({ code: "AIGPU-FRAME-SUBMITTED", where: "Frame.cancel" }); }
  await gpu.settled();
  gpu.dispose();
});

test("pass() after cancel() throws AIGPU-FRAME-CANCELED", async () => {
  const gpu = await init();
  const colorTarget = target(gpu, { size: [4, 4] });
  ...
  catch (error) { expect(error).toMatchObject({ code: "AIGPU-FRAME-CANCELED", where: "Frame.pass" }); }
  gpu.dispose();
});
...
test("cancel() rejects while a pass callback is active and keeps descriptor resources retained", async () => {
  ...
  currentFrame.pass({ target: scene, visibility: vis }, () => {
    ...
    try { currentFrame.cancel(); }
    catch (error) { expect(error).toMatchObject({ code: "AIGPU-FRAME-PASS-ACTIVE", where: "Frame.cancel" }); }
...
/** Captures resolve/copy/finish ordering on aigpu.frame encoders, matching pass-telemetry-rollback.test.ts. */
