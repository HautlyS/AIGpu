...
import { reflectSource, type EntryPointInfo, type Reflection } from "@aigpu/wgsl/reflect-source";
import { AIGpuError } from "../src/errors.ts";
...
test("visibilityForEntries fails loudly when an entry point has no reflected bindings", () => {
  ...
  expect(() => visibilityForEntries([], [stripped])).toThrowError(AIGpuError);
  expect(() => visibilityForEntries([], [stripped])).toThrowError(/has no reflected bindings/);
  try {
    visibilityForEntries([], [stripped]);
  } catch (error) {
    expect((error as AIGpuError).code).toBe("AIGPU-REFLECT-ENTRY-METADATA-MISSING");
  }
});
