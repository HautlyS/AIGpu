...
test("a duplicate span name within one frame throws at pass time", async () => {
  ...
  expect(() => frame(gpu, (currentFrame) => {
    ...
    currentFrame.pass({ target: colorTarget, timer: gpuTimer1.span("main") }, () => undefined);
  })).toThrowError(/AIGPU-TIMER-INVALID|duplicate span 'main'/);
...
test("a span from another gpu's timer is rejected with a clear error", async () => {
  ...
  expect(() => frame(gpuB, (currentFrame) => currentFrame.pass({ target: colorTarget, timer: timerA.span("main") }, () => undefined)))
    .toThrowError(/AIGPU-TIMER-INVALID|different gpu/);
...
test("non-TimerSpan timer options fail at pass open", async () => {
  ...
  for (const value of ["shadows", 1, {}, { name: "shadows" }, null]) {
    expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, timer: value as never }, () => undefined)))
      .toThrowError(/AIGPU-TIMER-INVALID|expected a TimerSpan/);
  }
  gpu.dispose();
});
...
test("dispose() destroys the query set and later use throws", async () => {
  ...
  expect(() => gpuTimer1.span("main")).toThrowError(/AIGPU-TIMER-INVALID|disposed/);
  expect(() => gpuTimer1.onResults(() => undefined)).toThrowError(/AIGPU-TIMER-INVALID|disposed/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, timer: span }, () => undefined))).toThrowError(/AIGPU-TIMER-INVALID|disposed/);
...
test("exceeding 2048 spans in one frame throws AIGPU-TIMER-CAPACITY", async () => {
  const gpu = await initWithTimestampQuery();
  const gpuTimer1 = timer(gpu);
  const colorTarget = target(gpu, { size: [4, 4] });
  ...
  })).toThrowError(/AIGPU-TIMER-CAPACITY|exceeds 2048 timed spans/);
...
test("invalid span names fail at span()", async () => {
  ...
  for (const value of ["", 1, null, undefined, {}]) {
    expect(() => gpuTimer1.span(value as never)).toThrowError(/AIGPU-TIMER-INVALID|non-empty string/);
  }
  gpuTimer1.dispose();
  gpu.dispose();
});
...
test("a readback that fails is reported on gpu.onError as AIGPU-QUERY-READBACK", async () => {
  const gpu = await initWithTimestampQuery();
  ...
  expect(errors[0]).toMatchObject({ code: "AIGPU-QUERY-READBACK", message: expect.stringContaining("aigpu.timer") });
  gpuTimer1.dispose();
  gpu.dispose();
  vi.restoreAllMocks();
});
...
test("timer(gpu) after gpu.dispose() throws AIGPU-GPU-DISPOSED instead of building on a dead device", async () => {
  const gpu = await initWithTimestampQuery();
  gpu.dispose();
  expect(thrownBy(() => timer(gpu))).toMatchObject({
    code: "AIGPU-GPU-DISPOSED",
    where: "timer",
...
test("timer(gpu) rejects an object this library did not create", () => {
  expect(thrownBy(() => timer({ disposed: false } as never))).toMatchObject({ code: "AIGPU-GPU-FOREIGN" });
});
