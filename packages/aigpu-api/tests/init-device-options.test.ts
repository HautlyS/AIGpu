...
import { createMockGPUDevice, Device, type CreateDeviceOptions, type AIGpuAdapter } from "@aigpu/core";
...
test("explicit adapters receive required features and limits unchanged", async () => {
  ...
  const adapter: AIGpuAdapter = { requestDevice };
...
test("mock init requesting an undeclared feature fails with AIGPU-FEATURE-UNSUPPORTED", async () => {
