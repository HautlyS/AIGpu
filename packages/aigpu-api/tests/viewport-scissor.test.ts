...
test("invalid viewport shapes and values fail at pass open", async () => {
  ...
  const expectInvalid = (viewport: unknown): void => {
    expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, viewport: viewport as never }, () => undefined))).toThrowError(/AIGPU-PASS-VIEWPORT-INVALID|Invalid viewport/);
  };
  expectInvalid("full");
...
test("invalid scissor shapes and values fail at pass open", async () => {
  ...
  const expectInvalid = (scissor: unknown): void => {
    expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, scissor: scissor as never }, () => undefined))).toThrowError(/AIGPU-PASS-SCISSOR-INVALID|Invalid scissor/);
  };
  expectInvalid("full");
...
test("out-of-bounds scissor errors report the target's current pixel size", async () => {
  ...
  const expectOutOfBounds = (scissor: readonly [number, number, number, number]): void => {
    expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, scissor }, () => undefined))).toThrowError(/AIGPU-PASS-SCISSOR-INVALID.*4x2px|4x2px/);
  };
