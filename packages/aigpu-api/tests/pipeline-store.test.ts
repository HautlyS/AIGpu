...
test("disposing the store rejects pending async compiles with AIGPU-COMPILE-DISPOSED", async () => {
  const gpu = await init();
  const colorTarget = target(gpu, { size: [2, 2] });
  const store = createPipelineStore(gpu.device);
  const modules = createShaderModuleCache(gpu.device);
  ...
  await expect(pending).rejects.toMatchObject({ code: "AIGPU-COMPILE-DISPOSED" });
  modules.dispose();
  gpu.dispose();
});
