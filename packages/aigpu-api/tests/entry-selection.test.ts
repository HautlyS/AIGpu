...
test("entry selects the named vertex entry point; its inputs feed the geometry layout resolver", async () => {
  ...
  expect(() => draw(gpu, { shader: TWO_VERTEX_WGSL, label: "default-vertex", geometry: geo })).toThrow(expect.objectContaining({ code: "AIGPU-MESH-ATTRIBUTE-UNMATCHED" }));
...
test("storage-stage limits validate against the selected vertex entry point", async () => {
  ...
  expect(() => draw(gpu, { shader: VERTEX_STORAGE_WGSL, label: "limit-storage", entry: { vertex: "vs_storage" } })).toThrow(expect.objectContaining({
    code: "AIGPU-LIMIT-STORAGE-VERTEX",
...
test("unknown and wrong-stage entry names fail at construction listing the available entry points", async () => {
  const gpu = await init();
  expect(() => draw(gpu, { shader: TWO_FRAGMENT_WGSL, label: "unknown", entry: { fragment: "fs_c" } })).toThrowError(/AIGPU-ENTRY-INVALID|"vs_main" \(@vertex\), "fs_a" \(@fragment\), "fs_b" \(@fragment\)/);
  expect(() => draw(gpu, { shader: TWO_FRAGMENT_WGSL, label: "wrong-stage", entry: { vertex: "fs_a" } })).toThrowError(/AIGPU-ENTRY-INVALID|is a @fragment entry point, not @vertex/);
  expect(() => draw(gpu, { shader: TWO_FRAGMENT_WGSL, label: "not-object", entry: "fs_b" as never })).toThrowError(/AIGPU-ENTRY-INVALID|expected \{ vertex\?, fragment\? \}/);
  expect(() => draw(gpu, { shader: TWO_FRAGMENT_WGSL, label: "not-string", entry: { fragment: 2 as never } })).toThrowError(/AIGPU-ENTRY-INVALID|expected an entry point name string/);
  gpu.dispose();
});
...
test("compute entry validates at construction with where compute", async () => {
  const gpu = await init();
  expect(() => compute(gpu, TWO_COMPUTE_WGSL, { label: "unknown", entry: "cs_c" })).toThrow(expect.objectContaining({ code: "AIGPU-ENTRY-INVALID", where: "compute" }));
  expect(() => compute(gpu, TWO_COMPUTE_WGSL, { label: "unknown-list", entry: "cs_c" })).toThrowError(/"cs_a" \(@compute\), "cs_b" \(@compute\)/);
  expect(() => compute(gpu, MIXED_STAGE_COMPUTE_WGSL, { label: "wrong-stage", entry: "vs_main" })).toThrowError(/AIGPU-ENTRY-INVALID|is a @vertex entry point, not @compute/);
  expect(() => compute(gpu, TWO_COMPUTE_WGSL, { label: "not-string", entry: 1 as never })).toThrowError(/AIGPU-ENTRY-INVALID|expected an entry point name string/);
  gpu.dispose();
});
