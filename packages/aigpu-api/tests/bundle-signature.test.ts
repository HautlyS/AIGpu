...
test("bundle(gpu, ...) validates malformed signatures at record time", async () => {
  ...
  expect(() => bundle(gpu, { target: { colors: [] }, label: "badSignature" }, (b) => b.draw(shader))).toThrowError(/AIGPU-COMPILE-SIGNATURE-INVALID|colors/);
  gpu.dispose();
});
...
test("signature bundle recording still requires draw resources to be set", async () => {
  ...
  expect(() => bundle(gpu, { target: { colors: ["rgba8unorm"] }, label: "unsetTextureBundle" }, (b) => b.draw(post))).toThrowError(/AIGPU-R1-BINDING-NEVER-SET|Unset/);
  gpu.dispose();
});
