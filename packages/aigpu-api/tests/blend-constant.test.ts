...
test("invalid blendConstant shapes fail at draw construction", async () => {
  ...
  const expectInvalid = (label: string, blendConstant: unknown): void => {
    expect(() => draw(gpu, { shader: DRAW_SHADER, label, blend: CONSTANT_BLEND, blendConstant: blendConstant as never })).toThrowError(/AIGPU-BLEND-CONSTANT-INVALID|Invalid blendConstant/);
  };
...
test("blendConstant without a constant blend factor fails at draw construction", async () => {
  ...
  const expectDead = (label: string, opts: object): void => {
    expect(() => draw(gpu, { shader: DRAW_SHADER, label, blendConstant: [0, 0, 0, 1], ...opts })).toThrowError(/AIGPU-BLEND-CONSTANT-INVALID|no effect/);
  };
...
test("bundles reject recording draws with blendConstant", async () => {
  ...
  expect(() => bundle(gpu, { target: colorTarget, label: "constantBundle" }, (b) => b.draw(constant))).toThrowError(/AIGPU-BUNDLE-BLEND-CONSTANT|blendConstant/);
