...
function expectOutsideFrame(fn: () => unknown): void {
  ...
  throw new Error("Expected AIGPU-SURFACE-NOT-IN-FRAME");
}
...
test("compile validates signatures and missing default targets synchronously", async () => {
  ...
  expect(() => drawable.compile()).toThrowError(/AIGPU-TARGET-REQUIRED|Target required/);
  expect(() => drawable.compileSync()).toThrowError(/AIGPU-TARGET-REQUIRED|Target required/);
  expect(() => drawable.compile({ colors: [] })).toThrowError(/AIGPU-COMPILE-SIGNATURE-INVALID|colors/);
  expect(() => drawable.compileSync({ colors: ["rgba8unorm"], sampleCount: 2 as never })).toThrowError(/AIGPU-COMPILE-SIGNATURE-INVALID|sampleCount/);
  expect(() => drawable.compileSync({ colors: ["rgba8unorm"], depth: { format: "depth24plus" } as never })).toThrowError(/AIGPU-COMPILE-SIGNATURE-INVALID|depth/);
  expect(() => drawable.compile({ colors: "rgba8unorm" } as never)).toThrowError(/AIGPU-COMPILE-SIGNATURE-INVALID|colors/);
  expect(() => drawable.compile("rgba8unorm" as never)).toThrowError(/AIGPU-COMPILE-SIGNATURE-INVALID|colors/);
  gpu.dispose();
});
