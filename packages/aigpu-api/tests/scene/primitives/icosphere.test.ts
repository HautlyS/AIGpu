...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("icosphere primitive snapshot battery matches", async () => {
...
function expectInvalid(fn: () => unknown, message?: RegExp): void { try { fn(); throw new Error("Expected AIGPU-CORE-INVALID-USAGE"); } catch (error) { expect(error).toMatchObject({ code: "AIGPU-CORE-INVALID-USAGE" }); if (message) expect(error).toMatchObject({ message: expect.stringMatching(message) }); } }
