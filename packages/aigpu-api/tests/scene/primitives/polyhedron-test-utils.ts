...
function expectInvalid(fn: () => unknown): void { try { fn(); throw new Error("Expected AIGPU-CORE-INVALID-USAGE"); } catch (error) { expect(error).toMatchObject({ code: "AIGPU-CORE-INVALID-USAGE" }); } }
