...
describe("stateful scene cameras", () => {
  ...
  test("projection parameters are validated", () => {
    expect(errorCode(() => perspectiveCamera({ fov: 0, position: [0, 0, 5] }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => perspectiveCamera({ fov: 45, near: 1, far: 0.5 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    const camera = perspectiveCamera({ fov: 45 });
    expect(errorCode(() => camera.set({ fov: 200 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
  });
