...
describe("scene materials", () => {
  ...
  test("color material validates inputs", () => {
    expect(errorCode(() => unlitMaterial({ color: [1, 1] as never }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => unlitMaterial().set({ opacity: 2 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
  });
