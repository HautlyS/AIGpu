...
describe("scene tree nodes", () => {
  ...
  test("add() rejects cycles with AIGPU-SCENE-CYCLE", () => {
    const root = scene();
    const middle = group();
    const leaf = group();
    root.add(middle);
    ...
    expect(errorCode(() => leaf.add(root))).toBe("AIGPU-SCENE-CYCLE");
    expect(errorCode(() => leaf.add(leaf))).toBe("AIGPU-SCENE-CYCLE");
  });

  test("set() validates vector lengths with AIGPU-SCENE-VALUE-INVALID", () => {
    const node = group();
    expect(errorCode(() => node.set({ position: [1, 2] as never }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => node.set({ quaternion: [0, 0, 1] as never }))).toBe("AIGPU-SCENE-VALUE-INVALID");
  });
