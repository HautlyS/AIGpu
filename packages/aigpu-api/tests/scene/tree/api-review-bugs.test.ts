...
describe("api-review §1.2 — a NaN deltaTime must not poison the easing", () => {
  ...
  test("set() rejects non-finite yaw/pitch/distance/target", () => {
    ...
    expect(errorCode(() => controls.set({ yaw: Number.NaN }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => controls.set({ pitch: Number.NaN }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => controls.set({ distance: Number.POSITIVE_INFINITY }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => controls.set({ target: [Number.NaN, 0, 0] }))).toBe("AIGPU-SCENE-VALUE-INVALID");
...
describe("api-review §1.3 — constructors validate before super() reparents children", () => {
  ...
  test("perspectiveCamera({ fov: 0 }) does not strand its children", () => {
    ...
    expect(code).toBe("AIGPU-SCENE-VALUE-INVALID");
  ...
  test("directionalLight with a bad intensity/direction does not strand its children", () => {
    ...
    expect(intensity.code).toBe("AIGPU-SCENE-VALUE-INVALID");
    ...
    expect(direction.code).toBe("AIGPU-SCENE-VALUE-INVALID");
...
describe("api-review §1.6 — degenerate aspect / ortho extents", () => {
  test("perspectiveCamera rejects a non-positive or non-finite aspect", () => {
    expect(errorCode(() => perspectiveCamera({ fov: 45, aspect: 0 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => perspectiveCamera({ fov: 45, aspect: -1 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => perspectiveCamera({ fov: 45, aspect: Number.NaN }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => perspectiveCamera({ fov: 45 }).set({ aspect: 0 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => perspectiveCamera({ fov: 45 }).set({ aspect: Number.POSITIVE_INFINITY }))).toBe("AIGPU-SCENE-VALUE-INVALID");
  });
  ...
  test("a rejected aspect leaves the previous projection intact", () => {
    ...
    expect(errorCode(() => camera.set({ aspect: 0 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(camera.aspect).toBe(2);
    expect(Array.from(camera.projection)).toEqual(before);
  });

  test("orthographicCamera rejects empty extents", () => {
    expect(errorCode(() => orthographicCamera({ left: 1, right: 1, bottom: -1, top: 1 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => orthographicCamera({ left: -1, right: 1, bottom: 1, top: 1 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(errorCode(() => orthographicCamera({ left: -1, right: Number.NaN, bottom: -1, top: 1 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
  });

  test("orthographicCamera.set() validates the resulting extents", () => {
    const camera = orthographicCamera({ left: -1, right: 1, bottom: -1, top: 1 });
    expect(errorCode(() => camera.set({ right: -1 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
    expect(camera.right).toBe(1);
    expect(errorCode(() => camera.set({ top: -1 }))).toBe("AIGPU-SCENE-VALUE-INVALID");
