...
import { getMockGPUDeviceInstrumentation, init, AIGpuError, geometry } from "../../src/mock.ts";
import { geometry as geometryOf } from "../../src/scene/geometry-descriptor.ts";

function meshErrorOf(fn: () => unknown): AIGpuError {
  try { fn(); } catch (error) { if (error instanceof AIGpuError) return error; throw error; }
  throw new Error("Expected a AIGpuError");
}
...
test("geometry(gpu, ...) derives tight auto stride, stepMode instance count, and rejects invalid layouts", async () => {
  ...
  try {
    ...
    expect(() => geometry(gpu, { buffers: [{ data: new Float32Array([1, 2, 3, 4]), attributes: { a: { format: "float32x2", location: 0 }, b: { format: "float32x2", location: 0 } } }] }))
      .toThrowError(/AIGPU-MESH-LOCATION-CONFLICT/);
  } finally {
    gpu.dispose();
  }
});
...
test("geometry(gpu, ...) validates locations, enums, and tight auto-stride eagerly", async () => {
  ...
  try {
    ...
    for (const location of [-1, 1.5, Number.NaN, gpu.device.gpu.limits.maxVertexAttributes]) {
      expect(() => geometry(gpu, { buffers: [{ ...base, attributes: { value: { format: "float32", location } } }] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    }
    expect(() => geometry(gpu, { topology: "bogus" as GPUPrimitiveTopology, buffers: [base] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    expect(() => geometry(gpu, { buffers: [{ ...base, stepMode: "bogus" as GPUVertexStepMode }] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    for (const format of ["float16", "float16x3", "unorm8x3", "uint8x3"] as GPUVertexFormat[]) {
      expect(() => geometry(gpu, { buffers: [{ data: new Uint8Array(12), attributes: { value: { format, location: 0 } } }] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    }
    expect(() => geometry(gpu, { buffers: [{ data: new Float32Array(5), attributes: { value: { format: "float32", offset: 16, location: 0 } } }] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
...
test("caller-owned buffers require explicit counts and a complete index trio", async () => {
  ...
  try {
    ...
    expect(() => geometry(gpu, { buffers: [raw] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    ...
    expect(() => geometry(gpu, { buffers: [{ ...hybridRaw, stepMode: "instance" }] })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    expect(() => geometry(gpu, { buffers: [raw], vertexCount: 3, indexBuffer: index })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    expect(() => geometry(gpu, { buffers: [raw], vertexCount: 3, indexBuffer: index, indexFormat: "uint16" })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
...
test("owned counts cannot exceed data capacity and geometry layout properties are immutable", async () => {
  ...
  try {
    ...
    expect(() => geometry(gpu, { ...descriptor, vertexCount: 2 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => geometry(gpu, { ...descriptor, indices: new Uint16Array([0, 0]), indexCount: 3 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => geometry(gpu, { ...descriptor, indices: new Uint16Array([0, 0]), indexFormat: "uint32" })).toThrowError(/AIGPU-MESH-LAYOUT-INVALID/);
    ...
    expect(() => geometry(gpu, { buffers: planar, vertexCount: 2 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    ...
    expect(() => geometry(gpu, { buffers: instances, instanceCount: 2 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
...
test("geometry writes enforce WebGPU alignment and stay structured after destroy", async () => {
  ...
  try {
    ...
    expect(() => geo.write(new Uint8Array(2))).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
    expect(() => geo.write(new Uint32Array([1]), 2)).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
    expect(() => geo.writeIndices(new Uint16Array([0]), 0)).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
    geo.destroy();
    expect(() => geo.write(new Uint32Array([1]))).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
    expect(() => geo.writeIndices(new Uint32Array([1]))).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
  } finally {
    gpu.dispose();
  }
});
...
test("geometry construction-time error codes include actionable fix hints", async () => {
  ...
  try {
    ...
    expect(errors.map((error) => error.code)).toEqual([
      "AIGPU-MESH-LAYOUT-INVALID",
      "AIGPU-MESH-LIMIT-EXCEEDED",
      "AIGPU-MESH-LOCATION-CONFLICT",
      "AIGPU-MESH-DATA-MISALIGNED",
      "AIGPU-MESH-RANGE-INVALID",
      "AIGPU-MESH-WRITE-RANGE",
    ]);
...
test("geometry writes are range checked and slices validate indexed/non-indexed direction eagerly", async () => {
  ...
  try {
    ...
    expect(() => geo.write(new Float32Array([1]), 16)).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
    expect(() => geo.slice({ firstIndex: 0, indexCount: 1 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => geo.slice({ firstVertex: 1, vertexCount: 2 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);

    const indexed = geometry(gpu, { buffers: [{ data: new Float32Array([0, 0, 1, 1]), attributes: { pos: { format: "float32x2", location: 0 } } }], indices: [0, 1] });
    expect(() => indexed.slice({ firstVertex: 0, vertexCount: 1 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => indexed.slice({ firstIndex: 1, indexCount: 2 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => indexed.writeIndices(new Uint32Array([0]), 8)).toThrowError(/AIGPU-MESH-WRITE-RANGE/);
  } finally {
    gpu.dispose();
  }
});
...
test("geometry(gpu, descriptor) builds the same layout as the facade and owns its buffers for the gpu's lifetime", async () => {
  ...
  gpu.dispose();
  expect(meshErrorOf(() => mesh.write(vertices)).code).toBe("AIGPU-MESH-WRITE-RANGE");
});
...
test("geometry(gpu, descriptor) borrows caller-owned buffers instead of allocating or destroying them", async () => {
  ...
  // Writing a borrowed stream is rejected: aigpu never owned those bytes, so it never frees them either.
  expect(meshErrorOf(() => mesh.write(new Float32Array([1, 2]))).code).toBe("AIGPU-MESH-WRITE-RANGE");
...
test("geometry(gpu, descriptor) validates the gpu and keeps the mesh error codes", async () => {
  ...
  expect(meshErrorOf(() => geometryOf(gpu, { buffers: [{ data: new Float32Array([1, 2, 3, 4]), attributes: { pos: { format: "float32x3", location: 0 } } }] })).code)
    .toBe("AIGPU-MESH-DATA-MISALIGNED");
  expect(meshErrorOf(() => geometryOf(gpu, { topology: "bogus" as GPUPrimitiveTopology, buffers: [] })).code)
    .toBe("AIGPU-MESH-LAYOUT-INVALID");
  gpu.dispose();
  expect(meshErrorOf(() => geometryOf(gpu, { buffers: [] }))).toMatchObject({ code: "AIGPU-GPU-DISPOSED", where: "geometry" });
});
