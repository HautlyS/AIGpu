...
import { createMockGPUDevice, Device } from "@aigpu/core";
...
test("a failed readback is dropped but reported on the errorSink instead of being swallowed", async () => {
  ...
  expect(errors[0]).toMatchObject({ code: "AIGPU-QUERY-READBACK", message: expect.stringContaining("device lost") });
  expect(errors[0]!.message).toContain("aigpu.timer");
...
test("a post-map failure whose unmap succeeds reports the drop and keeps the staging buffer in rotation", async () => {
  ...
  expect(errors.map((error) => error.code)).toEqual(["AIGPU-QUERY-READBACK"]);
...
test("an unmap that fails after a successful decode drops that readback and retires the slot", async () => {
  ...
  expect(unmaps).toEqual([0]);
  expect(errors[0]).toMatchObject({ code: "AIGPU-QUERY-READBACK", message: expect.stringContaining("unmap failed on staging0") });
  expect(ring.encodeResolve(encoder, 2)).toBe(false);
  ring.dispose();
  vi.restoreAllMocks();
});
