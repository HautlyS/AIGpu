...
import { createMockGPUDevice, Device, AIGpuError } from "@aigpu/core";
...
test("throws when pushes exceed capacity", () => {
  ...
  expectInvalidCode(() => slot.push(2), "AIGPU-UNIFORM-POOL-OVERFLOW");
  device.destroy();
});
test("throws when a layout cannot fit in one binding", () => {
  ...
  expectInvalidCode(() => pool.alloc(bytesLayout(300)), "AIGPU-UNIFORM-LAYOUT-OVERSIZED");
  device.destroy();
});
...
test("rejects a slot allocated by a different pool", () => {
  ...
  expectInvalidCode(() => pool.push(slot, 1), "AIGPU-CORE-INVALID-USAGE");
  expectInvalidCode(() => pool.pushBytes(slot, new Float32Array([1])), "AIGPU-CORE-INVALID-USAGE");
  device.destroy();
});
...
test("assertReadyForSubmit throws when there are unflushed pushes", () => {
  ...
  expectInvalidCode(() => pool.assertReadyForSubmit("test"), "AIGPU-CORE-INVALID-USAGE");
  expect(() => pool.assertReadyForSubmit("test")).toThrow(/AIGPU-CORE-INVALID-USAGE|unflushed pushes/);
  device.destroy();
});
...
function expectInvalidCode(action: () => void, code: string): void {
  ...
  expect(thrown).toBeInstanceOf(AIGpuError);
