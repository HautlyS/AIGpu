...
test("invalid multisample options fail at draw construction", async () => {
  const gpu = await init();
  expect(() => draw(gpu, { shader: SOLID, label: "not-object", multisample: "msaa" as never })).toThrowError(/AIGPU-MULTISAMPLE-INVALID|expected \{ alphaToCoverage\?, mask\? \}/);
  expect(() => draw(gpu, { shader: SOLID, label: "array", multisample: [true] as never })).toThrowError(/AIGPU-MULTISAMPLE-INVALID|expected \{ alphaToCoverage\?, mask\? \}/);
  expect(() => draw(gpu, { shader: SOLID, label: "bad-a2c", multisample: { alphaToCoverage: "yes" } as never })).toThrowError(/AIGPU-MULTISAMPLE-INVALID|alphaToCoverage must be a boolean/);
  for (const mask of [1.5, -1, 0x1_0000_0000, Number.NaN, "3"]) {
    expect(() => draw(gpu, { shader: SOLID, label: "bad-mask", multisample: { mask } as never })).toThrowError(/AIGPU-MULTISAMPLE-INVALID|mask must be an integer/);
  }
  gpu.dispose();
});
...
test("alphaToCoverage requires an MSAA target signature", async () => {
  ...
  expect(() => drawable.draw(plain)).toThrowError(/AIGPU-MULTISAMPLE-INVALID|msaa: true/);
  expect(() => drawable.compileSync({ colors: ["rgba8unorm"] })).toThrowError(/AIGPU-MULTISAMPLE-INVALID|msaa: true/);
  ...
  expect(() => draw(gpu, { shader: SOLID, label: "eager-needs-msaa", targets: [plain], multisample: { alphaToCoverage: true } })).toThrowError(/AIGPU-MULTISAMPLE-INVALID|msaa: true/);
  gpu.dispose();
});
