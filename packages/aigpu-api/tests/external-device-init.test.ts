...
import { createMockGPUDevice, getMockGPUDeviceInstrumentation } from "@aigpu/core";
...
test("browser external init preserves exact identity and bypasses adapter resolution", async () => {
  ...
  expect(() => compute(gpu, "@compute @workgroup_size(1) fn main() {}")).toThrow(expect.objectContaining({ code: "AIGPU-GPU-DISPOSED" }));
...
test("invalid external device shape has stable error code", async () => {
  await expect(initFromDevice({} as GPUDevice)).rejects.toMatchObject({ code: "AIGPU-INIT-DEVICE-INVALID" });
});
...
test("loss observed during external init rejects without native destruction", async () => {
  ...
  await expect(initFromDevice(device)).rejects.toMatchObject({ code: "AIGPU-DEVICE-LOST", message: expect.stringContaining("lost during init") });
  expect(device.destroy).not.toHaveBeenCalled();
});
...
test("disposing a wrapped buffer evicts Ring-1 cache identity and rejects later set", async () => {
  ...
  first.dispose();
  expect(() => pipeline.set({ source: first })).toThrow(expect.objectContaining({ code: "AIGPU-BUFFER-DISPOSED" }));
...
test("retained compute and uniform-like bindings respect logical disposal", async () => {
  ...
  wrapped.dispose();
  expect(() => set.set({ u: uniformLike })).toThrow(expect.objectContaining({ code: "AIGPU-BUFFER-DISPOSED" }));
  gpu.dispose();
  // Retained object, already built: our own guard reports the device it can no longer reach.
  expect(() => dispatch.dispatch(1)).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-DISPOSED" }));
});
...
test("retained draw and effect operations respect logical disposal", async () => {
  ...
  gpu.dispose();
  expect(() => fullscreen.set({})).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-DISPOSED" }));
  expect(() => triangle.set({})).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-DISPOSED" }));
});
...
test("an explicit submit still reports a device the owner killed", async () => {
  ...
  expect(() => pending.submit()).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-LOST" }));
  gpu.dispose();
});
...
test("retained compute reports device loss reason", async () => {
  ...
  await Promise.resolve();
  expect(() => pipeline.dispatch(1)).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-LOST", message: expect.stringContaining("runtime lost") }));
  gpu.dispose();
});

test("hostile plain-JS devices use stable validation codes", async () => {
  for (const bad of [null, undefined, 42, "device", {}, { queue: {} }]) {
    await expect(initFromDevice(bad as never)).rejects.toMatchObject({ code: "AIGPU-INIT-DEVICE-INVALID" });
  }
});
...
test("shared uniforms reject a disposed backing buffer", async () => {
  ...
  const buffer = (shared as unknown as { buffer: import("@aigpu/core").Buffer }).buffer;
  buffer.dispose();
  expect(() => pipeline.set({ u: shared })).toThrow(expect.objectContaining({ code: "AIGPU-BUFFER-DISPOSED" }));
  gpu.dispose();
});
...
test("frame state rejects reads and advances after loss", async () => {
  ...
  expect(() => clock(gpu).time).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-LOST" }));
  expect(() => clock(gpu).advance(1)).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-LOST" }));
});
