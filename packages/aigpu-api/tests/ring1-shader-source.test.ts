...
test("malformed ShaderSource without version throws AIGPU-SHADER-SOURCE-INVALID", async () => {
  const gpu = await init();

  expect(() => effect(gpu, { wgsl: FRAGMENT } as never)).toThrowError(
    /AIGPU-SHADER-SOURCE-INVALID: expected WGSL or \{ version, wgsl \}, got .* Fix: configure @aigpu\/wgsl loader-vite or loader-webpack\./,
  );
  gpu.dispose();
});

test("unsupported ShaderSource version throws AIGPU-SHADER-SOURCE-INVALID", async () => {
  const gpu = await init();

  expect(() => effect(gpu, { version: 2, wgsl: FRAGMENT } as never)).toThrowError(
    "AIGPU-SHADER-SOURCE-INVALID: unsupported ShaderSource v2; expected v1. Fix: update aigpu or regenerate it.",
  );
  gpu.dispose();
});
