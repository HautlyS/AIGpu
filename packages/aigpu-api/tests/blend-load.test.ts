...
test("invalid blend and writeMask options fail at draw construction", async () => {
  const gpu = await init();
  expect(() => draw(gpu, { shader: DRAW_SHADER, label: "badBlend", blend: "screen" as never })).toThrowError(/AIGPU-BLEND-INVALID|Invalid blend/);
  expect(() => draw(gpu, { shader: DRAW_SHADER, label: "badObject", blend: { alpha: { src: "one", dst: "zero" } } as never })).toThrowError(/AIGPU-BLEND-INVALID|Invalid blend/);
  expect(() => draw(gpu, { shader: DRAW_SHADER, label: "badMask", writeMask: "rgb" as never })).toThrowError(/AIGPU-WRITEMASK-INVALID|Invalid writeMask/);
  expect(() => draw(gpu, { shader: DRAW_SHADER, label: "badChannel", writeMask: ["r", "x"] as never })).toThrowError(/AIGPU-WRITEMASK-INVALID|Invalid writeMask/);
  gpu.dispose();
});
...
test("frame.pass rejects clear false with MSAA targets", async () => {
  ...
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: msaa, clear: false }, () => undefined))).toThrowError(/AIGPU-PASS-PRESERVE-MSAA|preserve MSAA/);
  gpu.dispose();
});
...
test("target.clearColor and surface.clearColor are mutable at runtime and validated", async () => {
  ...
  expect(() => { offscreen.clearColor = [0, 0, Number.NaN, 1] as never; }).toThrowError(/AIGPU-CLEAR-COLOR-INVALID|Invalid target\.clearColor/);
  expect(() => { canvasSurface.clearColor = { r: 0, g: 0, b: 0 } as never; }).toThrowError(/AIGPU-CLEAR-COLOR-INVALID|Invalid surface\.clearColor/);
