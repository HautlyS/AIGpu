...
test("bundles reject recording draws with stencil ref but record ref-less stencil", async () => {
  ...
  expect(() => bundle(gpu, { target: colorTarget, label: "refBundle" }, (b) => b.draw(withRef))).toThrowError(/AIGPU-BUNDLE-STENCIL-REF|stencil\.ref/);
...
test("stencil requires a target signature whose depth format has a stencil aspect", async () => {
  ...
  expect(() => drawable.draw(depthOnly)).toThrowError(/AIGPU-STENCIL-INVALID|depth24plus-stencil8/);
  expect(() => drawable.draw(noDepth)).toThrowError(/AIGPU-STENCIL-INVALID|no depth/);
  expect(() => refOnly.draw(depthOnly)).toThrowError(/AIGPU-STENCIL-INVALID|depth24plus-stencil8/);
  expect(() => drawable.compileSync({ colors: ["rgba8unorm"], depth: "depth32float" })).toThrowError(/AIGPU-STENCIL-INVALID|stencil aspect/);
  expect(() => drawable.compileSync({ colors: ["rgba8unorm"], depth: "depth16unorm" })).toThrowError(/AIGPU-STENCIL-INVALID|stencil aspect/);
  ...
  expect(() => draw(gpu, { shader: DRAW_SHADER, label: "eager-needs-stencil", targets: [depthOnly], stencil: { front: { compare: "equal" } } })).toThrowError(/AIGPU-STENCIL-INVALID|depth24plus-stencil8/);
  gpu.dispose();
});
...
test("invalid stencil options fail at draw construction", async () => {
  ...
  const expectInvalid = (label: string, stencil: unknown): void => {
    expect(() => draw(gpu, { shader: DRAW_SHADER, label, stencil: stencil as never })).toThrowError(/AIGPU-STENCIL-INVALID|Invalid stencil/);
  };
...
test("frame.pass validates clearStencil range, preserve, and stencil aspect", async () => {
  ...
  const depthOnly = target(gpu, { size: [2, 2], depth: true });
  const noDepth = target(gpu, { size: [2, 2] });
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearStencil: 0.5 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARSTENCIL-INVALID|clearStencil/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearStencil: -1 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARSTENCIL-INVALID|clearStencil/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearStencil: 0x1_0000_0000 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARSTENCIL-INVALID|clearStencil/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clearStencil: Number.NaN }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARSTENCIL-INVALID|clearStencil/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: colorTarget, clear: false, clearStencil: 1 }, () => undefined))).toThrowError(/AIGPU-PASS-PRESERVE-CLEARSTENCIL|preserves stencil/);
  // Dead option: the value is masked to the stencil aspect's bit width, but a missing stencil aspect can never see it.
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: depthOnly, clearStencil: 1 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARSTENCIL-INVALID|no stencil aspect/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: noDepth, clearStencil: 1 }, () => undefined))).toThrowError(/AIGPU-PASS-CLEARSTENCIL-INVALID|no stencil aspect/);
