...
test("invalid cull and frontFace options fail at draw construction", async () => {
  const gpu = await init();
  try {
    expect(() => draw(gpu, { shader: MESHLESS_WGSL, label: "badCull", cull: "backwards" as never })).toThrowError(/AIGPU-CULL-INVALID|Invalid cull/);
    expect(() => draw(gpu, { shader: MESHLESS_WGSL, label: "badFace", frontFace: "clockwise" as never })).toThrowError(/AIGPU-FRONTFACE-INVALID|Invalid frontFace/);
  } finally {
    gpu.dispose();
  }
});
...
test("indexed draw ranges and instance counts use draw options over slice over geometry", async () => {
  ...
  try {
    ...
    expect(() => drawable.draw({ target: colorTarget, indices: 5 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => drawable.draw({ target: colorTarget, indices: 6, firstIndex: 1 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => drawable.draw({ target: colorTarget, indices: -1 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
  } finally {
    gpu.dispose();
    vi.restoreAllMocks();
  }
...
test("non-indexed draw overrides validate absolute intervals against the parent geometry", async () => {
  ...
  try {
    ...
    expect(() => drawable.draw({ target: colorTarget, firstVertex: 2, vertices: 5 })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
    expect(() => drawable.draw({ target: colorTarget, firstVertex: Number.NaN })).toThrowError(/AIGPU-MESH-RANGE-INVALID/);
  } finally {
    gpu.dispose();
  }
});
