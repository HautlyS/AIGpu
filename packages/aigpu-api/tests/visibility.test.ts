...
import { getMockGPUDeviceInstrumentation } from "@aigpu/core";
...
test("visibility(gpu) needs no device feature and creates one occlusion query set of the declared capacity", async () => {
  ...
  expect(instrumentation.createQuerySetDescriptors).toEqual([
    { type: "occlusion", count: 8, label: "aigpu.visibility" },
  ]);
  vis.dispose();
  gpu.dispose();
});
...
test("AIGPU-VIS-CAPACITY-LIMIT rejects non-integer, < 1, and > 4096 capacities at visibility(gpu)", async () => {
  const gpu = await init();
  for (const capacity of [0, -1, 1.5, 4097, Number.NaN]) {
    expect(() => visibility(gpu, { capacity })).toThrowError(/AIGPU-VIS-CAPACITY-LIMIT|expected an integer in \[1, 4096\]/);
  }
  expect(() => visibility(gpu, { capacity: 4096 }).dispose()).not.toThrow();
  gpu.dispose();
});

test("AIGPU-VIS-CAPACITY throws at the occlusion() call that overflows the declared capacity", async () => {
  const gpu = await init();
  ...
  }))).toThrowError(/AIGPU-VIS-CAPACITY|more than the declared 2/);
...
test("AIGPU-VIS-LABEL-DUPLICATE rejects a live label; dispose() frees it for reuse", async () => {
  const gpu = await init();
  const vis = visibility(gpu);
  const q = vis.query("statue");

  expect(() => vis.query("statue")).toThrowError(/AIGPU-VIS-LABEL-DUPLICATE|already live/);
  q.dispose();
...
test("AIGPU-VIS-DISPOSED covers disposed handles and disposed visibility instances", async () => {
  const gpu = await init();
  const scene = target(gpu, { size: [4, 4], depth: true });
  const vis = visibility(gpu);
  const q = vis.query("statue");
  q.dispose();
  expect(() => q.reset()).toThrowError(/AIGPU-VIS-DISPOSED|disposed/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: scene, visibility: vis }, (p) => p.occlusion(q, () => undefined)))).toThrowError(/AIGPU-VIS-DISPOSED|disposed/);
  ...
  expect(() => vis2.query("x")).toThrowError(/AIGPU-VIS-DISPOSED|disposed/);
  expect(() => vis2.reset()).toThrowError(/AIGPU-VIS-DISPOSED|disposed/);
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: scene, visibility: vis2 }, (p) => p.occlusion(q2, () => undefined)))).toThrowError(/AIGPU-VIS-DISPOSED|disposed/);
...
test("AIGPU-VIS-NO-DEPTH rejects visibility on a pass whose target has no depth attachment", async () => {
  const gpu = await init();
  const vis = visibility(gpu);
  const flat = target(gpu, { size: [4, 4] });

  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: flat, visibility: vis }, () => undefined)))
    .toThrowError(/AIGPU-VIS-NO-DEPTH|no depth attachment/);
  vis.dispose();
  gpu.dispose();
});

test("AIGPU-QUERY-NO-VISIBILITY rejects occlusion() in a pass opened without visibility", async () => {
  const gpu = await init();
  const vis = visibility(gpu);
  const scene = target(gpu, { size: [4, 4], depth: true });
  const q = vis.query("statue");
  ...
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass(scene, (p) => p.occlusion(q, () => undefined))))
    .toThrowError(/AIGPU-QUERY-NO-VISIBILITY|no occlusionQuerySet/);
  vis.dispose();
  gpu.dispose();
});

test("AIGPU-QUERY-NESTED rejects occlusion() inside an active occlusion() body", async () => {
  const gpu = await init();
  const vis = visibility(gpu);
  const scene = target(gpu, { size: [4, 4], depth: true });
  const qA = vis.query("a");
  ...
  }))).toThrowError(/AIGPU-QUERY-NESTED|cannot nest/);
...
test("AIGPU-QUERY-DUPLICATE rejects reusing a handle within one frame, across passes too", async () => {
  const gpu = await init();
  const vis = visibility(gpu);
  const scene = target(gpu, { size: [4, 4], depth: true });
  const q = vis.query("statue");
  ...
  expect(() => frame(gpu, (currentFrame) => currentFrame.pass({ target: scene, visibility: vis }, (p) => {
    ...
    p.occlusion(q, () => undefined);
  }))).toThrowError(/AIGPU-QUERY-DUPLICATE|already used this frame/);

  // Cross-pass reuse silently overwrites the earlier result in native WebGPU; aigpu forbids it.
...
test("mismatched instances are rejected: foreign handles, foreign gpus, and non-visibility values", async () => {
  ...
  expect(() => frame(gpuA, (currentFrame) => currentFrame.pass({ target: sceneA, visibility: {} as never }, () => undefined)))
    .toThrowError(/AIGPU-VIS-INVALID|expected a Visibility/);
  expect(() => frame(gpuA, (currentFrame) => currentFrame.pass({ target: sceneA, visibility: visA }, (p) => p.occlusion({ label: "statue" } as never, () => undefined))))
    .toThrowError(/AIGPU-VIS-INVALID|expected a VisibilityQuery/);
...
test("invalid query labels fail at query()", async () => {
  ...
  for (const label of ["", 1, null, undefined, {}]) {
    expect(() => vis.query(label as never)).toThrowError(/AIGPU-VIS-INVALID|non-empty string/);
  }
  vis.dispose();
  gpu.dispose();
});
...
test("a readback that fails is reported on gpu.onError and leaves handles untouched", async () => {
  ...
  expect(errors[0]).toMatchObject({ code: "AIGPU-QUERY-READBACK", message: expect.stringContaining("aigpu.visibility") });
  vis.dispose();
  gpu.dispose();
  vi.restoreAllMocks();
});
...
test("gpu.dispose() releases visibility instances created by that gpu", async () => {
  ...
  gpu.dispose();
  expect(destroyed).toEqual([0]);
  expect(() => vis.query("tower")).toThrowError(/AIGPU-VIS-DISPOSED|disposed/);
...
/** Captures render pass descriptors plus resolve/copy/finish ordering on aigpu.frame encoders. */
function spyFrameEncoders(device: GPUDevice): FrameEncoderOps {
  ...
  vi.spyOn(device, "createCommandEncoder").mockImplementation((descriptor?: GPUCommandEncoderDescriptor) => {
    ...
    if (descriptor?.label !== "aigpu.frame") return encoder;
    return {
      ...encoder,
...
/** Orders executeBundles relative to the occlusion scope on aigpu.frame render passes. */
...
test("visibility(gpu) validates the gpu before touching the device", async () => {
  const gpu = await init();
  gpu.dispose();
  expect(thrownBy(() => visibility(gpu))).toMatchObject({ code: "AIGPU-GPU-DISPOSED", where: "visibility" });
  expect(thrownBy(() => visibility({ disposed: false } as never))).toMatchObject({ code: "AIGPU-GPU-FOREIGN" });
});

test("visibility(gpu) still rejects a capacity outside the WebGPU query-set limit", async () => {
  const gpu = await init();
  expect(thrownBy(() => visibility(gpu, { capacity: 4097 }))).toMatchObject({ code: "AIGPU-VIS-CAPACITY-LIMIT" });
  expect(thrownBy(() => visibility(gpu, { capacity: 0 }))).toMatchObject({ code: "AIGPU-VIS-CAPACITY-LIMIT" });
  gpu.dispose();
});

/** Returns what `run` threw, so an assertion can inspect the AIGpuError's code instead of its message. */
