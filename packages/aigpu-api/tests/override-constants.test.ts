...
test("unknown constants keys fail at construction listing the available overrides", async () => {
  const gpu = await init();
  expect(() => draw(gpu, { shader: OVERRIDE_WGSL, label: "unknown", constants: { NOPE: 1 } })).toThrowError(/AIGPU-CONSTANTS-INVALID|"SCALE", "7" \(@id of SAMPLES\)/);
  // An @id override is identified only by the decimal id string, never by its name.
  expect(() => draw(gpu, { shader: OVERRIDE_WGSL, label: "id-by-name", constants: { SAMPLES: 4 } })).toThrowError(/AIGPU-CONSTANTS-INVALID|available overrides/);
  expect(() => draw(gpu, { shader: FRAGMENT_ONLY_WGSL, label: "not-object", constants: [1] as never })).toThrowError(/AIGPU-CONSTANTS-INVALID|expected \{ overrideNameOrId/);
  gpu.dispose();
});
...
test("non-finite values fail at construction; strings are rejected", async () => {
  ...
  for (const value of [Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY, "2"]) {
    expect(() => draw(gpu, { shader: FRAGMENT_ONLY_WGSL, label: "bad-value", constants: { SHADE: value } as never })).toThrowError(/AIGPU-CONSTANTS-INVALID|finite number or a boolean/);
  }
  gpu.dispose();
});
...
test("an override without a default must be provided", async () => {
  const gpu = await init();
  expect(() => draw(gpu, { shader: NO_DEFAULT_WGSL, label: "missing" })).toThrowError(/AIGPU-CONSTANTS-INVALID|override 'gain' has no default value/);
  expect(() => draw(gpu, { shader: NO_DEFAULT_WGSL, label: "still-missing", constants: {} })).toThrowError(/AIGPU-CONSTANTS-INVALID|override 'gain' has no default value/);
...
test("compute constants validate at construction with where compute", async () => {
  const gpu = await init();
  expect(() => compute(gpu, COMPUTE_WGSL, { label: "unknown", constants: { LIMIT: 4 } })).toThrowError(/AIGPU-CONSTANTS-INVALID|"STEP", "3" \(@id of LIMIT\)/);
  expect(() => compute(gpu, COMPUTE_WGSL, { label: "nan", constants: { STEP: Number.NaN } })).toThrowError(/AIGPU-CONSTANTS-INVALID|finite number or a boolean/);
  expect(() => compute(gpu, COMPUTE_NO_DEFAULT_WGSL, { label: "missing" })).toThrowError(/AIGPU-CONSTANTS-INVALID|override 'seed' has no default value/);
