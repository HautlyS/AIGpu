...
function createPublishedConsumerFixture() {
  const root = mkdtempSync(join(tmpdir(), "aigpu-client-types-"));
  const nodeModules = join(root, "node_modules");
  mkdirSync(join(root, "src"), { recursive: true });
  mkdirSync(join(nodeModules, "@aigpu"), { recursive: true });
  mkdirSync(join(nodeModules, "@webgpu"), { recursive: true });
  mkdirSync(join(nodeModules, "aigpu"), { recursive: true });

  writeFileSync(join(root, "aigpu-env.d.ts"), "/// <reference types=\"aigpu/client\" />\n");
  writeFileSync(join(root, "src", "shader.wgsl"), "@compute @workgroup_size(1) fn main() {}\n");
  writeFileSync(
    join(root, "src", "index.ts"),
    [
      'import shaderSource from "./shader.wgsl";',
      'import type { AIGPUClientEnvironment } from "aigpu/client";',
      "const env: AIGPUClientEnvironment = {};",
  ...
  writeFileSync(
    join(nodeModules, "aigpu", "package.json"),
    JSON.stringify({ name: "aigpu", type: "module", exports: { "./client": { types: "./client.d.ts" } } }, null, 2),
  );
  writeFileSync(join(nodeModules, "aigpu", "client.d.ts"), ts.sys.readFile(resolve(repoRoot, "packages/aigpu-api/client.d.ts")) ?? "");
  symlinkSync(resolve(repoRoot, "node_modules/@webgpu/types"), join(nodeModules, "@webgpu", "types"), "dir");
  symlinkSync(resolve(repoRoot, "packages/wgsl"), join(nodeModules, "@aigpu", "wgsl"), "dir");

  return root;
}

test("aigpu-env reference accepts .wgsl imports under tsc", () => {
...
test("published aigpu/client types resolve WebGPU globals without monorepo tsconfig", () => {
