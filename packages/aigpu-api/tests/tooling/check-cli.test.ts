...
test("aigpu check emits reflection JSON for WGSL files", async () => {
...
test("aigpu check JSON carries per-entry bindings, sampling pairs and vertex inputs", async () => {
  const output = await runCheckSuccess(resolve(fixtureRoot, "sample.wgsl"));
...
test("aigpu check surfaces Phase-1 fix-it text verbatim", async () => {
  const result = await runCheck([resolve(fixtureRoot, "bool-uniform.wgsl")]);
  expect(result.code).toBe(1);
  expect(result.stdout).toBeUndefined();
  expect(result.stderr).toContain("AIGPU-WGSL-REFLECT-BOOL-HOST-SHAREABLE");
  expect(result.stderr).toContain("AIGpuError: `bool` is not host-shareable in uniform/storage. Fix: use `u32` (0 | 1) → struct Params { enabled: u32 }");
});

test("aigpu check fails on WGSL reserved words used as identifiers", async () => {
  // The fixture is deliberately invalid WGSL, so with a WebGPU device present naga also rejects it.
  // Deliberately *not* pinning AIGPU_VALIDATE: the JSON contract must be identical either way — the
  ...
  expect(output.diagnostics).toEqual([
    {
      code: "AIGPU-WGSL-RESERVED-IDENT",
...
test("aigpu check rejects imported modules that declare bindings", async () => {
