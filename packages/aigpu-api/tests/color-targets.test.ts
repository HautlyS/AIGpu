...
test("colors length must match the target signature's color attachment count", async () => {
  ...
  expect(() => drawable.draw(mrt)).toThrowError(/AIGPU-COLORS-INVALID|colors has 1, but the target signature has 2/);
  ...
  expect(() => draw(gpu, { shader: MRT_SHADER, label: "eager-mismatch", targets: [mrt], colors: [null] })).toThrowError(/AIGPU-COLORS-INVALID|colors has 1, but the target signature has 2/);
  gpu.dispose();
});

test("invalid colors options fail at draw construction", async () => {
  const gpu = await init();
  expect(() => draw(gpu, { shader: MRT_SHADER, label: "not-array", colors: "rgba" as never })).toThrowError(/AIGPU-COLORS-INVALID|must be an array/);
  expect(() => draw(gpu, { shader: MRT_SHADER, label: "bad-entry", colors: [42] as never })).toThrowError(/AIGPU-COLORS-INVALID|colors\[0\]/);
  expect(() => draw(gpu, { shader: MRT_SHADER, label: "array-entry", colors: [["r"]] as never })).toThrowError(/AIGPU-COLORS-INVALID|colors\[0\]/);
  expect(() => draw(gpu, { shader: MRT_SHADER, label: "bad-blend", colors: [{ blend: "screen" }] as never })).toThrowError(/AIGPU-BLEND-INVALID|Invalid blend/);
  expect(() => draw(gpu, { shader: MRT_SHADER, label: "bad-mask", colors: [{ writeMask: ["x"] }] as never })).toThrowError(/AIGPU-WRITEMASK-INVALID|Invalid writeMask/);
  gpu.dispose();
});
