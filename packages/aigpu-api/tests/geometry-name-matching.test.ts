...
import { getMockGPUDeviceInstrumentation, init, AIGpuError, geometry, draw, target } from "../src/mock.ts";

const FS = `@fragment fn fs_main() -> @location(0) vec4f { return vec4f(1.0); }`;

function errorOf(fn: () => unknown): AIGpuError {
...
test("draw construction reports unmatched, ambiguous, missing, and format mismatch geometry inputs", async () => {
  ...
  try {
    ...
    expect([unmatched.code, ambiguous.code, missing.code, mismatch.code]).toEqual(["AIGPU-MESH-ATTRIBUTE-UNMATCHED", "AIGPU-MESH-ATTRIBUTE-UNMATCHED", "AIGPU-MESH-INPUT-MISSING", "AIGPU-MESH-FORMAT-MISMATCH"]);
