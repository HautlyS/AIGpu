...
test("frame(gpu) drives one runner per gpu: the clock advances once per frame and reentrancy is rejected", async () => {
  ...
  expect(codeOf(() => frame(gpu, () => { frame(gpu, () => undefined); }))).toBe("AIGPU-FRAME-REENTRANT");
  expect(codeOf(() => frame(gpu, () => { frame(gpu, () => undefined); }))).toBe("AIGPU-FRAME-REENTRANT");
  gpu.dispose();
});
...
test("surface(gpu, canvas) is one per canvas and frees the canvas when disposed", async () => {
  ...
  expect(codeOf(() => surface(gpu, canvas))).toBe("AIGPU-SURFACE-DUPLICATE");
...
test("the render factories refuse a disposed gpu instead of handing back a dead handle", async () => {
  const gpu = await init();
  gpu.dispose();
  expect(codeOf(() => draw(gpu, { shader: fullscreenSource(FREE_FN_FRAGMENT) }))).toBe("AIGPU-GPU-DISPOSED");
  expect(codeOf(() => effect(gpu, FREE_FN_FRAGMENT))).toBe("AIGPU-GPU-DISPOSED");
  expect(codeOf(() => target(gpu, { size: [4, 4] }))).toBe("AIGPU-GPU-DISPOSED");
  expect(codeOf(() => sampler(gpu))).toBe("AIGPU-GPU-DISPOSED");
  expect(codeOf(() => surface(gpu, mockCanvas(4, 4)))).toBe("AIGPU-GPU-DISPOSED");
  expect(codeOf(() => frame(gpu))).toBe("AIGPU-GPU-DISPOSED");
});
