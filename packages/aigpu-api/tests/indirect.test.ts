...
test("indirect draws require a buffer created with the indirect flag", async () => {
  ...
  expect(() => drawable.draw({ target: colorTarget, indirect: args })).toThrowError(/AIGPU-INDIRECT-INVALID|storage\(gpu, 16, \{ indirect: true \}\)/);
  gpu.dispose();
});
...
test("indirect offsets must be 4-aligned non-negative integers", async () => {
  ...
  const expectInvalid = (offset: number): void => {
    expect(() => drawable.draw({ target: colorTarget, indirect: { buffer: args, offset } })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  };
...
test("indirect arguments must fit: 16 bytes for drawIndirect, 20 for drawIndexedIndirect, 12 for dispatchWorkgroupsIndirect", async () => {
  ...
  expect(() => drawable.draw({ target: colorTarget, indirect: storage(gpu, 12, { indirect: true }) })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  expect(() => drawable.draw({ target: colorTarget, indirect: { buffer: storage(gpu, 16, { indirect: true }), offset: 4 } })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  // drawIndexedIndirect reads 5 32-bit values: 16 bytes are not enough.
  expect(() => indexed.draw({ target: colorTarget, indirect: storage(gpu, 16, { indirect: true }) })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  ...
  expect(() => sim.dispatch({ indirect: storage(gpu, 8, { indirect: true }) })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  expect(() => sim.dispatch({ indirect: storage(gpu, 12, { indirect: true }) })).not.toThrow();
  gpu.dispose();
});

test("malformed indirect values fail with AIGPU-INDIRECT-INVALID", async () => {
  const gpu = await init();
  const colorTarget = target(gpu, { size: [2, 2] });
  const drawable = draw(gpu, { shader: DRAW_SHADER, label: "bad-shape" });
  const expectInvalid = (indirect: unknown): void => {
    expect(() => drawable.draw({ target: colorTarget, indirect: indirect as never })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  };
...
test("indirect conflicts with CPU-side counts in the same draw call", async () => {
  ...
  for (const conflict of conflicts) {
    expect(() => drawable.draw({ target: colorTarget, indirect: args, ...conflict })).toThrowError(/AIGPU-INDIRECT-INVALID|ignored/);
  }
...
test("compute indirect validation mirrors the draw rules", async () => {
  ...
  expect(() => sim.dispatch({ indirect: storage(gpu, 12) })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  expect(() => sim.dispatch({ indirect: { buffer: storage(gpu, 64, { indirect: true }), offset: 6 } })).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  // Explicit workgroup counts alongside indirect are dead options.
  expect(() => (sim.dispatch as (x: unknown, y?: number) => void)({ indirect: storage(gpu, 12, { indirect: true }) }, 2)).toThrowError(/AIGPU-INDIRECT-INVALID|Invalid indirect/);
  gpu.dispose();
});
...
test("a storage(gpu) buffer drives an indirect draw and keeps the missing-usage diagnostic", async () => {
  ...
  expect(() => draw(gpu, { shader: DRAW_SHADER }).draw({ target: colorTarget, indirect: plain }))
    .toThrowError(/AIGPU-INDIRECT-INVALID|storage\(gpu, 16, \{ indirect: true \}\)/);
  gpu.dispose();
});
