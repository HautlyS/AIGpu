# @aigpu/wgsl-std/color

Pure WGSL color utilities for shaders resolved by `@aigpu/wgsl`. Import these functions from WGSL modules when you need sRGB transfer, luminance, exposure, tone mapping, or bloom threshold helpers without declaring any resources.

## Import

```wgsl
import { applyExposure, luminance, luminanceThreshold, tonemapAces, tonemapReinhard } from "@aigpu/wgsl-std/color";
```
...
**Throws:** These WGSL declarations do not throw. `resolveShader()` can still throw `AIGPU-WGSL-SYM-NOEXPORT` for misspelled imports, `AIGPU-WGSL-PKG-NOTFOUND` if the package import cannot be resolved, or validation errors such as `AIGPU-WGSL-NAGA-UNKNOWN` if caller WGSL is invalid.

## Examples

```ts
const shaderWgsl = `
import { applyExposure, linearToSrgb3, srgbToLinear3, tonemapAces } from "@aigpu/wgsl-std/color";
...
```ts
const bloomWgsl = `
import { luminanceThreshold } from "@aigpu/wgsl-std/color";
...
- **See also:** `@aigpu/wgsl-std/hash`, `@aigpu/wgsl-std/noise`, `resolveShader`.
