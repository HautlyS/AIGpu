# @aigpu/wgsl-std/noise/simplex

Pure WGSL simplex noise (`simplex2d`, `simplex3d`) and amplitude-normalized fBM (`fbmSimplex2d`, `fbmSimplex3d`) for resolver-managed shaders. Import these for smooth gradient noise — clouds, terrain, water, plasma — when you want the cheaper, higher-frequency sibling of `@aigpu/wgsl-std/noise/perlin`. For cell/edge patterns use `@aigpu/wgsl-std/noise` (Voronoi) instead.

## Import

```wgsl
import { simplex2d, simplex3d, fbmSimplex2d, fbmSimplex3d } from "@aigpu/wgsl-std/noise/simplex";
```
...
**Throws:** These WGSL declarations do not throw. `resolveShader()` can still throw `AIGPU-WGSL-SYM-NOEXPORT` for a misspelled import (note the module is `@aigpu/wgsl-std/noise/simplex`, not `@aigpu/wgsl-std/noise`), `AIGPU-WGSL-PKG-NOTFOUND` if the package import cannot be resolved, or validation errors such as `AIGPU-WGSL-NAGA-UNKNOWN` if the calling WGSL is invalid. This module reaches `@aigpu/wgsl-std/hash` through a private gradient core, so package resolution must be able to resolve that package export too.
...
Extremes are rare: with sigma ≈ 0.39–0.53 most samples sit well inside the range. **Remap, do not saturate** — `saturate(simplex3d(p))` throws away the whole negative half of the field. Use `remap(-0.6, 0.6, 0.0, 1.0, value)` from `@aigpu/wgsl-std/math` and clamp afterwards if you need `[0, 1]`.

## Examples

```ts
const simplexWgsl = `
import { simplex3d } from "@aigpu/wgsl-std/noise/simplex";
import { remap, saturate } from "@aigpu/wgsl-std/math";
...
```ts
const cloudWgsl = `
import { fbmSimplex3d } from "@aigpu/wgsl-std/noise/simplex";
import { remap, saturate } from "@aigpu/wgsl-std/math";
...
- **See also:** `@aigpu/wgsl-std/noise/perlin` (`perlin2d`/`perlin3d`/`fbmPerlin2d`/`fbmPerlin3d` — smoother, exactly 0 on the integer lattice, ~2.5× less slope), `@aigpu/wgsl-std/noise` (Voronoi cells), `@aigpu/wgsl-std/math` (`remap`, `saturate`), `@aigpu/wgsl-std/hash` (`pcg2d`, `pcg3d`).
