...
**Throws:** These WGSL declarations do not throw. `resolveShader()` can still throw `AIGPU-WGSL-SYM-NOEXPORT` for a misspelled import (note this module is `@aigpu/wgsl-std/noise/perlin`, *not* `@aigpu/wgsl-std/noise`), `AIGPU-WGSL-PKG-NOTFOUND` if the package cannot be resolved, or validation errors such as `AIGPU-WGSL-NAGA-UNKNOWN` if caller WGSL is invalid. This module also reaches `@aigpu/wgsl-std/hash`, so that package export must resolve too.
...
```ts
const cloudsWgsl = `
import { fbmPerlin3d } from "@aigpu/wgsl-std/noise/perlin";
import { remap, saturate } from "@aigpu/wgsl-std/math";
...
- **Range and shaping.** Output is `(-1, 1)`, never clipped, but `σ ≈ 0.305` (2D) / `0.260` (3D) means extremes are rare: over 1e6 samples the largest magnitudes seen were 0.9937 and 0.9560. Use `remap` from `@aigpu/wgsl-std/math` to stretch the band you care about; reaching for `saturate` alone discards the entire negative half of the field.
...
- **See also:** `@aigpu/wgsl-std/noise/simplex` (same shape, ~2.5× the slope and ~1.7× the σ at the same input scale — scale `position` by ~0.4–0.5 when migrating, and it hashes 4 corners instead of 8 in 3D), `@aigpu/wgsl-std/noise` (Voronoi/cellular, for cells rather than smooth fields), `@aigpu/wgsl-std/math` (`remap`, `saturate`), `@aigpu/wgsl-std/hash`, `resolveShader`.
