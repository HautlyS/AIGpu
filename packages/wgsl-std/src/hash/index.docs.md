...
```wgsl
import { hash1, hash2, hash3, hashU32, pcg2d, pcg3d, unitFloat } from "@aigpu/wgsl-std/hash";
```
...
```ts
const jitterWgsl = `
import { hash2 } from "@aigpu/wgsl-std/hash";
...
- **See also:** `@aigpu/wgsl-std/noise`, `@aigpu/wgsl-std/color`, `resolveShader`.
