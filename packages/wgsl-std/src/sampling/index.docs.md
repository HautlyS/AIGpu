# @aigpu/wgsl-std/sampling

Raw WGSL sampling utility module for `@aigpu/wgsl` imports. The module contains pure declarations only: no bindings, overrides, hidden state, resources, or entry points.

```wgsl
import { goldenAngle, vogelDisk, hammersley2d } from "@aigpu/wgsl-std/sampling";
