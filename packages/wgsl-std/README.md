...
Pure WGSL utility snippets for use with `@aigpu/wgsl` import resolution.
...
```wgsl
import { saturate, remap, safeNormalize3, rotate2d } from "@aigpu/wgsl-std/math";
import { srgbToLinear3, linearToSrgb3, luminance, applyExposure } from "@aigpu/wgsl-std/color";
import { vogelDisk, hammersley2d } from "@aigpu/wgsl-std/sampling";
import { pi, tau, goldenAngle } from "@aigpu/wgsl-std/constants";
```

There is no root WGSL export. Subpath exports resolve to physical WGSL files:

- `@aigpu/wgsl-std/math` -> `src/math/index.wgsl`
- `@aigpu/wgsl-std/color` -> `src/color/index.wgsl`
- `@aigpu/wgsl-std/sampling` -> `src/sampling/index.wgsl`
- `@aigpu/wgsl-std/constants` -> `src/constants/index.wgsl`
- `@aigpu/wgsl-std/hash` -> `src/hash/index.wgsl`
- `@aigpu/wgsl-std/noise` -> `src/noise/index.wgsl` (Voronoi/cellular noise only)
- `@aigpu/wgsl-std/noise/perlin` -> `src/noise/perlin/index.wgsl` (Perlin gradient noise + fBM)
- `@aigpu/wgsl-std/noise/simplex` -> `src/noise/simplex/index.wgsl` (simplex gradient noise + fBM)
- `@aigpu/wgsl-std/fullscreen` -> `src/fullscreen/index.wgsl`
- `@aigpu/wgsl-std/light` -> `src/light/index.wgsl`

`@aigpu/wgsl-std/noise` stays Voronoi-only for backward compatibility; it does not re-export Perlin or simplex. Import `@aigpu/wgsl-std/noise/perlin` or `@aigpu/wgsl-std/noise/simplex` directly for smooth gradient noise — `import { perlin2d } from "@aigpu/wgsl-std/noise"` does not work, each family is its own subpath so importing one never pulls the other's WGSL into your bundle.
...
`@aigpu/wgsl-std/constants` includes common math constants as plain WGSL `const` declarations:
...
The constants module is intentionally small so importing it does not add much WGSL text. `resolveShader()` also performs conservative declaration-level dead-code elimination, so unused declarations from larger WGSL utility modules are pruned from shaders with entry points before minification. `goldenAngle` remains available from `@aigpu/wgsl-std/sampling` for sampling-only shaders.

See `src/constants/index.docs.md` for examples and precision notes.

## Math utilities

`@aigpu/wgsl-std/math` includes scalar range helpers and vector safety helpers:
...
`@aigpu/wgsl-std/color` includes non-PBR color helpers:
...
```wgsl
import { vogelDisk } from "@aigpu/wgsl-std/sampling";
