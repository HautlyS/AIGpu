...
test("sampling helpers resolve from @aigpu/wgsl-std/sampling and produce valid declarations", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { goldenAngle, vogelDisk, radicalInverseVdc, hammersley2d } from "@aigpu/wgsl-std/sampling";
fn main() -> vec4f {
...
test("sampling helper minified output is deterministic", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { hammersley2d } from "@aigpu/wgsl-std/sampling";
fn main() -> vec2f {
