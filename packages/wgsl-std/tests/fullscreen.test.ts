...
test("fullscreen helpers resolve from @aigpu/wgsl-std/fullscreen and produce valid declarations", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { fullscreenTriangleClip, fullscreenTriangleUv } from "@aigpu/wgsl-std/fullscreen";
fn main() -> vec4f {
...
test("fullscreen helper minified output is deterministic", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { fullscreenTriangleClip } from "@aigpu/wgsl-std/fullscreen";
fn main() -> vec4f {
