...
test("noise helpers resolve from @aigpu/wgsl-std/noise and produce valid declarations", async () => {
...
test.skipIf(!dockerTest)("resolved noise utility shader validates with naga", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { voronoi2d, voronoi3d } from "@aigpu/wgsl-std/noise";
