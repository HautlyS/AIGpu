...
test("color helpers resolve from @aigpu/wgsl-std/color and produce valid declarations", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { srgbToLinear, srgbToLinear3, srgbToLinear4, linearToSrgb, linearToSrgb3, linearToSrgb4, luminance, applyExposure, tonemapAces, tonemapReinhard, luminanceThreshold } from "@aigpu/wgsl-std/color";
fn main() -> vec4f {
...
test("single color helper import resolves deterministically", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { luminance } from "@aigpu/wgsl-std/color";
fn main() -> f32 {
...
test("color helper minified output is deterministic", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { srgbToLinear3, luminance } from "@aigpu/wgsl-std/color";
fn main() -> f32 {
...
test.skipIf(!dockerTest)("resolved color utility shader validates with naga", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { srgbToLinear3, linearToSrgb3, luminance, applyExposure } from "@aigpu/wgsl-std/color";
