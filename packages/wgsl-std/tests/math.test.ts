...
test("math helpers resolve from @aigpu/wgsl-std/math and produce valid declarations", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { saturate, clamp01, inverseLerp, remap, safeNormalize2, safeNormalize3, safeNormalize4, rotate2d } from "@aigpu/wgsl-std/math";
fn main() -> vec4f {
...
test("math helper minified output is deterministic", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { saturate, remap } from "@aigpu/wgsl-std/math";
fn main() -> f32 {
...
test.skipIf(!dockerTest)("resolved math utility shader validates with naga", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { saturate, safeNormalize2, rotate2d } from "@aigpu/wgsl-std/math";
...
async function runMathCompute(): Promise<Float32Array> {
  ...
  const modules = {
    "/main.wgsl": `import { saturate, inverseLerp, remap, safeNormalize2, rotate2d } from "@aigpu/wgsl-std/math";
...
fn main() {
  ...
  const shader = await resolveShader({ entry: "/main.wgsl", modules, packageMap: { "@aigpu/wgsl-std/math": mathPath }, validate: false });
