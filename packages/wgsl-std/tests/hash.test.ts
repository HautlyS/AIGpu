...
test("hash helpers resolve from @aigpu/wgsl-std/hash and produce valid declarations", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { hashU32, pcg2d, pcg3d, unitFloat, hash1, hash2, hash3 } from "@aigpu/wgsl-std/hash";
fn main() -> vec4f {
  let a = hashU32(1u);
...
test("hash helper minified output is deterministic", async () => {
  const dir = await workspaceFixture();
  const entry = join(dir, "app", "main.wgsl");
  await writeFile(entry, `import { hash3 } from "@aigpu/wgsl-std/hash";
fn main() -> vec3f {
