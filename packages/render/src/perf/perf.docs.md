...
WebGPU verification helpers for the optimize loop. These utilities live under `@aigpu/render/perf`; they are not meant for runtime use.
...
```ts
import { gpuFrameTime } from "@aigpu/render/perf";
```
...
| options.label | string | ✖ | "aigpu-gpuFrameTime" | Assigns encoder labels and buffer names to make debugging captures easier. |
...
| label | string | ✖ | "aigpu-gpuFrameTime" | Label used for encoders and buffers created internally. |
...
```ts
import { pixelDiff } from "@aigpu/render/perf";
```
...
```ts
import { pixelDiff } from "@aigpu/render/perf";
import type { Texture } from "@aigpu/core";
