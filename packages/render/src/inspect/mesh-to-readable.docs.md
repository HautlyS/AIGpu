...
```ts
import { meshToReadable } from "@aigpu/render/inspect";
```
...
|---|---|---|---|---|
| mesh | Mesh | ✔ | — | Source mesh. Must come from `@aigpu/render` helpers or user code with a valid `vertexBuffer`. |
...
**Throws:** `AIGPU-CORE-INVALID-USAGE` when the source vertex buffer exposes an invalid `GPUBuffer.usage` mask (non-finite); fix by creating the geometry through `geometry(gpu, ...)` or adding `copy_src` when constructing buffers manually.
