...
`@aigpu/render/edit` is the CPU-side mesh editing layer for mesh-like render data.
...
`@aigpu/render/src/edit/kernel-handle.ts` and calling `unwrapKernel(handle)`. Code
...
import { toEditable, extrude, bevel } from "@aigpu/render/edit";
import { geometry } from "aigpu";
...
- **Triangle-only kernel.** v1 stores triangles. Operators that produce n-gons
  ...
  [#44](https://github.com/hautlys/AIGpu/issues/44).
...
- **See [#44](https://github.com/hautlys/AIGpu/issues/44)** for the kernel +
