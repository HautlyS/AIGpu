...
1. Run `AIGPU_WRITE_SNAPSHOTS=1 pnpm test:docker -- packages/render/tests/inspect`
...
4. Re-run the test suite without `AIGPU_WRITE_SNAPSHOTS` to confirm byte-equal pass.

## Determinism

Snapshots assume Docker/Dawn rendering. Local non-Dawn runs may differ.
The test suite gates these tests on `AIGPU_DOCKER_TEST=1`.
