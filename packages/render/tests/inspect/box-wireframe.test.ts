...
import { perspectiveCamera, type Vec3 } from "aigpu/scene";
import { meshToWireframe, wireframeMaterial } from "@aigpu/render/inspect";
...
for (const [angle, { position }] of Object.entries(CAMERAS)) {
  test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")(`wireframe ${angle} matches snapshot`, async () => {
