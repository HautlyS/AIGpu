...
import { bevel, dissolveFaces, extrude, inset, subdivideEdges, toEditable, type EditableMeshValue } from "@aigpu/render/edit";
...
test("long mesh-edit pipeline stays within loose runtime budget", async () => {
  ...
  try {
    ...
    if (process.env.AIGPU_WRITE_PERF_BASELINES === "1") await writeBaseline(baseline);
