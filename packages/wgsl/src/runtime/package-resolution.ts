...
/** Fix-it for a bare package specifier that is not installed. WGSL packages are npm packages: `@aigpu/wgsl-std` ships with `aigpu`, anything else has to be installed. */
...
function packageNotFound(pkg: string, fixit: string): ReturnType<typeof wgslError> {
  return wgslError("AIGPU-WGSL-PKG-NOTFOUND", `Package ${pkg} was not found. ${fixit.replace("<pkg>", pkg)}`);
}

export function resolveImport(spec: string, from: string, opts: PackageResolveOptions, diagnostics: Diagnostic[]): string {
  if (spec.startsWith("/")) throw wgslError("AIGPU-WGSL-RES-ABS", "Absolute WGSL imports are not portable");
...
export async function readModule(path: string, opts: PackageResolveOptions): Promise<string> {
  ...
  throw wgslError("AIGPU-WGSL-RES-NOTFOUND", `WGSL module ${path} was not found`);
}
 ...
 * which is why `@aigpu/wgsl-std` reaching a project transitively through `aigpu` worked under npm
 * (accidental hoisting) but failed under pnpm's isolated store and Yarn PnP. `@aigpu/wgsl` depends on
 * `@aigpu/wgsl-std`, so the package manager guarantees it next to *us* in every layout. Node's own
 ...
 *
 * This runs after the loop above, so it never shadows a project-local copy. Scoped to `@aigpu/*`
 * specifiers only: it exists solely to rescue aigpu's own transitives in isolated pnpm/PnP layouts.
 * Any other bare specifier that happens to be reachable from `@aigpu/wgsl`'s own install location
 * (e.g. a devDependency like `webpack`) must NOT resolve here — it should fail with
 * AIGPU-WGSL-PKG-NOTFOUND instead of silently picking up an unrelated JS file that then fails much
...
function packageExport(pkgJson: string, sub: string, diagnostics: Diagnostic[]): string {
  ...
  if (value && typeof value.default === "string") {
    warnOnce(diagnostics, "AIGPU-WGSL-PKG-CONDITIONAL", `Package export ${sub} uses conditional exports; selecting default`);
  ...
  throw wgslError("AIGPU-WGSL-PKG-NOTFOUND", `Package export ${sub} was not found in ${parsed.name ?? root}. Check the package's exports map or fix the import subpath`);
}
...
function defaultVirtual(path: string, modules: Record<string, string>): string { const clean = normalize(path).replace(/\\/g, "/"); if (modules[clean] !== undefined) return clean; if (modules[`${clean}.wgsl`] !== undefined) return `${clean}.wgsl`; if (modules[`${clean}/index.wgsl`] !== undefined) return `${clean}/index.wgsl`; throw wgslError("AIGPU-WGSL-RES-NOTFOUND", `WGSL module ${clean} was not found`); }
function defaultFile(path: string): string { if (existsSync(path) && statSync(path).isDirectory()) path = join(path, "index.wgsl"); for (const choice of extname(path) ? [path] : [`${path}.wgsl`, join(path, "index.wgsl")]) if (existsSync(choice)) return choice; throw wgslError("AIGPU-WGSL-RES-NOTFOUND", `WGSL module ${path} was not found`); }
