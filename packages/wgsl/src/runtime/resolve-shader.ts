...
export interface ResolveOptions {
   ...
   *   `AIGPU-WGSL-NAGA-UNKNOWN`. If no device/adapter is available, warns once to stderr with the
   *   error code and fix, continues, and records the skip on `ResolvedShader.validation`.
   * - `"require"` / `true` — attempt validation; throws `AIGPU-WGSL-VALIDATE-NO-DEVICE` (or
   *   `AIGPU-WGSL-VALIDATE-ADAPTER-MISSING`) instead of silently skipping when no device is
   *   available.
   *
   * Defaults to `"auto"`, or to `AIGPU_VALIDATE` (`"off"|"auto"|"require"`) when set — an explicit
   * `validate` option here always wins over `AIGPU_VALIDATE`.
   *
...
/** `true` -> `"require"`, `false` -> `"off"`, unset -> `AIGPU_VALIDATE` (default `"auto"`). */
...
async function loadGraph(path: string, opts: ResolveOptions, loaded: Map<string, MangleModule>, stack: string[], diagnostics: DiagnosticList[number][]): Promise<void> {
  if (stack.includes(path)) throw wgslError("AIGPU-WGSL-IMP-SELF", `Import cycle: ${[...stack, path].join(" -> ")}`);
...
function checkImportShadows(module: MangleModule): void {
  ...
  for (const imp of module.parsed.imports) for (const binding of imp.bindings) {
    if (imported.has(binding.local)) throw wgslError("AIGPU-WGSL-SYM-IMPORT-SHADOW", `Import ${binding.local} conflicts with another import`);
    imported.add(binding.local);
    if (!binding.namespace && module.parsed.locals.some((local) => local.name === binding.local)) throw wgslError("AIGPU-WGSL-SYM-IMPORT-SHADOW", `Import ${binding.local} shadows a local symbol`);
  }
}
...
function assertNoJsVisibleDuplicates(modules: readonly MangleModule[]): void {
  ...
  for (const module of modules) for (const local of module.parsed.locals) {
    if (local.kind === "override") duplicate(overrides, local.name, module.path, "AIGPU-WGSL-OVERRIDE-DUP");
    if (entryKind(module, local.name, local.kind) === "entry") duplicate(entries, local.name, module.path, "AIGPU-WGSL-ENTRYPOINT-DUP");
  }
}
