...
export function expectIdent(token: Token | undefined): string {
  if (token?.kind !== "ident" && token?.kind !== "keyword") {
    throw wgslError("AIGPU-WGSL-REFLECT-PARSE", "Expected identifier", token?.line, token?.column);
  }
  return token.text;
}
...
export function findNext(tokens: readonly Token[], start: number, text: string): number {
  ...
  throw wgslError("AIGPU-WGSL-REFLECT-PARSE", `Expected ${text}`, tokens[start]?.line, tokens[start]?.column);
}
...
export function matching(tokens: readonly Token[], open: number): number {
  ...
  throw wgslError("AIGPU-WGSL-REFLECT-PARSE", `Unclosed ${start}`, tokens[open]?.line, tokens[open]?.column);
}
