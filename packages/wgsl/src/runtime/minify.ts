import { wgslError, type AIGpuError } from "./errors.ts";
...
export function normalizeMinifyOption(option: MinifyOption | undefined): NormalizedMinifyOptions {
  ...
  if (identifiers !== "none" && identifiers !== "safe") {
    throw wgslError("AIGPU-WGSL-MINIFY-IDENTIFIERS", `Unknown WGSL minify identifiers mode: ${String(identifiers)}`);
  }
...
export function minifyWgsl(source: string, options: MinifyWgslOptions = {}): string {
  try {
    ...
    if (isWgslError(error) && error.code === "AIGPU-WGSL-LEX-UNTERM-COMMENT") {
      throw wgslError("AIGPU-WGSL-MINIFY-BLOCK", "Unterminated WGSL block comment", error.line, error.column);
    }
    throw error;
  }
}

function isWgslError(error: unknown): error is AIGpuError {
