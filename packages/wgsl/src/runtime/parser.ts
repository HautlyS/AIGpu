...
export function parseModule(tokens: readonly Token[]): ModuleParse {
  ...
  while (i < tokens.length) {
    ...
    if (token.text === "import") {
      if (sawDecl) throw wgslError("AIGPU-WGSL-IMP-ORDER", "Imports must precede declarations", token.line, token.column);
      const [decl, next] = parseImport(tokens, i);
      imports.push(decl); i = next; continue;
    }
    if (token.text === "export" && tokens[i + 1]?.text === "{") throw wgslError("AIGPU-WGSL-EXP-REEXPORT-CYCLE", "Re-export cycles are not supported", token.line, token.column);
    if (token.text === "@" && tokens[i + 2]?.text === "export" && tokens[i + 3]?.text === "@") throw wgslError("AIGPU-WGSL-EXP-NOTDECL", "Repeated export attributes", token.line, token.column);
...
function parseImport(tokens: readonly Token[], start: number): [ImportDecl, number] {
  ...
  if (tokens[i]?.text === "{") {
    ...
    throw wgslError("AIGPU-WGSL-IMP-SIDEEFFECT", "Side-effect imports are not supported", tokens[i]!.line, tokens[i]!.column);
  } else {
    throw wgslError("AIGPU-WGSL-IMP-DEFAULT", "Default imports are not supported", tokens[i]?.line, tokens[i]?.column);
  }
  const fromToken = tokens[i];
  if (fromToken?.kind !== "string") throw wgslError("AIGPU-WGSL-RES-NOTFOUND", "Import path must be a string", fromToken?.line, fromToken?.column);
...
function findDeclName(tokens: readonly Token[], kindIndex: number): string {
  ...
  throw wgslError("AIGPU-WGSL-EXP-NOTDECL", "Exported declaration has no name", tokens[kindIndex]?.line, tokens[kindIndex]?.column);
}
function expectText(token: Token | undefined, text: string): void { if (token?.text !== text) throw wgslError("AIGPU-WGSL-IMP-DEFAULT", `Expected ${text}`, token?.line, token?.column); }
function expectIdent(token: Token | undefined): string { if (token?.kind !== "ident") throw wgslError("AIGPU-WGSL-IMP-DEFAULT", "Expected identifier", token?.line, token?.column); return token.text; }
