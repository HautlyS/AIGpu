...
export const NON_ASCII_IDENTIFIER_CODE = "AIGPU-WGSL-IDENT-NONASCII";
const XID_ISSUE_URL = "https://github.com/hautlys/AIGpu/issues/294";
...
export function scan(source: string, path?: string): Token[] {
  ...
  while (i < source.length) {
    ...
    if (ch === "/" && source[i + 1] === "*") {
      ...
      if (depth !== 0) throw wgslError("AIGPU-WGSL-LEX-UNTERM-COMMENT", "Unterminated block comment", atLine, atColumn);
      continue;
    }
    if (ch === '"' || ch === "'") {
      const quote = ch;
      ...
      while (i < source.length && source[i] !== quote) {
        if (source[i] === "\n") throw wgslError("AIGPU-WGSL-LEX-UNTERM-STRING", "Unterminated string", atLine, atColumn);
        if (source[i] === "\\") step();
        step();
      }
      if (i >= source.length) throw wgslError("AIGPU-WGSL-LEX-UNTERM-STRING", "Unterminated string", atLine, atColumn);
...
function nonAsciiIdentifierError(source: string, index: number, line: number, column: number, path: string | undefined): WGSLError {
  ...
  const error = wgslErrorWithFix(
    ...
    `Non-ASCII identifier '${text}'${where} at line ${line} column ${atColumn}; aigpu's WGSL pipeline supports ASCII identifiers only`,
