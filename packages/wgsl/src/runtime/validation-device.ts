...
/** The slice of `@aigpu/adapter-node` this module uses. */
...
async function acquire(): Promise<GPUDevice> {
  ...
  try {
    // `@aigpu/adapter-node` is an *optional* peer dependency, imported lazily and never hoisted to
    ...
    adapterNode = (await import("@aigpu/adapter-node")) as AdapterNodeModule;
  } catch (cause) {
    throw wgslErrorWithFix(
      "AIGPU-WGSL-VALIDATE-ADAPTER-MISSING",
      "WGSL validation needs @aigpu/adapter-node to acquire a WebGPU device, but it could not be imported.",
      {
  ...
  try {
    ...
    throw wgslErrorWithFix(
      "AIGPU-WGSL-VALIDATE-NO-DEVICE",
      `device acquisition failed via @aigpu/adapter-node${adapterError?.code ? ` (${adapterError.code})` : ""}: ${adapterError?.message ?? String(cause)}`,
      {
        // Forwarded verbatim: @aigpu/adapter-node owns the remediation text for device failures, and
        // re-wording it here would let the two drift apart.
        fix: adapterError?.fix ?? "Run `npx aigpu doctor` to diagnose the local WebGPU/Dawn setup.",
        cause,
        where: "resolveShader",
        metadata: {
