...
import type { EntryPointInfo, Reflection } from "@aigpu/wgsl/reflect-source";
```
...
`ReflectedBindingLayout`, and `WGSLType` — all importable from `@aigpu/wgsl/reflect-source`.
...
**Throws:** `AIGPU-WGSL-REFLECT-SOURCE-IMPORT` when `wgsl` contains a top-level `import` — `reflectSource()` intentionally rejects WGSL import graphs. Use `resolveShader()` (`@aigpu/wgsl/runtime`) instead, which resolves imports first and exposes the identical `Reflection` shape on `ResolvedShader.reflection`.
**Throws:** The same lexer/parser `AIGPU-WGSL-LEX-*`/`AIGPU-WGSL-*` diagnostics `resolveShader()` raises for malformed WGSL (unterminated comments/strings, non-ASCII identifiers, etc.) — fix the WGSL reported by the diagnostic.
...
```ts
import { reflectSource, type EntryPointInfo } from "@aigpu/wgsl/reflect-source";
...
- **See also:** `ResolvedShader` (`compile()`, `@aigpu/wgsl`), `resolveShader` (`@aigpu/wgsl/runtime`).
