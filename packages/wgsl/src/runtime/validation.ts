...
const deviceErrorCodes = new Set(["AIGPU-WGSL-VALIDATE-ADAPTER-MISSING", "AIGPU-WGSL-VALIDATE-NO-DEVICE"]);
let warned = false;
...
export function resolveDefaultValidateMode(): ValidateMode {
  const raw = process.env.AIGPU_VALIDATE;
  if (raw === undefined || raw === "") return "auto";
  if (raw === "off" || raw === "auto" || raw === "require") return raw;
  throw wgslErrorWithFix("AIGPU-WGSL-VALIDATE-ENV-INVALID", `Invalid AIGPU_VALIDATE=${JSON.stringify(raw)}; expected "off", "auto", or "require".`, {
...
function warnValidationSkippedOnce(error: WGSLError): void {
  if (warned) return;
  warned = true;
  console.error(`aigpu: WGSL validation skipped (${error.code}): ${error.message}${error.fix ? `\n  fix: ${error.fix}` : ""}\n  Set validate: "require" (or AIGPU_VALIDATE=require) to make this a hard failure, or validate: "off" to silence it.`);
}
...
function diagnostic(wgsl: string, message: CompilationMessage, cause: unknown): ValidationDiagnostic {
  ...
  const error = wgslError("AIGPU-WGSL-NAGA-UNKNOWN", message.message ?? "WGSL validation failed", mapped.line, mapped.column) as ValidationDiagnostic;
  ...
  if (!mapped.columnPrecise) {
    error.relatedDiagnostics = [{ code: "AIGPU-WGSL-COL-APPROX", message: "column position is approximate; this line contained substituted identifiers" }];
    error.metadata = { ...(error.metadata ?? {}), codes: ["AIGPU-WGSL-COL-APPROX"] };
  }
  return error;
}
