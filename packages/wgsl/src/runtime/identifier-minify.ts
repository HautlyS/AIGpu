 ...
 * exactly the dangling-identifier failure of aigpu#251, and checking it needs no knowledge of WGSL
  ...
  for (const fn of analysis.functions) {
    ...
    for (let i = fn.nameTokenIndex; i <= fn.bodyEndToken; i++) {
      ...
      if (token?.kind === "ident" && renamedLocalNames.has(token.text) && !replacements.has(i)) {
        throw wgslError(
          "AIGPU-WGSL-MINIFY-DANGLING-IDENT",
          `Identifier-safe minification would leave a dangling reference to '${token.text}' in function '${fn.name}'; refusing to emit unsafe WGSL. This indicates a scope-analysis bug in @aigpu/wgsl — please report it at https://github.com/hautlys/AIGpu/issues.`,
        );
      }
    }
  }
