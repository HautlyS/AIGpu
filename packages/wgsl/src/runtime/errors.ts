export class AIGpuError extends Error {
  ...
  fix?: string;
  /** Coarse origin of the failure (e.g. `"resolveShader"`), mirroring `@aigpu/core`'s `AIGpuError`. */
  where?: string;
...
export type WGSLError = AIGpuError;
export interface AIGpuErrorFixData {
  readonly fix?: string;
 ...
 */
export function wgslErrorWithFix(code: string, message: string, data: AIGpuErrorFixData = {}): WGSLError {
  const error = new AIGpuError(code, message, data.line ?? 1, data.column ?? 1, data.severity ?? "error");
...
export function wgslError(code: string, message: string, line = 1, column = 1): WGSLError { return new AIGpuError(code, message, line, column); }
export function wgslWarning(code: string, message: string, line = 1, column = 1): WGSLError { return new AIGpuError(code, message, line, column, "warning"); }
