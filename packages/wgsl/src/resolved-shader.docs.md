...
```ts
import type { ResolvedShader, ShaderSource, SourceMap, WGSLAst, WGSLSource } from "@aigpu/wgsl";
```
...
| cacheKey | `Record<string, string>` | ✔ | — | Deterministic FNV-style key in the form `aigpu-wgsl-1:<hash>` under `default`. |
...
```ts
import { compile, type ResolvedShader, type ShaderSource } from "@aigpu/wgsl";
...
- **`entryPoints` here is not reflection.** `ResolvedShader.entryPoints` (this page, `compile()`'s output) is just `readonly string[]` — names matched by a regex over `@(vertex|fragment|compute) fn <name>`, nothing more. It is a different, older, and much simpler shape than the reflection `EntryPointInfo[]` returned by `reflectSource()` and by `resolveShader()`'s `ResolvedShader.reflection.entryPoints` (`@aigpu/wgsl/runtime`), which carries `stage`, `workgroupSize`, `inputs`, `bindings`, and `samplingPairs` per entry point. If you need stage/binding/input data, reach for `reflectSource` (`npx aigpu docs cat /@aigpu/wgsl/reflect-source/reflect-source.docs.md`) or `resolveShader`, not `compile()`.
