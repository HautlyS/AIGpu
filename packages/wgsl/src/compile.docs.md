...
Converts one raw runtime WGSL string into the data-only `ResolvedShader` shape used by `@aigpu/wgsl`. Use it when the shader source is already a complete WGSL entry module and does not need import resolution.
...
```ts
import type { ResolvedShader } from "@aigpu/wgsl";
...
**Throws:** `AIGPU-WGSL-RUNTIME-IMPORT` when the trimmed source starts with a top-level `import` statement after comments are stripped — remove the import, pre-resolve with `resolveShader`, or use the Vite/webpack loader.
...
try {
  ...
  if (error instanceof Error && "code" in error) {
    console.log(error.code === "AIGPU-WGSL-RUNTIME-IMPORT");
  }
}
```
