...
function cacheKey(wgsl: string): Record<string, string> {
  ...
  return { default: `aigpu-wgsl-1:${(hash >>> 0).toString(16).padStart(8, "0")}` };
}
...
function runtimeImportError(): RuntimeImportError {
  ...
  const error = new Error(
    "Runtime WGSL strings cannot contain import statements. Use a build-time loader or @aigpu/wgsl/runtime.",
  ) as RuntimeImportError;
  error.name = "AIGPUWGSLRuntimeImportError";
  error.code = "AIGPU-WGSL-RUNTIME-IMPORT";
