...
test("non-exported imports error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { x } from './a.wgsl'; fn main(){x();}", "/a.wgsl": "fn x(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-SYM-NOEXPORT" }));
...
test("conflicting imports error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { a as x } from './a.wgsl'; import { b as x } from './b.wgsl';", "/a.wgsl": "export fn a(){}", "/b.wgsl": "export fn b(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-SYM-IMPORT-SHADOW" }));
test("default imports error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import x from './a.wgsl';", "/a.wgsl": "" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-IMP-DEFAULT" }));
test("side-effect imports error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import './a.wgsl';", "/a.wgsl": "" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-IMP-SIDEEFFECT" }));
test("imports after declarations error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "fn f(){} import { x } from './a.wgsl';", "/a.wgsl": "export fn x(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-IMP-ORDER" }));
test("self imports error", async () => await expect(resolveShader({ entry: "/a.wgsl", modules: { "/a.wgsl": "import { a } from './a.wgsl'; export fn a(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-IMP-SELF" }));
test("absolute imports error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { x } from '/x.wgsl';" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-RES-ABS" }));
...
fn f() { let x = palette; }`,
  ...
  } })).rejects.toMatchObject({ code: "AIGPU-WGSL-NS-NOTVALUE" });
});
...
test("raw import rejected", () => {
  expect(() => compile(`import { color } from "./p.wgsl";`)).toThrow(expect.objectContaining({ code: "AIGPU-WGSL-RUNTIME-IMPORT" }));
});
test("duplicate reachable overrides error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { a } from './a.wgsl'; import { b } from './b.wgsl';", "/a.wgsl": "override SAMPLES=1; export fn a(){}", "/b.wgsl": "override SAMPLES=2; export fn b(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-OVERRIDE-DUP" }));
...
test("duplicate entry points error", async () => await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { a } from './a.wgsl'; @compute @workgroup_size(1) fn main(){}", "/a.wgsl": "@compute @workgroup_size(1) export fn main(){} export fn a(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-ENTRYPOINT-DUP" }));
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("pipeline constants work on Dawn", async () => {
...
test("missing package errors", async () => {
  await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { x } from 'missing-pkg';" }, validate: false }))
    .rejects.toMatchObject({ code: "AIGPU-WGSL-PKG-NOTFOUND" });
});
test("missing package error names the scoped package and teaches the packageMap fix", async () => {
  await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": "import { voronoi3d } from '@aigpu/wgsl-std/noise';" }, validate: false }))
    .rejects.toMatchObject({
      code: "AIGPU-WGSL-PKG-NOTFOUND",
      message: "Package @aigpu/wgsl-std was not found. Map it with packageMap or add the module to modules",
    });
});
...
async function nsValue(expr: string): Promise<void> {
  await expect(resolveShader({ entry: "/m.wgsl", modules: { "/m.wgsl": `import * as math from './p.wgsl'; fn foo(){} fn main(){ ${expr} }`, "/p.wgsl": "export fn x(){}" }, validate: false })).rejects.toMatchObject({ code: "AIGPU-WGSL-NS-NOTVALUE" });
}
