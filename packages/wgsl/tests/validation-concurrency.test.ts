...
test("concurrent validations keep their own diagnostics on the shared device", async () => {
  ...
  expect(rejection.reason).toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN", message: "cannot convert value for bad_marker_1" });
...
test("a rejected validation does not strand the ones queued behind it", async () => {
  ...
  await expect(resolveShader({ entry: "/first.wgsl", validate: "require", modules: { "/first.wgsl": badShader(9) } })).rejects.toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN" });
