...
/**
 * Real device, no mocks: `AIGPU_DOCKER_TEST=1` is this repo's "this runner has a WebGPU device"
 * marker (see validation.test.ts, representative-shaders.test.ts). The docker-gpu CI job runs this
 * file with `AIGPU_VALIDATE=require` so a device regression fails loudly instead of skipping.
 */
const hasDevice = process.env.AIGPU_DOCKER_TEST === "1";
...
test.skipIf(!hasDevice)("require mode throws the naga diagnostic for an invalid shader on the real device", async () => {
  await expect(resolveShader({ entry: "/bad.wgsl", validate: "require", modules: { "/bad.wgsl": "@compute @workgroup_size(1) fn main(){ let x: u32 = 1.0; }" } })).rejects.toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN" });
});
...
test.skipIf(!hasDevice)("whitespace-only minification never returns WGSL the device has not accepted", async () => {
  ...
  if (!outcome.ok) {
    expect(outcome.error).toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN" });
    return;
  }
...
test.skipIf(!hasDevice)("concurrent validations keep their own diagnostics on the real device", async () => {
  ...
  expect((settled[1] as PromiseRejectedResult).reason).toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN", message: expect.stringContaining("vec2<u32>") });
