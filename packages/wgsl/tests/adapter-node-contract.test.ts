...
/**
 * `validation-device.ts` cannot `import type` from `@aigpu/adapter-node` (that would form the
 ...
 */
test("@aigpu/adapter-node still matches the shape validation-device.ts assumes", async () => {
  ...
  const specifier = "@aigpu/adapter-node";
