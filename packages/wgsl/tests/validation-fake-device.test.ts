...
test("tags an approximate column when the line contains substituted identifiers", async () => {
  ...
  await expect(resolveShader({ entry: "/m.wgsl", validate: "auto", modules: twoModules })).rejects.toMatchObject({
    code: "AIGPU-WGSL-NAGA-UNKNOWN",
    columnPrecise: false,
    relatedDiagnostics: [expect.objectContaining({ code: "AIGPU-WGSL-COL-APPROX" })],
    metadata: { codes: ["AIGPU-WGSL-COL-APPROX"] },
  });
});
...
test("keeps the column precise when no identifier was substituted on the line", async () => {
  ...
  try {
    ...
    expect(error).toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN", range: { file: "single.wgsl" }, columnPrecise: true });
...
test("whitespace-only minification fails loudly when only the minified text is invalid", async () => {
  ...
  await expect(resolveShader({ entry: "/m.wgsl", validate: "require", minify: { whitespace: true }, modules: twoModules })).rejects.toMatchObject({ code: "AIGPU-WGSL-NAGA-UNKNOWN" });
  expect(seen).toHaveLength(2);
});
