...
test("unused imported @aigpu/wgsl-std declarations are removed", async () => {
  const colorStd = readFileSync(join(process.cwd(), "packages/wgsl-std/src/color/index.wgsl"), "utf8");
  const resolved = await resolveShader({
    entry: "/main.wgsl",
    packageMap: { "@aigpu/wgsl-std/color": "/std/color.wgsl" },
    validate: false,
    modules: {
      "/main.wgsl": `import { srgbToLinear3 } from "@aigpu/wgsl-std/color";
