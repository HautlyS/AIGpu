...
describe.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("reflection layouts against GPU compute readback", () => {
...
test.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("texture reflection BGL classification validates depth + MRT sampling on GPU", async () => {
  const source = `
