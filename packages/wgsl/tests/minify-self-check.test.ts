...
// Always-on self-check for aigpu#251: before emitting, identifier-safe minification asserts that no
...
test("self-check rejects a replacement map that would orphan a local reference", () => {
  ...
  expect(() => assertNoDanglingLocalReferences(analysis, orphaning)).toThrow(expect.objectContaining({ code: "AIGPU-WGSL-MINIFY-DANGLING-IDENT" }));
