...
test("reserved words are reported at every declaration site", async () => {
  ...
  expect(new Set(diagnostics.map((item) => item.code))).toEqual(new Set(["AIGPU-WGSL-RESERVED-IDENT"]));
});
...
test("reserved words are reported in imported modules with their own file location", async () => {
  ...
  expect(diagnostics).toHaveLength(1);
  expect(diagnostics[0]).toMatchObject({ code: "AIGPU-WGSL-RESERVED-IDENT", line: 2, range: { file: "/paint.wgsl" } });
...
test("valid identifiers and aigpu module syntax produce no diagnostics", async () => {
  const diagnostics = await diagnosticsFor(
    [
