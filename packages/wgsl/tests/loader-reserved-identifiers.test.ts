...
// there pulls the scanner into the browser-facing `@aigpu/wgsl` entry: measured
// 688 B -> 4062 B gzip against a 1024 B budget (`aigpuExportBundleBudgetsGzipBytes`).
...
// (`aigpu check`, resolveShader, and both loaders) instead.
...
test("vite loader fails the build on a reserved identifier in a leaf shader", async () => {
  await expect(transformWgsl(RESERVED_LEAF, "/paint.wgsl")).rejects.toMatchObject({ code: "AIGPU-WGSL-RESERVED-IDENT" });
...
test("webpack loader fails the build on a reserved identifier in a leaf shader", async () => {
  await expect(runWebpackLoader("/paint.wgsl", RESERVED_LEAF)).rejects.toMatchObject({ code: "AIGPU-WGSL-RESERVED-IDENT" });
...
test("vite loader fails the build on a reserved identifier in an imported module", async () => {
  ...
  await expect(transformWgsl({ source, id: entry })).rejects.toMatchObject({ code: "AIGPU-WGSL-RESERVED-IDENT" });
...
test("webpack loader fails the build on a reserved identifier in an imported module", async () => {
  const dir = await graphFixture(RESERVED_MODULE);
  const source = `import { paint } from "./paint.wgsl";\n@fragment fn main() -> @location(0) vec4f { return vec4f(paint()); }\n`;
  await expect(runWebpackLoader(join(dir, "entry.wgsl"), source)).rejects.toMatchObject({ code: "AIGPU-WGSL-RESERVED-IDENT" });
});
