 ...
 * `@aigpu/adapter-node`, so the error translation — verbatim `fix` forwarding, `metadata.causeCode`,
...
test("forwards the adapter-node code, fix and cause on a device failure", async () => {
  const cause = { code: "AIGPU-NODE-NO-ADAPTER", message: "No WebGPU adapter available with Dawn flags [].", fix: "To diagnose the environment, run: npx aigpu doctor" };
  ...
  expect(error.message).toContain("(AIGPU-NODE-NO-ADAPTER)");
...
test("degrades to a generic fix when the failure carries no code or fix", async () => {
  ...
  await expect(acquireValidationDevice()).rejects.toMatchObject({
    code: "AIGPU-WGSL-VALIDATE-NO-DEVICE",
    message: "device acquisition failed via @aigpu/adapter-node: boom",
    fix: "Run `npx aigpu doctor` to diagnose the local WebGPU/Dawn setup.",
  });
});

test("labels native stderr provenance when the failure exposes it", async () => {
  adapter.thrown = { code: "AIGPU-NODE-NO-ADAPTER", message: "no adapter", fix: "run doctor", detail: { nativeStderr: "vkEnumeratePhysicalDevices failed" } };
  await expect(acquireValidationDevice()).rejects.toMatchObject({
    metadata: { causeCode: "AIGPU-NODE-NO-ADAPTER", nativeStderr: "[dawn/vulkan] vkEnumeratePhysicalDevices failed" },
  });
});
...
test("keeps a failed acquisition memoized instead of retrying it after idle", async () => {
  vi.useFakeTimers();
  adapter.thrown = new Error("boom");
  await expect(acquireValidationDevice()).rejects.toMatchObject({ code: "AIGPU-WGSL-VALIDATE-NO-DEVICE" });
  releaseValidationDevice();
  await vi.advanceTimersByTimeAsync(1_000);
