...
test("normalizeMinifyOption rejects unknown identifier modes", () => {
  expect(() => normalizeMinifyOption({ identifiers: "unsafe" } as never)).toThrow(expect.objectContaining({ code: "AIGPU-WGSL-MINIFY-IDENTIFIERS" }));
});
...
test("minify throws a deterministic error for unterminated block comments", () => {
  expect(() => minifyWgsl("fn main(){ /* unterminated")).toThrow(expect.objectContaining({ code: "AIGPU-WGSL-MINIFY-BLOCK", message: "Unterminated WGSL block comment" }));
});
