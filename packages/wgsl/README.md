...
`@aigpu/wgsl` turns WGSL files into JavaScript shader-source modules and resolves WGSL-to-WGSL imports before bundling. It includes a resolver/runtime, webpack and Vite integrations, and a TypeScript ambient-types sub-export for `import shader from "./shader.wgsl"`.
...
```ts
/// <reference types="@aigpu/wgsl/wgsl-types" />
```
...
```js
module.exports = { module: { rules: [{ test: /\.wgsl$/, loader: "@aigpu/wgsl/loader-webpack" }] } };
```
...
module.exports = {
  module: {
    rules: [
      {
        test: /\.wgsl$/,
        loader: require.resolve("@aigpu/wgsl/loader-webpack"),
        options: { minify: true },
      },
    ],
  },
...
const config: NextConfig = {
  turbopack: {
    rules: {
      "*.wgsl": {
        loaders: ["@aigpu/wgsl/loader-webpack"],
        as: "*.js",
      },
    },
  },
...
const config: NextConfig = {
  turbopack: {
    rules: {
      "*.wgsl": {
        loaders: [{ loader: "@aigpu/wgsl/loader-webpack", options: { minify: true } }],
        as: "*.js",
      },
    },
  },
...
Use legacy `experimental.turbo.rules` on older Next 15.0-15.2 apps. The `as: "*.js"` mapping is required because other bundlers's webpack-loader bridge expects loaders to return JavaScript.
...
See `examples/three-tsl/src/*.wgsl` and `packages/wgsl/tests/*.test.ts` for more examples.
...
```ts
import { reflectSource, resolveShader } from "@aigpu/wgsl/runtime";
...
// Synchronous single-source reflection for main API (aigpu)/runtime WGSL strings.
...
`resolveShader`, `@aigpu/wgsl/loader-webpack`, `@aigpu/wgsl/loader-vite`, and `transformWgsl` accept `minify?: boolean | { whitespace?: boolean; identifiers?: "none" | "safe" }`. The default is `false`, preserving previous output.
...
```ts
import { transformWgsl, wgslVitePlugin } from "@aigpu/wgsl/loader-vite";
...
- other bundlers: the webpack-loader bridge tracks patched async `fs.readFile` calls; the resolver uses `fs/promises.readFile` so transitive imports are intercepted. `resolveShader()` does not keep a stale entry-level `resolveCache`, so reloads re-read changed imports.

## API reference

| Export | Purpose |
| --- | --- |
| `@aigpu/wgsl` | `compile()` for plain WGSL source strings plus shared public types. |
| `@aigpu/wgsl/runtime` | `resolveShader()` for file/module graph resolution and reflection. |
| `@aigpu/wgsl/loader-webpack` | Default webpack-compatible `.wgsl` loader. |
| `@aigpu/wgsl/loader-vite` | Default/named `wgslVitePlugin()` and `transformWgsl()`. |
| `@aigpu/wgsl/wgsl-types` | Ambient `declare module "*.wgsl"` types for TypeScript apps. |
