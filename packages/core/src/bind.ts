...
export function createBindGroup(device: DeviceLike, opts: CreateBindGroupOptions): GPUBindGroup {
  if (!opts.layout) {
    throw new ValidationError({
      code: "AIGPU-CORE-BIND-GROUP-LAYOUT-REQUIRED",
      message: "createBindGroup requires an explicit layout. aigpu does not use layout: \"auto\" for bind groups.",
...
/**
 * Creates a WebGPU sampler from a raw `GPUSamplerDescriptor` plus aigpu sampler sugar.
 *
 * `filter` expands to `magFilter` and `minFilter` only; it does not set
 * `mipmapFilter`. `wrap` expands to `addressModeU`, `addressModeV`, and
 * `addressModeW` using aigpu shorthand values (`"clamp"`, `"repeat"`, `"mirror"`).
 *
...
function expandSamplerDescriptor(descriptor: SamplerDescriptorWithSugar): GPUSamplerDescriptor {
  ...
  if (filter !== undefined && expanded.maxAnisotropy !== undefined && expanded.maxAnisotropy > 1 && !samplerDescriptorHasAllLinearFilters(expanded)) {
    throw new ValidationError({
      code: "AIGPU-CORE-SAMPLER-ANISOTROPY-FILTERS",
      message:
...
function unwrapBindingResource(value: unknown): GPUBindingResource {
  ...
  if (isAIGPUTextureLike(value)) return value.createView();
  if (isAIGPUBufferLike(value)) return { buffer: value.gpu };
...
function isAIGPUBufferLike(value: unknown): value is { readonly gpu: GPUBuffer } {
  return isObject(value) && isRawGPUBuffer(value.gpu);
}

function isAIGPUTextureLike(value: unknown): value is { createView(desc?: GPUTextureViewDescriptor): GPUTextureView } {
...
function explicitBinding(binding: number): number {
  if (!Number.isInteger(binding) || binding < 0) {
    throw new ValidationError({
      code: "AIGPU-CORE-BINDING-INVALID",
...
function shaderStageFlag(name: string): number {
  ...
  if (!flag) throw new ValidationError({ code: "AIGPU-CORE-VISIBILITY-INVALID", message: `Unknown shader stage visibility '${name}'.`, where: "bind" });
  return flag;
}
