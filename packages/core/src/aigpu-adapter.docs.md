...
`AIGpuAdapter` is the minimal core adapter interface. Use it when code should request a aigpu `Device` without caring whether the backing implementation is browser WebGPU, Node/Dawn, or the mock adapter.

## Import

```ts
import type { AIGpuAdapter } from "aigpu/core";
```

## Signature

```ts
import type { Device, RequiredDeviceLimits } from "aigpu/core";
...
| opts.label | `string` | ✖ | `undefined` | Node adapter assigns it to `GPUDevice.label`; browser core request path currently does not assign it in `aigpu-api`, and the mock adapter ignores it. |

**Returns:** `requestDevice(opts?)` returns `Promise<Device>` wrapping the raw `GPUDevice` created by the concrete adapter.

**Throws:**

- `AIGPU-RING1-UNSUPPORTED` may be thrown by higher-level browser/node initialization paths when no adapter factory or browser adapter is available — provide a concrete adapter such as `createMockAdapter()` or run in a WebGPU-capable environment.
- Adapter-specific `AIGPU-*` errors can be thrown before the core `Device` exists, for example Node adapter binary/adapter failures — inspect `.code`, `.message`, and `.fix` on `AIGpuError`.
- `AIGPU-FEATURE-UNSUPPORTED` is thrown before device creation when `requiredFeatures` names a feature the adapter does not support — remove the unsupported name(s) or use an adapter that supports them.
...
```ts
import type { Device, AIGpuAdapter } from "aigpu/core";
import { createMockAdapter } from "aigpu/mock";

async function withDevice(adapter: AIGpuAdapter): Promise<Device> {
...
const adapter: AIGpuAdapter = createMockAdapter();
...
- `AIGpuAdapter` is an interface, not a class. Import it with `import type` unless you only need documentation prose.
- The core interface intentionally has a small portable option set. Concrete adapters can extend it without changing `AIGpuAdapter`.
...
- **See also:** `Device`, `CreateDeviceOptions`, `AIGpuError`, `createMockAdapter`.
