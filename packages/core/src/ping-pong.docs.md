...
`pingPong(device, opts)` creates a double-buffered pair of ordinary AIGPU
...
pair because the halves are intentionally ordinary, individually bindable AIGPU
resources.

## Texture example

```text
import { pingPong, type Device } from "@aigpu/core";
