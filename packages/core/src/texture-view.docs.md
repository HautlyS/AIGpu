...
`cubeView(texture, { compat })` and `layerView(texture, layer)` create explicit `GPUTextureView`s for cubemap and array-layer workflows. They are free functions so they work with both AIGPU `Texture` objects and raw `GPUTexture`s from WebGPU APIs.
...
```text
import { cubeView } from "@aigpu/core";
...
```text
import { layerView } from "@aigpu/core";
