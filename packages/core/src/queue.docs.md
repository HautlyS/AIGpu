...
```ts
import { Queue } from "aigpu/core";
```

## Signature

```ts
import type { BufferWriteData } from "aigpu/core";
...
| Param | Type | Required | Default | Notes |
|---|---|---:|---|---|
| buffer | `GPUBuffer` | ✔ | — | Raw destination buffer. Pass `buffer.gpu` when you have a aigpu `Buffer`. |
...
- `Buffer.write(data, offset = 0)` is the safer high-level call for aigpu buffers; it defaults the offset and checks wrapper lifecycle before reaching the queue.
