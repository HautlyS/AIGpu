...
```ts
import { bind, createBindGroup, createBindGroupLayout, createPipelineLayout, createSampler } from "aigpu/core";
```

## Signature

```ts
import type { Buffer, Device, Texture } from "aigpu/core";
...
|---|---|---:|---|---|
| device | `DeviceLike` | ✔ | — | A raw `GPUDevice`, a aigpu `Device`, or an object with `.gpu: GPUDevice`. Helpers unwrap it before calling WebGPU. |
| binding | `number` | ✔ | — | Explicit non-negative integer `@binding(n)`. Invalid values throw `AIGPU-CORE-BINDING-INVALID`. |
| visibility | `GPUShaderStageFlags \| string \| readonly ("vertex" \| "fragment" \| "compute")[]` | ✔ | — | Numeric flags pass through. Strings split on `|`, comma, or whitespace. Arrays use stage names directly. Unknown names throw `AIGPU-CORE-VISIBILITY-INVALID`. |
...
| opts.layout | `GPUBindGroupLayout` | ✔ | — | Required explicit layout. Missing/falsy layout throws `AIGPU-CORE-BIND-GROUP-LAYOUT-REQUIRED`; aigpu never uses bind group `layout: "auto"`. |
...
| Param | Type | Required | Default | Notes |
|---|---|---:|---|---|
| descriptor | `SamplerDescriptorWithSugar` | ✖ | `{}` | Raw `GPUSamplerDescriptor` plus aigpu sugar. Sugar is stripped before calling WebGPU. |
...
| bind.resource value | `unknown` | ✔ | — | Converts aigpu `Buffer` to `{ buffer: buffer.gpu }`, aigpu `Texture`/texture-like objects to `createView()`, raw `GPUBuffer` to `{ buffer }`, and passes through other binding resources. |

**Returns:**

- `createBindGroupLayout(...)` returns `GPUBindGroupLayout` with aigpu layout metadata attached.
- `createPipelineLayout(...)` returns raw `GPUPipelineLayout`.
- `createBindGroup(...)` returns `GPUBindGroup` with aigpu bind-group metadata attached.
...
- `AIGPU-CORE-BIND-GROUP-LAYOUT-REQUIRED` when `createBindGroup(...)` receives a missing/falsy `opts.layout` — create and pass an explicit `GPUBindGroupLayout`.
- `AIGPU-CORE-SAMPLER-ANISOTROPY-FILTERS` when `createSampler(...)` uses `filter` sugar with `maxAnisotropy > 1` but the resolved `magFilter`, `minFilter`, and `mipmapFilter` are not all `"linear"` — add `mipmapFilter: "linear"` and avoid raw non-linear overrides.
- `AIGPU-CORE-BINDING-INVALID` when any `bind.*` helper receives a negative, non-integer, or otherwise invalid `binding` — pass an explicit non-negative integer.
- `AIGPU-CORE-VISIBILITY-INVALID` when a string/array visibility contains an unknown shader stage name — use only `"vertex"`, `"fragment"`, and `"compute"`, or pass numeric `GPUShaderStageFlags`.
...
```ts
import { createSampler } from "aigpu/core";
import { createMockAdapter } from "aigpu/mock";
...
```ts
import { bind, createBindGroupLayout } from "aigpu/core";
import { createMockAdapter } from "aigpu/mock";
...
- `bind.resource(...)` creates a default texture view for aigpu `Texture`; use a raw `GPUTextureView` when you need a non-default mip/array/format view.
- Keep shader `@group/@binding` numbers synchronized with helper `binding` numbers; aigpu does not reflect or renumber them here.
- **See also:** `Device`, `Buffer`, `Texture`, `Queue`, `AIGpuError`.
