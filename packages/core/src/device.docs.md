...
```ts
import { Device } from "aigpu/core";
```

## Signature

```ts
import type { Buffer, BufferOptions, Queue, Shader, ShaderInput, Texture, TextureOptions, AIGpuError } from "aigpu/core";
...
declare class Device {
  ...
  popErrorScope(): Promise<AIGpuError | null>;
  destroy(): void;
  dispose(): void;
}
```
...
| Param | Type | Required | Default | Notes |
|---|---|---:|---|---|
| input | `ShaderInput` | ✔ | — | A WGSL string or a resolved shader object with `.wgsl`. Strings are compiled with `@aigpu/wgsl` before creating the native shader module. |
...
| Param | Type | Required | Default | Notes |
|---|---|---:|---|---|
| filter | `GPUErrorFilter` | ✔ | — | Passed to `gpu.pushErrorScope(filter)` and also starts a aigpu structured-error scope. |
...
- `popErrorScope()` returns `Promise<AIGpuError | null>`; the first captured aigpu error wins, otherwise a native `GPUError` is converted to `AIGPU-CORE-VALIDATION`, otherwise `null`.

**Throws:**

- `AIGPU-CORE-INVALID-USAGE` when `createBuffer({ size })` receives a non-finite size, `size <= 0`, or an empty `usage` array — pass a positive byte size and at least one buffer usage.
- `AIGPU-CORE-VALIDATION` can be returned from `popErrorScope()` when the native WebGPU scope reports a `GPUError` — inspect `.message` and fix the invalid WebGPU descriptor or command.
...
console.log(badBuffer.options.size, error?.code); // "AIGPU-CORE-INVALID-USAGE"
device.dispose();
```
...
- Prefer `device.createBuffer(...)` and `device.createTexture(...)` over raw `.gpu` creation when you want aigpu wrappers, readback, lifecycle callbacks, or structured core errors.
- `destroy()` is idempotent and `dispose()` is an alias. Do not call `device.gpu.destroy()` directly unless you intentionally bypass aigpu lifecycle.
- `createBuffer` throws immediately without an error scope, but captures into the current aigpu error scope when one is active.
...
- **See also:** `Buffer`, `Texture`, `Queue`, `AIGpuError`, `AIGpuAdapter`.
