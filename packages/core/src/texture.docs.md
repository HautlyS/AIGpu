...
```ts
import { Texture } from "aigpu/core";
```
...
| opts.usage | `readonly TextureUsageName[]` | ✔ | — | AIGpu usage names mapped to `GPUTextureUsage` flags. |
...
| options | `TextureOptions` | ✔ | — | Original aigpu descriptor exposed as `texture.options`. |
...
- `AIGPU-CORE-TEXTURE-DESTROYED` when `view`, `resize(...)`, `read()`, or `readFloats()` is used after `destroy()`/`dispose()` — create a new texture instead.
- `AIGPU-CORE-EXTERNAL-TEXTURE` when `resize(...)` is called on a texture constructed with `ownership: "external"` — resize the owning canvas/swapchain/resource instead.
- `AIGPU-CORE-TEXTURE-RESIZE-LOCKED` when an internal resize lock is active — follow the lock message and resize through the owner that installed the lock.
- `AIGPU-CORE-UNSUPPORTED-FORMAT` when `read()`/`readFloats()` is called on a format outside [Readback formats](#readback-formats) (depth/stencil, packed, snorm/uint/sint, and compressed formats) — blit into a supported format first, or read the data through a storage buffer.
