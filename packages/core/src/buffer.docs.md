...
| opts.usage | `readonly BufferUsageName[]` | ✔ | — | One or more aigpu usage names mapped to `GPUBufferUsage` flags. Empty arrays throw. |
...
| options | `BufferOptions` | ✔ | — | Original aigpu descriptor exposed as `buffer.options`. |
...
- `AIGPU-CORE-INVALID-USAGE` from `Device.createBuffer(...)` when `opts.size` is non-finite, `opts.size <= 0`, or `opts.usage.length === 0` — pass a positive byte length and at least one usage.
- `AIGPU-BUFFER-DISPOSED` (`"Buffer is destroyed."`) when `write(...)` or `read(...)` is called after `destroy()`/`dispose()` — create a new buffer instead of reusing a destroyed wrapper. Reported for a wrapper you disposed and for one destroyed with its gpu, and in preference to the owning device's own disposal error.
