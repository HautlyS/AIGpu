...
import { ValidationError, type AIGpuError } from "./errors.ts";
...
export class Device {
  ...
  readonly isCompatibilityMode: boolean;
  private readonly scopes: AIGpuError[][] = [];
  ...
  wrapBuffer(buffer: GPUBuffer): Buffer {
    ...
    if (!isExternalBufferShape(buffer)) {
      throw new ValidationError({
        code: "AIGPU-EXTERNAL-BUFFER-INVALID",
  ...
  async popErrorScope(): Promise<AIGpuError | null> {
    this.#assertUsable("Device.popErrorScope");
    const scope = this.scopes.pop();
    const nativeError = await this.gpu.popErrorScope?.();
    this.#assertUsable("Device.popErrorScope");
    return scope?.[0] ?? nativeErrorToAIGpuError(nativeError) ?? null;
  }
  ...
  private captureError(error: AIGpuError): void {
...
function invalidUsage(message: string): ValidationError {
  return new ValidationError({ code: "AIGPU-CORE-INVALID-USAGE", message, where: "Device.createBuffer" });
}
...
function nativeErrorToAIGpuError(error: GPUError | null | undefined): AIGpuError | null {
  if (!error) return null;
  return new ValidationError({ code: "AIGPU-CORE-VALIDATION", message: error.message, where: "GPUDevice.popErrorScope", cause: error });
}
