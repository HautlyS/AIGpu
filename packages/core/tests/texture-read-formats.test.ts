...
test("read() rejects formats that have no readback layout, exactly like a real device", async () => {
  ...
  await expect(depth.read()).rejects.toMatchObject({ code: "AIGPU-CORE-UNSUPPORTED-FORMAT" });
  device.destroy();
});
...
test("readFloats() rejects formats that have no readback layout", async () => {
  ...
  await expect(depth.readFloats()).rejects.toMatchObject({ code: "AIGPU-CORE-UNSUPPORTED-FORMAT" });
  device.destroy();
});
...
test("readFloats() throws on a destroyed texture instead of decoding stale bytes", async () => {
  ...
  await expect(texture.readFloats()).rejects.toMatchObject({ code: "AIGPU-CORE-TEXTURE-DESTROYED" });
  device.destroy();
});
...
test("mock writeTexture uploads multiple layers using rowsPerImage", async () => {
  ...
  expect([...(await texture.read())]).toEqual([1, 2, 3, 4]);
  expect([...texture.gpu.__aigpuMockBytes.subarray(4)]).toEqual([5, 6, 7, 8]);
  device.destroy();
});
