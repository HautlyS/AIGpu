...
test("device loss preserves reason and message and invalidates public operations", async () => {
  ...
  expect(() => device.createBuffer({ size: 4, usage: ["copy_dst"] })).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-LOST", message: expect.stringContaining("ORT released the device") }));
  device.dispose();
...
test("wrapBuffer preserves identity, metadata, signal semantics, and never native-destroys", () => {
  ...
  expect(() => wrapped.write(new Uint32Array([1]))).toThrow(expect.objectContaining({ code: "AIGPU-BUFFER-DISPOSED" }));
  sibling.dispose();
  expect(raw.destroy).not.toHaveBeenCalled();
  device.dispose();
});
...
test("external buffer validates usage, alignment, and range", async () => {
  ...
  expect(() => wrapped.write(new Uint32Array([1]))).toThrow(expect.objectContaining({ code: "AIGPU-EXTERNAL-BUFFER-VALIDATION" }));
  await expect(wrapped.read(4)).rejects.toMatchObject({ code: "AIGPU-EXTERNAL-BUFFER-VALIDATION" });
  ...
  expect(() => copyable.write(new Uint32Array([1]))).toThrow(expect.objectContaining({ code: "AIGPU-EXTERNAL-BUFFER-VALIDATION", cause: nativeCause }));
  expect(() => copyable.write(new Uint32Array([1]), 2)).toThrow(expect.objectContaining({ code: "AIGPU-EXTERNAL-BUFFER-VALIDATION" }));
  await expect(copyable.read(20)).rejects.toMatchObject({ code: "AIGPU-EXTERNAL-BUFFER-VALIDATION" });
  device.dispose();
});
...
test("retained texture operations reject after device disposal", () => {
  ...
  device.dispose();
  expect(() => texture.createView()).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-DISPOSED" }));
  expect(() => texture.resize([2, 2])).toThrow(expect.objectContaining({ code: "AIGPU-DEVICE-DISPOSED" }));
  expect(f.destroy).not.toHaveBeenCalled();
});

test("wrapBuffer rejects non-observable buffer shapes", () => {
  const f = fakeDevice(); const device = new Device(f.gpu, null, "external");
  expect(() => device.wrapBuffer({ size: -1, usage: 4, destroy() {} } as unknown as GPUBuffer)).toThrow(expect.objectContaining({ code: "AIGPU-EXTERNAL-BUFFER-INVALID" }));
  device.dispose();
});
