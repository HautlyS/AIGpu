 ...
 * __aigpuMockBytes, so exercising the staging path needs a plain stub.
 */
...
test("readTexture() rejects unsupported formats before allocating a staging buffer", async () => {
  ...
  await expect(new Readback(device).readTexture(texture, [1, 1], "depth24plus")).rejects.toMatchObject({ code: "AIGPU-CORE-UNSUPPORTED-FORMAT" });
