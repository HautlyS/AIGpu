...
`createMockAdapter(options?)` returns a pure-JavaScript `AIGpuAdapter`. Its devices allocate
...
`requiredFeatures` entry outside that set with `AIGPU-FEATURE-UNSUPPORTED`, and —
