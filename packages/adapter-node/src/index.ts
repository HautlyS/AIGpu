...
import { Device, validateRequiredFeatures, AIGpuError, type CreateDeviceOptions, type AIGpuAdapter } from "@aigpu/core";
...
export function nodeAdapterEnvironmentOverride(): NodeAdapterMode | undefined {
  const value = process.env.AIGPU_ADAPTER;
  if (value === undefined || value === "") return undefined;
  if (value !== "software" && value !== "hardware") throw new AIGpuError({
    code: "AIGPU-NODE-ADAPTER-INVALID",
    message: `Invalid AIGPU_ADAPTER=${JSON.stringify(value)}; expected "software" or "hardware".`,
    fix: "Unset AIGPU_ADAPTER or set it to software or hardware.",
    where: "init",
  });
  if (!announcedAdapterOverrides.has(value)) {
    announcedAdapterOverrides.add(value);
    console.error(`aigpu: adapter overridden by AIGPU_ADAPTER=${value}`);
  }
  return value;
}
...
export function createNodeAdapter(options: CreateNodeAdapterOptions = {}): AIGpuAdapter {
...
async function requestDevice(opts: RequestDeviceOptions = {}, mode: NodeAdapterMode): Promise<Device> {
  ...
  if (mode === "software") {
    ...
    try {
      ...
      if (!icd) throw noAdapterError(error, "Install the portable CPU renderer with `npx aigpu install-software-renderer`, then retry. Check the Mesa/driver version, Vulkan ICD (VK_ICD_FILENAMES), and display variables. To diagnose the environment, run: npx aigpu doctor");
...
function noAdapterError(cause: unknown, fix = "Check the Mesa/driver version, Vulkan ICD (VK_ICD_FILENAMES), and display variables (DISPLAY, WAYLAND_DISPLAY, XDG_RUNTIME_DIR). Use AIGPU_DAWN_FLAGS to select a Dawn backend explicitly. Install a portable CPU renderer with `npx aigpu install-software-renderer`. To diagnose the environment, run: npx aigpu doctor"): AIGpuError {
  return new AIGpuError({
    code: "AIGPU-NODE-NO-ADAPTER",
...
function isNoAdapterError(error: unknown): boolean { return error instanceof AIGpuError && error.code === "AIGPU-NODE-NO-ADAPTER"; }
...
function requireCachedSoftwareRenderer(): string {
  ...
  throw new AIGpuError({
    code: "AIGPU-NODE-SOFTWARE-RENDERER-MISSING",
    message: "The portable CPU software renderer is not installed.",
    fix: "Run `npx aigpu install-software-renderer`, then retry.",
    where: "createNodeAdapter",
  });
}
...
function getDawnGPU(opts: RequestDeviceOptions, webgpu: WebGPUModule, forceSoftware = false): GPU {
  ...
  if (dawnGPU) {
    if (!flagsEqual(flags, dawnFlagsUsed)) console.warn(`[@aigpu/adapter-node] Dawn already initialized with flags [${dawnFlagsUsed?.join(",") ?? ""}], ignoring requested flags [${flags.join(",")}]. Re-init causes SIGSEGV in Dawn.`);
    return dawnGPU;
  }
...
function backendFlags(opts: RequestDeviceOptions): readonly string[] {
  const envFlags = process.env.AIGPU_DAWN_FLAGS?.split(/\s+/).filter(Boolean);
