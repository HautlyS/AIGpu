...
export const dawnReleaseTag = "dawn-v0.4.0-aigpu.1";
export const dawnArtifactVersion = "0.4.0-aigpu.1";
...
const publicAssetUrl = `https://github.com/hautlys/AIGpu/releases/download/${dawnReleaseTag}/${dawnAssetName}`;
const githubReleasesApiUrl = "https://api.github.com/repos/hautlys/AIGpu/releases?per_page=100";
const manualInstall = "Run `pnpm exec aigpu install-dawn` (or `npx @aigpu/cli install-dawn`) with network access, or set AIGPU_DAWN_BINARY to a verified Dawn .node file.";
...
export function dawnCachePath(options: Pick<DawnInstallOptions, "cacheRoot"> = {}): string {
  const root = options.cacheRoot ?? process.env.AIGPU_CACHE_DIR ?? process.env.XDG_CACHE_HOME ?? join(homedir(), ".cache");
  return join(root, "aigpu", "dawn", dawnArtifactVersion, "linux-arm64-gnu", dawnAssetName);
}
...
export function createPrivateDawnCopy(source: string, expectedSha256?: string): PrivateDawnCopy {
  ...
  const directory = mkdtempSync(join(tmpdir(), "aigpu-dawn-load-"));
...
export async function installDawn(options: DawnInstallOptions = {}): Promise<DawnInstallResult> {
  ...
  try {
    ...
    if (cause instanceof AIGpuError) throw cause;
...
export function getUnsupportedPrebuildReason(
  ...
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  if (platform !== "linux") return `unsupported platform ${platform}/${arch}; aigpu prebuilds currently target Linux arm64 only`;
  if (arch !== "arm64") return `unsupported platform ${platform}/${arch}; no aigpu prebuild is published for this CPU yet`;
...
async function resolveAuthenticatedAsset(
  ...
  const headers = { Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json", "User-Agent": "@aigpu/adapter-node" };
  ...
  return { url: asset.url, headers: { Authorization: `Bearer ${token}`, Accept: "application/octet-stream", "User-Agent": "@aigpu/adapter-node" } };
}
...
function checksumError(path: string, reason: string, cause?: unknown): AIGpuError {
  return new AIGpuError({
    code: "AIGPU-NODE-PREBUILD-CHECKSUM",
...
function prebuildMissing(reason: string, cause: unknown, platform: string, arch: string): AIGpuError {
  return new AIGpuError({
    code: "AIGPU-NODE-PREBUILD-MISSING",
