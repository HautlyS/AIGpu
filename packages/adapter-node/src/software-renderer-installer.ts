...
export async function installSoftwareRenderer(options: SoftwareRendererInstallOptions = {}): Promise<SoftwareRendererInstallResult> {
  ...
  if (platform !== "linux" || !softwareRendererExpectedHashes[arch]) throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-UNSUPPORTED", `The portable software renderer is not available for ${platform}/${arch}; it currently supports Linux x64 and arm64.`);
  ...
  if (!fetchImpl) throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-DOWNLOAD", "Software renderer download failed: this Node.js runtime does not provide fetch.");
  mkdirSync(dirname(destination), { recursive: true, mode: 0o700 });
  const temporary = `${destination}.${process.pid}.${randomUUID()}.tmp`;
  mkdirSync(temporary, { mode: 0o700 });
  const asset = `mesa-lavapipe-25.0.7-linux-${arch}.tar.gz`;
  const archive = join(temporary, asset);
  const url = `https://github.com/hautlys/AIGpu/releases/download/${softwareRendererReleaseTag}/${asset}`;
  ...
  try {
    ...
    throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-DOWNLOAD", `Software renderer download failed: ${String(cause)}`, cause);
