...
export async function resolveWebGPU(options: DawnResolverOptions = {}): Promise<WebGPUModule> {
  ...
  if (env.AIGPU_DAWN_BINARY) {
    const path = resolve(env.AIGPU_DAWN_BINARY);
    return requireNative(requireImpl, path, "AIGPU_DAWN_BINARY");
  }
  ...
  if (!unsupportedPrebuild) {
    ...
    if (cached) return requireVerifiedNative(requireImpl, cached, "aigpu cache", options.expectedSha256);
  }
  ...
  try {
    ...
    return requireVerifiedNative(requireImpl, installed.path, "downloaded aigpu prebuild", options.expectedSha256);
  } catch (prebuildError) {
...
function requireNative(requireImpl: Require, path: string, provider: string, reportedPath = path): WebGPUModule {
  try {
    ...
    throw new AIGpuError({
      code: mismatch ? "AIGPU-NODE-GLIBC-MISMATCH" : "AIGPU-NODE-NATIVE-LOAD",
      message: mismatch
        ? `Dawn from ${provider} requires GLIBC ${mismatch.required}, but this host reports GLIBC ${mismatch.host ?? "unknown"}.`
        : `@aigpu/adapter-node could not load Dawn from ${provider} (${reportedPath}).`,
      fix: mismatch
        ? "Install the portable prebuild with `pnpm exec aigpu install-dawn`, or set AIGPU_DAWN_BINARY to a compatible binary. Do not upgrade glibc in place."
...
function combineResolutionErrors(stockError: unknown, prebuildError: unknown, platform: string, arch: string): AIGpuError {
  const mismatch = glibcMismatch(stockError);
  if (prebuildError instanceof AIGpuError && prebuildError.code === "AIGPU-NODE-PREBUILD-MISSING") {
  ...
  if (prebuildError instanceof AIGpuError) return prebuildError;
  return new AIGpuError({
    code: "AIGPU-NODE-PREBUILD-MISSING",
    message: `Neither stock webgpu nor the aigpu Dawn prebuild could load on ${platform}/${arch}.`,
    fix: "Run `pnpm exec aigpu install-dawn`, or set AIGPU_DAWN_BINARY to a compatible binary.",
    where: "createNodeAdapter",
    cause: { stock: stockError, prebuild: prebuildError },
  });
}
