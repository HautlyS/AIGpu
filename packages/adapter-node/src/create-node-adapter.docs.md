...
`createNodeAdapter()` returns a Dawn-backed `AIGpuAdapter` using the
`webgpu@0.4.0` API. The native loader resolves Dawn in this order:

1. the file named by `AIGPU_DAWN_BINARY`;
2. a verified aigpu prebuild in the versioned user cache;
3. the stock `webgpu` package;
4. on Linux, a lazy download of the aigpu prebuild if stock Dawn cannot load.
...
`hautlys/AIGpu` GitHub Release and are accepted only when their SHA-256 equals
the hash pinned in `@aigpu/adapter-node`. Private-repository and draft-release
access honors `GH_TOKEN` or `GITHUB_TOKEN`; published public assets are fetched
without a token. Override the cache root with `AIGPU_CACHE_DIR` (or
...
```sh
pnpm exec aigpu install-dawn
# or
npx aigpu install-dawn
```

`AIGPU-NODE-PREBUILD-MISSING` includes the failed reason and those remediation
commands. `AIGPU-NODE-PREBUILD-CHECKSUM` refuses a corrupt or substituted asset.
`AIGPU-NODE-GLIBC-MISMATCH` names both versions when a selected native binary
cannot load. Do not upgrade a host's glibc in place; install the portable
prebuild or provide an audited binary with `AIGPU_DAWN_BINARY`.
...
`VK_ICD_FILENAMES`; `AIGPU_DAWN_FLAGS=backend=vulkan` remains available when
strict backend selection is required. Mesa and GPU drivers remain host-managed
and are never downloaded by aigpu. `AIGPU_DAWN_FLAGS` may contain any
...
window, `AIGPU-NODE-NO-ADAPTER` reports the request options and Dawn flags and
...
(such as `pngjs`, `pixelmatch`, or existing snapshots), not in the AIGPU API. See
`createNodeDevice` for a full native-before/AIGPU-after guide.
