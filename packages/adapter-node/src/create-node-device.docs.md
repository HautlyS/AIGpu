...
the Node adapter directly. Use `createNodeDevice()` for focused scripts, server-side renders, and agentic headless snapshot tests that need one explicit WebGPU device. Application code can also use `init()` from `aigpu/node` for the main API (`aigpu`) facade.
...
bytes to project-owned PNG or pixel-diff tooling. AIGPU does not provide an
...
`Texture.read()` wraps that readback path for AIGPU textures, including the
...
WebGPU pipeline creation through `.gpu` because AIGPU intentionally keeps that
escape hatch available for native interop; wrapper lifecycle methods should still
own teardown.

```text
import { createNodeDevice } from "@aigpu/adapter-node";
...
Keep PNG encoding and image comparison outside the AIGPU API. A Vitest/Jest-style
...
if (process.env.AIGPU_WRITE_SNAPSHOTS === "1") {
...
(2.38 on arm64). aigpu no longer treats that as a hard requirement: when the
stock binary fails to load, the adapter falls back to aigpu's portable Dawn
...
manually with `npx aigpu install-dawn`. `AIGPU_DAWN_BINARY` overrides resolution
entirely for air-gapped hosts. Load failures surface as structured
`AIGPU-NODE-*` errors that name the platform, the reason, and the fix.
...
```bash
AIGPU_WRITE_SNAPSHOTS=1 pnpm test:docker
```
...
aigpu can render on CPU anywhere. Check the machine first:
...
This downloads aigpu's portable lavapipe build (Mesa 25, ~20 MB, sha256-verified,
...
Those lines come from the driver stack, not from aigpu; the prebuilt Dawn binding
exposes no logging hook, so aigpu cannot capture or relabel them. Instead, when a
run ends up on the CPU renderer, aigpu prints one labelled notice on stderr —
once per process, after the adapter is known, so it lands *below* the native
lines it explains:

```text
aigpu: notice — no hardware GPU adapter is available; using CPU software renderer (llvmpipe (LLVM 19.1.7, 128 bits)). This is expected on a machine without a usable GPU, and rendering continues normally.
aigpu: notice — Vulkan/XDG_RUNTIME_DIR "error" and "Warning" lines printed above come from the GPU driver stack, not from aigpu, and are harmless. Run `npx aigpu doctor` for details.
```
...
The `AIGPU_ADAPTER` environment variable overrides the code-level choice and
announces itself on stderr — handy for forcing a mode in CI without editing
code.

## `.gpu` lifecycle guidance

`.gpu` is a raw WebGPU escape hatch. Use it for operations AIGPU does not wrap,
such as custom pipelines, render passes, query sets, or native interop. Prefer
AIGPU wrapper lifecycle methods for resources AIGPU owns: call `texture.destroy()`,
...
- **Unsupported format**: `Texture.read()` supports the formats documented by
  ...
  snorm/uint/sint, compressed) throw `AIGPU-CORE-UNSUPPORTED-FORMAT`.
...
- **Native environment failures**: Xvfb or Mesa setup problems are
  ...
  the structured `AIGPU-NODE-*` error names the fix (typically
  `npx aigpu install-dawn` or `AIGPU_DAWN_BINARY`).
