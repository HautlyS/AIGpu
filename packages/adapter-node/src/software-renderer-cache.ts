...
export const softwareRendererReleaseTag = "lavapipe-v25.0.7-aigpu.1";
export const softwareRendererVersion = "25.0.7-aigpu.1";
...
export function softwareRendererCacheDirectory(options: Pick<SoftwareRendererCacheOptions, "cacheRoot" | "arch"> = {}): string {
  ...
  return join(root, "aigpu", "software-renderer", softwareRendererVersion, `linux-${options.arch ?? process.arch}`);
}
...
export function softwareRendererExpectedHash(arch: string): string {
  ...
  if (!hash) throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-UNSUPPORTED", `No portable software renderer checksum is pinned for ${arch}.`);
  return hash;
}
...
function verifyFileHash(path: string, expected: string): void {
  ...
  if (actual !== expected) throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-CHECKSUM", `Refusing software renderer ${path}: SHA-256 ${actual} does not match pinned ${expected}.`);
}
export function createPrivateSoftwareRendererCopy(icd: string, arch = process.arch): { readonly path: string; readonly cleanup: () => void } {
  ...
  const directory = mkdtempSync(join(tmpdir(), "aigpu-lavapipe-load-"));
...
function copyVerified(source: string, destination: string, expected: string, mode: number): void {
  ...
  if (actual !== expected) throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-CHECKSUM", `Refusing software renderer ${source}: SHA-256 ${actual} does not match pinned ${expected}.`);
}
export function assertSoftwareRendererRegularFile(path: string): void {
  const stats = lstatSync(path);
  if (stats.isSymbolicLink() || !stats.isFile()) throw softwareRendererError("AIGPU-NODE-SOFTWARE-RENDERER-CHECKSUM", `Refusing software renderer cache path ${path}: it is not a regular non-symlink file.`);
}
export function softwareRendererError(code: string, message: string, cause?: unknown): AIGpuError {
  return new AIGpuError({ code, message, fix: "Run `npx aigpu install-software-renderer` with network access.", where: "installSoftwareRenderer", cause });
}
