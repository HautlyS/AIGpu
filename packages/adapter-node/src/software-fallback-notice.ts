 ...
 * relabelled from JavaScript. aigpu instead prints one clearly labelled notice after the
...
export function formatSoftwareFallbackNotice({ adapter, reason }: SoftwareFallbackNotice): string {
  ...
  return [
    `aigpu: notice — ${cause}; using CPU software renderer (${adapter}). This is expected on a machine without a usable GPU, and rendering continues normally.`,
