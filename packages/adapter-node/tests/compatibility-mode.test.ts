...
test("node adapter reports structured diagnostics after identical adapter retries", async () => {
  ...
  expect(error).toMatchObject({ name: "AIGpuError", code: "AIGPU-NODE-NO-ADAPTER", where: "createNodeAdapter" });
...
test("AIGPU_ADAPTER accepts hardware/software, announces the override, and rejects invalid values", async () => {
  const previous = process.env.AIGPU_ADAPTER;
  const error = vi.spyOn(console, "error").mockImplementation(() => undefined);
  const { nodeAdapterEnvironmentOverride } = await import("../src/index.ts");
  try {
    process.env.AIGPU_ADAPTER = "hardware";
    expect(nodeAdapterEnvironmentOverride()).toBe("hardware");
    expect(error).toHaveBeenCalledWith("aigpu: adapter overridden by AIGPU_ADAPTER=hardware");
    process.env.AIGPU_ADAPTER = "invalid";
    expect(() => nodeAdapterEnvironmentOverride()).toThrowError(expect.objectContaining({ code: "AIGPU-NODE-ADAPTER-INVALID" }));
  } finally {
    if (previous === undefined) delete process.env.AIGPU_ADAPTER;
    else process.env.AIGPU_ADAPTER = previous;
    error.mockRestore();
  }
});
