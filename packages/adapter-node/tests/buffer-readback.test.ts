...
describe.skipIf(process.env.AIGPU_DOCKER_TEST !== "1")("adapter-node Docker GPU", () => {
  ...
  test("node error scope captures validation error from invalid buffer creation", async () => {
    ...
    await expect(device.popErrorScope()).resolves.toMatchObject({ name: "ValidationError", code: "AIGPU-CORE-INVALID-USAGE" });
    device.destroy();
  });
