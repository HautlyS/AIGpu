...
function tempDir(): string {
  const path = mkdtempSync(`${tmpdir()}/aigpu-dawn-test-`);
...
describe("Dawn prebuild integrity", () => {
  test("accepts the expected hash and refuses a hash mismatch", () => {
    ...
    expect(() => verifyDawnBinary(path)).toThrowError(expect.objectContaining({ code: "AIGPU-NODE-PREBUILD-CHECKSUM" }));
  });
  ...
  test("rejects a symlinked cache entry", async () => {
    ...
    expect(() => getCachedDawnBinary({ cacheRoot, expectedSha256: fixtureSha256 })).toThrowError(
      expect.objectContaining({ code: "AIGPU-NODE-PREBUILD-CHECKSUM" }),
    );
  });
});

describe("Dawn resolution order", () => {
  test("AIGPU_DAWN_BINARY wins over cache and stock webgpu", async () => {
    const calls: string[] = [];
    const envPath = resolve(tempDir(), "custom.node");
    const loaded = await resolveWebGPU({
      env: { AIGPU_DAWN_BINARY: envPath },
      require: ((id: string) => {
        calls.push(id);
        return fakeModule;
      }) as NodeJS.Require,
...
describe("Dawn error taxonomy", () => {
  ...
  test("reports unsupported platform when stock and prebuild are unavailable", async () => {
    ...
    ).rejects.toMatchObject({ code: "AIGPU-NODE-PREBUILD-MISSING", message: expect.stringContaining("unsupported platform darwin/arm64") });
  });
  ...
  test("bounds requests even when fetch ignores AbortSignal", async () => {
    ...
    ).rejects.toMatchObject({ code: "AIGPU-NODE-PREBUILD-MISSING", message: expect.stringContaining("timed out") });
  });
  ...
  test("reports offline download failure with manual install guidance", async () => {
    await expect(
      ...
      fix: expect.stringContaining("aigpu install-dawn"),
    });
  });
});
